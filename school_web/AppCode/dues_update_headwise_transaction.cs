...
{
    public class dues_update_headwise_transaction
    {

        internal static void update_student_dues(string session_id, string class_id, string admission_no, string firm_id, string isFineRepeate, SqlConnection con)
        {
            string Discount_on = "";
            My mycode = new My();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    payments.exeSql("Delete from Student_wise_dues_amount where admission_no='" + admission_no + "' and Class_id='" + class_id + "' and Session_id='" + session_id + "'", con);
                    ...
                    if (dtMnth.Rows.Count > 0)
                    {
                        foreach (DataRow drMnth in dtMnth.Rows)
                        {
                            ...
                            {
                                if (firm_id == "MDA-01" || firm_id == "MDA-02")
                                {
                                    fine_calculations(session_id, class_id, admission_no, dr["Session_id"].ToString(), dr["session"].ToString(), drMnth["Month"].ToString(), drMnth["Month_Id"].ToString(), "1", isFineRepeate, con);
                        ...
                        if (fdt.Rows.Count > 0)
                        {
                            ...
                            try
                            {
                                ...
                                sbc.DestinationTableName = "Student_wise_dues_amount";
                                sbc.BatchSize = 10000;
                    ...
                    string qry = "select '0' as Id,*,(amount - cast(disc_amount as float) - cast(Prev_paid as float)) Dues_amt,'0' as Month_position,'0' as Month_id,'" + mycode.date() + "' as Updated_date,'" + mycode.time() + "' as Updated_time from (select admission_no admission_no, Class_id class_id, '" + dr["Session_id"] + "' Session_id, parameter, 'April' as months, feetype content, content_id, cast(payable as float) amount, Disc as disc_amount, paid Prev_paid  from dbo.[Typewise_fee_collection] WHERE admission_no = '" + admission_no + "' and parameter = '" + parameterAd + "' and session = '" + dr["session"].ToString() + "') t";
                    DataTable fee_dt = payments.dataTable(qry, con);
                    if (fee_dt.Rows.Count == 0)
                    {
                        if (Hostel_id == "0")
                        {
                            qry = "select '0' as Id,Admission_No admission_no, '" + dr["Class_id"].ToString() + "' Class_id,Session_id,Type_Mode as parameter,'April' as months,Perticular as content,'ANN01' as content_id,Amount as amount, '0' disc_amount,'0' as Prev_paid, Amount as Dues_amt,'0' as Month_position,'0' as Month_id,'" + mycode.date() + "' as Updated_date,'" + mycode.time() + "' as Updated_time from Misc_Fee_Master_Studentwise where (Type_Mode='AdmissionFee' or Type_Mode='AnnualFee') and session='" + dr["session"].ToString() + "' and Admission_No='" + admission_no + "' UNION ALL  select '0' as Id,*,(amount-cast(disc_amount as float)) Dues_amt,'0' as Month_position,'0' as Month_id,'" + mycode.date() + "' as Updated_date,'" + mycode.time() + "' as Updated_time from(select '" + admission_no + "' as admission_no,class_id Class_id,fmc.session_id Session_id,parameter,'April' as months,cm.content content,cm.content_id,cast(fmc.amount as float) amount,isnull((isnull((select top 1 disc_amount from dbo.[Discount_Master] where admission_no='" + admission_no + "'  and parameter_id='" + parameter_idAd + "' and session='" + dr["session"].ToString() + "' and fee_head_id=cm.content_id and Discount_on='" + Discount_on + "'),(select top 1 disc_amount from dbo.[Discount_Master] where Class_id='" + dr["Class_id"].ToString() + "' and admission_no='All'  and parameter_id='" + parameter_idAd + "' and session='" + dr["session"].ToString() + "' and fee_head_id=cm.content_id and Discount_on='"+ Discount_on + "' and category_id='" + dr["category_id"].ToString() + "' and sub_category_id='" + dr["SubCategory_id"].ToString() + "'))),0) disc_amount,'0' Prev_paid from dbo.[Fee_master_content_wise] fmc join Content_master cm on fmc.content_id=cm.content_id where parameter='" + parameterAd + "'  and session='" + dr["session"].ToString() + "' and class_id='" + dr["Class_id"].ToString() + "')t";
                        }
                        else
                        {
        ...
        private static void save_fine_amount(string session_id, string class_id, string admission_no, string Session_id, string session, string Month_name, string Month_Id, string Branch_id, string MonthPosition, string parameter, SqlConnection con)
        {
            ...
            if (dtFine.Rows.Count > 0)
            {
                ...
                {
                    SqlCommand cmd;
                    string query = "INSERT INTO STUDENT_WISE_DUES_AMOUNT (admission_no,Class_id,Session_id,parameter,months,content,content_id,amount,disc_amount,Prev_paid,Dues_amt,Month_position,Month_id,Updated_date,Updated_time) values (@admission_no,@Class_id,@Session_id,@parameter,@months,@content,@content_id,@amount,@disc_amount,@Prev_paid,@Dues_amt,@Month_position,@Month_id,@Updated_date,@Updated_time);";
        ...
        private static void fine_calculations(string session_id, string class_id, string admission_no, string Session_id, string session, string Month_name, string Month_Id, string Branch_id, string isFineRepeate, SqlConnection con)
        {
                        ...
                        if (My.toint(fine_applicable_month) <= pay_month_with_year)
                        {
                            if (isFineRepeate.ToUpper() == "YES")
                            {
                                ...
                                if (My.toDouble(totalmonths) > 0)
                                {
                                    ...
                                    save_fine_amount(session_id, admission_no, pay_month_two_digit, ttl_fine_amt, Branch_id, con);
                                }
                                else
                                {
                                    save_fine_amount(session_id, admission_no, pay_month_two_digit, 0, Branch_id, con);
                                }
                            }
                            else
                            {

    ...
    public class extra_fee
    {
        ...
        public void Bind_fee_details(string Payment_type, string Session, string Session_id, string Class_id, string regid, string category_id, string sub_category_id, string Total_pay, string Date, string Name, string Branch_id, string razorpay_payment_id, string Section, string orderid, string paymentmode, string hostaltaken, string Hostel_id, string paymentfrom, string classname)
        {
            ...
            else
            {
                ...
                string previous_dues = "0";//get_previous_amount(regid, Session_id, Class_id);
        ...
        private void send_data_in_feetypewise_collection(string Payment_type, string session, string session_id, string class_id, string regid, string category_id, string sub_category_id, string total_pay, string date, string type, string name, string slip_no, string entry_id, string total_discount, string branch_id, string razorpay_payment_id, string section, string previous_dues, string hostaltaken, string Hostel_id)
        {
            ...
            SqlDataAdapter ad = new SqlDataAdapter(" select * from Typewise_fee_collection  where admission_no='" + regid + "' and session='" + session + "' and status='Dues' and parameter='" + parameter + "' and content_id='ADM01'", My.conn);
            DataSet ds = new DataSet();
                ...
                foreach (DataRow dr in tdt.Rows)
                {
                    if (paid_amount >= 0)
                    {
                        ...
                        if (paid_amount >= dues)
                        {
                            ...
                            send_data_in_fee_collection_slip(dr["payable"].ToString(), dues.ToString(), dr["parameter"], dr["feetype"], dr["content_id"], slip_no, entry_id, dr["Ledger"], regid, session, class_id, section, dr["Disc"].ToString(), dr["month"], dr["position"], prevpaid, date, branch_id);
                            #endregion
                        }
                        else
                        {
                            ...
                            #region send in collection slip 
                            send_data_in_fee_collection_slip(dr["payable"].ToString(), paid_amount.ToString(), dr["parameter"], dr["feetype"], dr["content_id"], slip_no, entry_id, dr["Ledger"], regid, session, class_id, section, dr["Disc"].ToString(), dr["month"], dr["position"], prevpaid, date, branch_id);
                            #endregion
        ...
        { 
            DataTable dt = mycode.FillData("select Dues_Amount from Previous_Year_Dues  where Session_id='" + Session_id + "' and AdmissionNumber='" + regid + "'  and Class_id='" + Class_id + "'");
            if (dt.Rows.Count == 0)
            {
                return "0.00";
            }

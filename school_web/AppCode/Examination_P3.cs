...
namespace school_web.AppCode
{
    public class Examination_P3
    {
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            foreach (DataRow drs in dts.Rows)
            {
                ...
                else
                {
                    ...
                    {
                        total_number = total_number + My.toDouble(dt.Rows[i]["Marks"].ToString());
                        full_marks = full_marks + My.toDouble(dt.Rows[i]["Full_marks"].ToString());
                    }

                    final_marks_temp = total_number / My.toDouble(full_marks) * My.toDouble(assesment_weightage);
                    final_marks = final_marks + final_marks_temp;
                }
            }
            ...
            string final_markss = get_round_off_value(With_Decimal, Without_Decimal, Round_up, Round_down, Half_Round_Up, Half_Round_Down, With_Decimal_Per, Without_Decimal_Per, Round_up_Per, Round_down_Per, Half_Round_Up_Per, Half_Round_Down_Per, Round_Percentage_Checked, Maximum_numbe_decimal, final_marks);
            final_marks = My.toDouble(final_markss);


            save_obtain_mark(session_id, class_id, admission_no, term_id, subject_id, branch_id, final_marks, "100");
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
        ...
        private static void save_obtain_mark(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, double final_marks, string full_mark)
        {
            if (IsUserExist("select Id from Exam_tem_subject_marks where Session_id='" + session_id + "' and Branch_id='" + branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "'"))
            {
                SqlCommand cmd;
                string query = "INSERT INTO Exam_tem_subject_marks (Session_id,Class_id,Branch_id,Admission_no,Term_id,Subject_id,Obtain_mark,Full_mark) values (@Session_id,@Class_id,@Branch_id,@Admission_no,@Term_id,@Subject_id,@Obtain_mark,@Full_mark)";
                cmd = new SqlCommand(query);
                ...
                cmd.Parameters.AddWithValue("@Obtain_mark", final_marks);
                cmd.Parameters.AddWithValue("@Full_mark", full_mark);
            ...
            {
                SqlCommand cmd;
                string query = "Update Exam_tem_subject_marks set Obtain_mark=@Obtain_mark,Full_mark=@Full_mark where Session_id='" + session_id + "' and Branch_id='" + branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "'";
                cmd = new SqlCommand(query);
                cmd.Parameters.AddWithValue("@Obtain_mark", final_marks);
                cmd.Parameters.AddWithValue("@Full_mark", full_mark);
                if (My.InsertUpdateData(cmd))
        ...
        private static string get_unit_weightage(string session_id, string branch_id, string class_id, string admission_no, string term_id, string subject_id, string assessment_id)
        {
            string weightage = "0";
            string query = "select Maximum_Marks from Exam_Assessment_Subject_Mapping_Details where Session_Id=" + session_id + " and Branch_Id='" + branch_id + "' and Exam_Term_Id=" + term_id + " and Istatus='1' and Class_id=" + class_id + " and Subject_id=" + subject_id + " and Assessment_Id=" + assessment_id + "";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count > 0)
            {
                weightage = dt.Rows[0]["Maximum_Marks"].ToString();
            }
            return weightage;
        }
        ...
        {
            string table_name = "Exam_tem_subject_marks";
            string returN = "0/0/0";
            string query = "select sum(isnull(convert(float, Obtain_mark),0)) as total_obt_mark,sum(isnull(convert(float, Full_mark),0)) as total_full_mark from " + table_name + " where  Session_id=" + session_id + " and Branch_id='" + branch_id + "' and Admission_no='" + admission_no + "' and Term_id=" + term_id + " and Class_id=" + class_id + "";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;

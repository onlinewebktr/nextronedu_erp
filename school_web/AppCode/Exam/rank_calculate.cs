...
{
    public class rank_calculate
    {
        My mycode = new My();
        #region ASSESMENT WISE
        //=================================
        internal static string get_marks(string Session_id, string Class_id, string Admission_no, string Term_id, string Assessment_id, string Subject_id, string Branch_id, string Subject_type, string firm_id)
        {
            ...
            string round_off_value = "0";
            if (firm_id == "BD-001")
            {
                round_off_value = get_round_off_value(With_Decimal, Without_Decimal, Round_up, Round_down, Half_Round_Up, Half_Round_Down, With_Decimal_Per, Without_Decimal_Per, Round_up_Per, Round_down_Per, Half_Round_Up_Per, Half_Round_Down_Per, Round_Percentage_Checked, Maximum_numbe_decimal, final_marks);
            }
            else
            {
                round_off_value = final_marks.ToString();// get_round_off_value(With_Decimal, Without_Decimal, Round_up, Round_down, Half_Round_Up, Half_Round_Down, With_Decimal_Per, Without_Decimal_Per, Round_up_Per, Round_down_Per, Half_Round_Up_Per, Half_Round_Down_Per, Round_Percentage_Checked, Maximum_numbe_decimal, final_marks);
            }
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    if (is_grace == "1")
                    {
                        return final_marks.ToString() + "/" + grade + "/" + obt_and_full_mark + "/" + exam_setting + "/" + attandance + "/" + Grade_head_text + "/" + remarkss + "/" + exam_settings + "/" + full_number;
                    }
                    else
                    {
                        return final_marks.ToString() + "/" + grade + "/" + obt_and_full_mark + "/" + exam_setting + "/" + attandance + "/" + Grade_head_text + "/" + remarkss + "/" + exam_settings + "/" + full_number;
        ...
        #region  OVERALL SUBJECTS 
        internal static string get_subject_total_full_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            double total_number = 0; double full_number = 0;
            #region OVERALL CALCULATION 
            string query = "select eta.Marks,eta.Full_marks from Exam_total_no_for_rank_temp eta join Exam_Term_Details etd on eta.Session_id=etd.Session_id and eta.Branch_id=etd.Branch_id and eta.Class_id=etd.Class_id and eta.Term_id=etd.Exam_Term_Id join Exam_Assessment_Details ad on eta.Session_id=ad.Session_id and eta.Branch_id=ad.Branch_id and eta.Class_id=ad.Class_id and eta.Term_id=ad.Exam_Term_Id and eta.Assessment_id=ad.Assessment_Id  where  eta.Session_Id='" + session_id + "' and eta.Branch_Id='" + branch_id + "' and eta.Term_id=" + term_id + " and eta.Class_id=" + class_id + "  and eta.Subject_id=" + subject_id + " and eta.Admission_no='" + admission_no + "' ";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                total_number = 0;
            ...
            if (Type == "Scholastic")
            {
                ReturNs = final_marks + "~" + full_number;
            }
            return ReturNs;
        }
        #endregion

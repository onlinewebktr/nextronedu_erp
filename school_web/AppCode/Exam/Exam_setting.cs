...
namespace school_web.AppCode.Exam
{
    public class Exam_setting
    {
        ...
        internal static string check_dues_for_admit_card(string session_id, string class_id, string admission_no, string term_id, string Exam_id)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                string month_position = My.get_single_column_data("select top 1 Position as Column_Name from Month_Index where Month='" + dt.Rows[0]["Month_name"].ToString() + "'");
                string dues_amt = Exam_setting.get_dues_amt(session_id, class_id, admission_no, dt.Rows[0]["Paid_month_id"].ToString(), dt.Rows[0]["Month_name"].ToString(), "0", month_position);

...
namespace school_web.AppCode.Exam
{
    public class exam_IIII
    {
        ...
        private static string get_obt_and_full_marks(string session_id, string branch_id, string admission_no, string term_id, string class_id)
        {
            ...
            string query = "select isnull(sum(convert(float, Obt_mark)),0) as total_obt_mark,isnull(sum(convert(float, Full_mark)),0) as total_full_mark  from (select top 5 * from (select Subject_id,isnull(sum(convert(float, Marks)),0) as Obt_mark,isnull(sum(convert(float, Full_marks)),0) as Full_mark from " + table_name + " where Session_Id=" + session_id + " and Branch_Id='" + branch_id + "' and Admission_no='" + admission_no + "' and Term_id=" + term_id + " GROUP by Subject_id) t order by Obt_mark desc) tt";

...
namespace school_web.AppCode.Exam
{
    public class dps_termI_IX_X
    {
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            double final_marks = Math.Round(total_number);
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
        ...
        #region ASSESMENT WISE
        //=================================
        internal static string get_marks_of_group_subject(string Session_id, string Class_id, string Admission_no, string Term_id, string Assessment_id, string Subject_id, string Branch_id, string Subject_type, string group_id)
        {
            Dictionary<string, object> dc1 = get_marks_calculation(Session_id, Branch_id, Class_id, Term_id, Assessment_id, Subject_id);
            string Grade_System_Id = (String)dc1["Grade_System_Id"];
            //string Maximum_Marks = (String)dc1["Maximum_Marks"];
            ...
            if (Calculation_Type == "Sum")
            {
                ...
                {
                    ///=========================IS  NOT ADVANCE ////===---===--==--==--==--==--==--
                    string query = "select em.Marks,essl.Maximum_Marks from dbo.[Exam_marks] em join Exam_Subject_Sub_Level essl on em.Session_id=essl.Session_id and em.Branch_id=essl.Branch_id and em.Class_id=essl.Class_id and em.Term=essl.Exam_Term_Id and em.Assessment=essl.Assessment_Id and em.Subject=essl.Subject_id  and  em.Subject_activity=essl.Subject_Sub_Level_Id  join Subject_Master sm on em.Subject=sm.Subject_id  where  em.Session_Id='" + Session_id + "' and em.Branch_Id='" + Branch_id + "' and em.Term=" + Term_id + " and em.Assessment=" + Assessment_id + " and em.Class_id=" + Class_id + " and sm.Subject_group='" + group_id + "' and em.Admission_no='" + Admission_no + "'";
                    SqlCommand cmd = new SqlCommand(query);
                    DataTable dt = FillDatastatic(query);
                    if (dt.Rows.Count == 0)
                    {

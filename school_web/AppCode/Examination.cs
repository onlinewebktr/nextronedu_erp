...
namespace school_web.AppCode
{
    ...
    public class Examination
    {
        ...
        {
            string returN = "hidden";
            string query = "select *,(select top 1 Signature from user_details where user_id=Ptm_class_teacher_mapping.UserID) as Signature from Ptm_class_teacher_mapping where Session_Id='" + Session_id + "' and CategoryID='" + Class_id + "' and Section='" + Section + "'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;
        ...
        {
            string returN = "hidden";
            string query = "select Signature from user_details where User_Type='Principal' and Branch_id='" + Branch_id + "' and status='Active'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;
        ...
        {
            string querymain = "Select  studentname as Student_Name,admissionserialnumber as Admission_No,rollnumber as Roll_No  ";
            string query = " Select   su.subject2,su.Subject_id,su.course_id ,su.subject2 as Subject_Short_Name   from   Subject_Master su join Exam_Assessment_Subject_Mapping_Details smc on smc.Class_id=su.course_id and smc.Subject_id=su.Subject_id and smc.Branch_Id=su.Branch_id where  smc.Exam_Term_Id=" + termid + " and  smc.Session_Id=" + sessionid + " and  smc.Branch_Id='" + branchid + "' and smc.Class_id=" + Class_Id + " and smc.Assessment_Id=" + assesment_id + " ORDER BY smc.Sequence_No";
            SqlCommand cmd = new SqlCommand(query);
            DataTable dt = mycode.GetData(cmd);
            if (dt.Rows.Count == 0)
            {
        ...
        {
            string querymain = "Select  studentname as Student_Name,admissionserialnumber as Admission_No,rollnumber as Roll_No  ";
            string query = " Select   DISTINCT su.Subject_position as Sequence_No,su.subject2,su.Subject_id,su.course_id ,su.subject2 as Subject_Short_Name from Subject_Master su join Exam_Assessment_Subject_Mapping_Details smc on smc.Class_id=su.course_id and smc.Subject_id=su.Subject_id and smc.Branch_Id=su.Branch_id where  smc.Exam_Term_Id=" + termid + " and  smc.Session_Id=" + sessionid + " and  smc.Branch_Id='" + branchid + "' and smc.Class_id=" + Class_Id + " ORDER BY su.Subject_position asc";
            SqlCommand cmd = new SqlCommand(query);
            DataTable dt = mycode.GetData(cmd);
            if (dt.Rows.Count == 0)
            {

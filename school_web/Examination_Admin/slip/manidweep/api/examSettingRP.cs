...
{
    public class examSettingRP
    {
        internal static string get_exam_settings(string session_id, string branch_id, string class_id)
        {
            string returN = "hidden月hidden月hidden月hidden月hidden月160px月hidden月hidden月hidden月show月hidden月hidden月hidden月hidden月1250px月sameLine月2px月2px月2px月2px月2px月2px月2px月hidden月hidden";
            ...
            else
            {
                ...
                returN = special_note + "月" + Qr_code + "月" + instruction2 + "月" + Rank + "月" + Graph + "月" + GraphHeight + "月" + percent_remark + "月" + sign_top + "月" + sign_bottom + "月" + is_max_mark_show + "月" + is_estd_show + "月" + is_contact_no_show + "月" + is_email_show + "月" + is_class_text_show + "月" + height_dv + "月" + is_class_new_line + "月" + co_sch_area_margn + "月" + overall_area_margn + "月" + percent_remark_area_margn + "月" + graph_area_margn + "月" + ins1_area_margn + "月" + ins2_area_margn + "月" + toppers_area_margn + "月" + is_watermark_Show + "月" + is_std_img_hide + "月" + is_std_section_hide + "月" + aff_text + "月" + father_name1 + "月" + father_name2 + "月" + is_subj_code_hide + "月" + is_text_center + "月" + Is_website_show + "月" + WhatHeadShow + "月" + Is_aff_no_hide + "月" + Is_overall_grade_hide + "月" + Is_grace + "月" + Is_aff_year + "月" + Is_co_scholastic_hide;
                return returN;
            }
        }
        ...
        internal static string check_dues(string session_id, string class_id, string admission_no, string term_id)
        {
            string dues = "NO";
            DataTable dt = My.dataTable("select * from Validation_for_admit_report_card where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Type='ReportCard' and Status='1'");
            if (dt.Rows.Count > 0)
            {
                DataTable dtFee = My.dataTable("select * from Typewise_fee_collection where session='" + dt.Rows[0]["Session_name"].ToString() + "' and admission_no='" + admission_no + "' and (parameter='MonthlyFee' or parameter='HostelMonthlyFee') and month='" + dt.Rows[0]["Month_name"].ToString() + "'");
                if (dtFee.Rows.Count > 0)
                {
                    DataTable dtFeeFChk = My.dataTable("select * from Typewise_fee_collection where session='" + dt.Rows[0]["Session_name"].ToString() + "' and admission_no='" + admission_no + "' and (parameter='MonthlyFee' or parameter='HostelMonthlyFee') and month='" + dt.Rows[0]["Month_name"].ToString() + "' and status='Dues'");
                    if (dtFeeFChk.Rows.Count > 0)
                    {
                        dues = "YES";
                    }
                }
        ...
        internal static string check_dues_for_admit_card(string session_id, string class_id, string admission_no, string term_id, string Exam_id)
        {
            string dues = "NO";
            DataTable dt = My.dataTable("select * from Validation_for_admit_report_card where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Exam_id='" + Exam_id + "' and Type='AdmitCard' and Status='1'");
            if (dt.Rows.Count > 0)
            {
                DataTable dtFee = My.dataTable("select * from Typewise_fee_collection where session='" + dt.Rows[0]["Session_name"].ToString() + "' and admission_no='" + admission_no + "' and (parameter='MonthlyFee' or parameter='HostelMonthlyFee') and month='" + dt.Rows[0]["Month_name"].ToString() + "'");
                if (dtFee.Rows.Count > 0)
        ...
        internal static string get_grade_of_a_subject_with_bg(string session_id, string branch_id, string Grade_System_Id, double final_marks, double full_number, string class_id)
        {
            string is_calculate_100 = "1";
            try
            {
        ...
        internal static string get_dues_message_for_admit_card(string session_id, string class_id, string term_id, string Exam_id)
        {
            string dues = "NO";
            DataTable dt = My.dataTable("select * from Validation_for_admit_report_card where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Exam_id='" + Exam_id + "' and Type='AdmitCard' and Status='1'");
            if (dt.Rows.Count > 0)
            {
                dues = dt.Rows[0]["message"].ToString();
            }
            return dues;
        }
        internal static string get_dues_amt(string session_id, string class_id, string admission_no, string dues_month_id, string dues_month_name, string dues_cal_type, string month_position)
        {
            string dues_amt = "0";
            string qry = "";
            if (dues_cal_type == "1")
            {
                qry = "select isnull(sum(convert(float, Dues_amt)),0) as Dues_amt from Student_wise_dues_amount where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Admission_no='" + admission_no + "' and Month_position<=" + month_position + " and parameter in('MonthlyFee','HostelMonthlyFee')";
            }
            else if (dues_cal_type == "3")
            {
                qry = "select isnull(sum(convert(float, Dues_amt)),0) as Dues_amt from Student_wise_dues_amount where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Admission_no='" + admission_no + "' and Month_position<=" + month_position + " and parameter in('MonthlyFee','HostelMonthlyFee','HostelAnnualFee','AnnualFee')";
            }
            else if (dues_cal_type == "4")
            {
                qry = "select isnull(sum(convert(float, Dues_amt)),0) as Dues_amt from Student_wise_dues_amount where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Admission_no='" + admission_no + "' and Month_position<=" + month_position + " and parameter in('MonthlyFee','HostelMonthlyFee','HostelAdmissionFee','AdmissionFee')";
            }
            else
            {
                qry = "select isnull(sum(convert(float, Dues_amt)),0) as Dues_amt from Student_wise_dues_amount where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Admission_no='" + admission_no + "' and Month_position<=" + month_position + "";
            }
            DataTable dt = My.dataTable(qry);
            dues_amt = dt.Rows[0]["Dues_amt"].ToString();
            return dues_amt;
        }
    }
}
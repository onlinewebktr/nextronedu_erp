...
namespace school_web.Examination_Admin.slip.greenv.api
{
    public class greenVp2
    {
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            double total_number = 0; double full_no = 0;
            #region OVERALL CALCULATION
            ...
            if (Calculation_Type == "Sum")
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            total_number = total_number + My.toDouble(dt.Rows[i]["Marks"].ToString());
                            full_no = full_no + My.toDouble(dt.Rows[i]["Full_marks"].ToString());
                        }
                    }
                }
                #endregion
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = get_grade_2(session_id, branch_id, Grade_System_Id, final_marks, full_no);   // Grade of a subject
        ...
        private static string get_grade_2(string session_id, string branch_id, string Grade_System_Id, double final_marks, double full_marks)
        {
            double final_perc = (final_marks / full_marks) * 100;
            final_marks = Math.Round(final_perc);
            string returN = "FAIL";
        ...
        private static string get_obt_and_full_marks(string session_id, string branch_id, string admission_no, string term_id, string class_id)
        {
            ...
            string returN = "0/0/0";
            string query = "select sum(isnull(convert(float, Marks),0)) as total_obt_mark,sum(isnull(convert(float, Full_marks),0)) as total_full_mark from " + table_name + " where  Session_Id=" + session_id + " and Branch_Id='" + branch_id + "' and Admission_no='" + admission_no + "' and Term_id=" + term_id + " and Is_mark_not_sum!=1";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;

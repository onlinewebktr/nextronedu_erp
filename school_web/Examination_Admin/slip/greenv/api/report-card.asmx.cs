...
namespace school_web.Examination_Admin.slip.greenv.api
{
    ...
    public class report_card : System.Web.Services.WebService
    {
        ...
        //=====================================================
        public class Fetch_Details_of_report_head_M_G
        {
            public string TextShow { get; set; }
        }

        List<Fetch_Details_of_report_head_M_G> Show_of_report_head_M_G = new List<Fetch_Details_of_report_head_M_G>();
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void fetch_report_card_M_G(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            string query = "select Assessment_Name,Short_Name,Assessment_Id,Maximum_Marks,(select top 1 Maximum_Marks from Exam_Term_Details where Session_Id=Exam_Assessment_Details.Session_Id and Branch_Id=Exam_Assessment_Details.Branch_Id and Exam_Term_Id=Exam_Assessment_Details.Exam_Term_Id) as Term_Maximum_Marks from Exam_Assessment_Details where Session_Id='" + Session_id + "' and  Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Exam_Term_Id=" + Term_id + "  and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    Show_of_report_head_M_G.Add(new Fetch_Details_of_report_head_M_G
                    {
                        TextShow = "M",
                    });
                    Show_of_report_head_M_G.Add(new Fetch_Details_of_report_head_M_G
                ...
                JavaScriptSerializer js = new JavaScriptSerializer();
                Context.Response.Write(js.Serialize(Show_of_report_head_M_G));
            }
        }
        ...
        public class MySubjectMarksShow
        {
            ...
            public string Full_no_of_a_subj { get; set; }
        ...
        {
            My.exeSql("delete from Exam_temp_assesment_total_no where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Term_id='" + Term_id + "' and Admission_no='" + Admission_no + "'; delete from Exam_temp_assesment_total_no_term_II where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Term_id='" + Term_id + "' and Admission_no='" + Admission_no + "'");
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = Examination.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string full_no_of_a_subj = arrs[48];   // WhatHeadShow
                    ...
                    EMySubMark.Add(new MySubjectMarksShow
                    {
                        ...
                        Full_no_of_a_subj = full_no_of_a_subj,
        ...
        private List<MySubjectMarkDetails> findmyBookingProduct(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    double markPercntage = (My.toDouble(obt_marks) / My.toDouble(full_marks)) * 100;
                    string gradeS = Examination.get_grade_final(dr["Session_id"].ToString(), dr["Branch_id"].ToString(), dr["Grade_System_Id"].ToString(), markPercntage);
        ...
        {
            string query = "select * from Exam_grade_explain";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Exam_grade_explain");
            DataTable dt = ds.Tables[0];
            int rowcount1 = dt.Rows.Count;
            if (rowcount1 == 0)
            {
        ...
        {
            public string Subject_Name { get; set; }
            public List<MySubjectMarkDetailsCoScholestic> MySubjectMarkItemCoScholestic { get; set; }
        }
        public class MySubjectMarkDetailsCoScholestic
        {
            public string Marks { get; set; }
        ...
        {
            string query = "select DISTINCT t1.Session_id,t1.Class_id,t1.Term,t1.Subject,t1.Admission_no,t1.Branch_id,t2.Subject_name,t2.Subject_position from Exam_marks t1 join Subject_Master t2 on t1.Subject=t2.Subject_id where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Admission_no='" + Admission_no + "' and t1.Term=" + Term_id + " and t2.Subject_Type_Scholastic_Co_Scholastic='Co-Scholastic' order by t2.Subject_position asc";
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    List<MySubjectMarkDetailsCoScholestic> MBdetails = findCoScolestic(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id);
                    Show_of_report_CoScholastic.Add(new Fetch_Details_of_report_CoScholastic
                    {
                        Subject_Name = dr["Subject_name"].ToString(),
                        MySubjectMarkItemCoScholestic = MBdetails
                    });
                }
                JavaScriptSerializer js = new JavaScriptSerializer();
        ...
        private List<MySubjectMarkDetailsCoScholestic> findCoScolestic(string subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            List<MySubjectMarkDetailsCoScholestic> MySubjectMarkItemCoScholestic = new List<MySubjectMarkDetailsCoScholestic>();
            // string query = "select DISTINCT t1.Session_id,t1.Class_id,t1.Term,t1.Subject,t1.Admission_no,t1.Branch_id,t2.Subject_name,t2.Subject_position,t1.Assessment from Exam_marks t1 join Subject_Master t2 on t1.Subject=t2.Subject_id where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Admission_no='" + Admission_no + "' and t1.Term=" + Term_id + " and t2.Subject_Type_Scholastic_Co_Scholastic='Co-Scholastic'  and t1.Subject=" + subject_id + "  and t1.Assessment in (select Assessment_Id from  Exam_Assessment_Details where Session_Id=" + Session_id + " and Class_id=" + Class_id + " and  Exam_Term_Id=" + Term_id + " and  Scholastic_Co_scholastic='Co-Scholastic') order by t2.Subject_position asc";

            string query = "select DISTINCT t1.Session_id,t1.Class_id,t1.Exam_Term_Id,t1.Branch_id,t1.Assessment_Id,t1.Sequence_No from Exam_Assessment_Details t1 where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Exam_Term_Id=" + Term_id + " and Scholastic_Co_scholastic='Co-Scholastic' order by t1.Sequence_No asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Exam_marks");
            DataTable dt = ds.Tables[0];
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = Examination.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Exam_Term_Id"].ToString(), dr["Assessment_Id"].ToString(), subject_id, dr["Branch_id"].ToString(), "Co-Scholastic");
                    ...
                    MySubjectMarkItemCoScholestic.Add(new MySubjectMarkDetailsCoScholestic
                    {
                        Marks = obt_marks,
                        ColspaN = "2",
                    });
            ...
            return MySubjectMarkItemCoScholestic;
        }

...
namespace school_web.Examination_Admin.slip.nni.api
{
    ...
    public class report_card : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void fetch_rp_card_attendances(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            string termids = "2";
            string first_term = Exam_setting.get_first_term(Session_id, Branch_id, Class_id);
            if (first_term == Term_id)

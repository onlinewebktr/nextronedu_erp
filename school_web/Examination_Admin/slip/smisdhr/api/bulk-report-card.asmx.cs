...
namespace school_web.Examination_Admin.slip.smisdhr.api
{
    /// <summary>
    ...
    public class bulk_report_card : System.Web.Services.WebService
    {
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Admission_no)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string signatures = smi_examination.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = smi_examination.get_term_name(Branch_id, Term_id);
                    string rank = "0";
                    ...
                    try
                    {
                        age = get_agess(Convert.ToDateTime(dt.Rows[0]["dob"].ToString()));
                    }
                    catch (Exception ex)
                    {
                    }
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = smi_examination.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = smi_examination.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = smi_examination.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_Signature> findSigDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        stgnature = smi_examination.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = smi_examination.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = smi_examination.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        private List<Fetch_Details_of_report_head> findSubjectHeadings(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (i == 1)
                    {
                        extra_marks = smi_examination.get_extra_marks(Class_id, Branch_id);
                        if (extra_marks != "0")
                        {
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = smi_examination.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_report_descpline> findDescplineData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = smi_examination.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
                    ...
                    MyDescplineData.Add(new Fetch_Details_of_report_descpline
                    {
                        ...
                        Activity_details = dr["Activity_details"].ToString(),
                        Term_grade = grade,
                    });
                }
            }
        ...
        private List<Fetch_Details_of_report_grade_remark> findGradeRemarkData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            string sys_grade_id = smi_examination.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = smi_examination.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string subjnameS = dr["Subject_name"].ToString();
                    if (subjnameS.ToUpper() == "ENGLISH")
                    {
                        subjnameS = "Language-I";
                    }
                    if (subjnameS.ToUpper() == "HINDI" || subjnameS.ToUpper() == "BENGALI")
                    {
                        subjnameS = "Language-II";
                    }
                    if (subjnameS.ToUpper() == "S.SC")
                    {
                        subjnameS = "Social Science";
                    }
                    ...
                    MySubjectMarkShowItemII.Add(new MySubjectMarkShowII
                    {
                        ...
                        Subject_name = subjnameS, //dr["Subject_name"].ToString(),
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
                string subject_name = smi_examination.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = smi_examination.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
        ...
        private void save_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string Branch_id, string marks, string full_marks, string marks_type, string total_no)
        {
            ...
            string first_term = smi_exam_setting.get_first_term(session_id, Branch_id, class_id);

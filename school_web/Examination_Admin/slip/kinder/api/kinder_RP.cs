...
namespace school_web.Examination_Admin.slip.kinder.api
{
    public class kinder_RP
    {
        ...
        internal static string get_marks(string Session_id, string Class_id, string Admission_no, string Term_id, string Assessment_id, string Subject_id, string Branch_id, string Subject_type)
        {
            ...
            if (Subject_type == "Scholastic")
            {
                ...
                else   // GRADE
                {
                    ...
                    {
                        string marks_type = "Grade";
                        string grade = get_grade_bg_color(Session_id, Branch_id, grade_system_id, IsChar);
                        return IsChar + "/" + assesment_weightage + "/" + marks_type + "/" + round_off_value + "/" + grade;
                    }
                }
            }
            else
            {
                ...
                {
                    string marks_type = "Grade";
                    string grade = get_grade_bg_color(Session_id, Branch_id, grade_system_id, IsChar);   // Grade of a Assessment
                    return IsChar + "/" + assesment_weightage + "/" + marks_type + "/" + round_off_value + "/" + grade;
                }
            }
        }

        private static string get_grade_bg_color(string session_id, string branch_id, string grade_system_id, string isChar)
        {
            string returN = "0";
            DataTable dt = My.dataTable("select Background_color from Exam_Grade_System_Range_Grade where Session_Id='" + session_id + "' and Branch_Id='" + branch_id + "' and Grade='" + isChar + "' and Grade_System_Id='" + grade_system_id + "'");
            if (dt.Rows.Count > 0)
            {
                returN = dt.Rows[0]["Background_color"].ToString();
            }
            return returN;
        }
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = kinder_exam_setting.get_grade_of_a_subject_with_bg(session_id, branch_id, Grade_System_Id, final_marks, full_number, class_id);   // Grade of a subject
                    ...
                    string exam_settings = kinder_exam_setting.get_exam_settings(session_id, branch_id, class_id);  // Settings
                    ...
                    if (is_grace == "1")
                    {
                        return final_marks.ToString() + "/" + grade + "/" + obt_and_full_mark + "/" + exam_setting + "/" + attandance + "/" + Grade_head_text + "/" + remarkss + "/" + exam_settings + "/" + full_number + "/" + gradeBG;
                    }
                    else
                    {
                        return final_marks.ToString() + "/" + grade + "/" + obt_and_full_mark + "/" + exam_setting + "/" + attandance + "/" + Grade_head_text + "/" + remarkss + "/" + exam_settings + "/" + full_number + "/" + gradeBG;
                ...
                else
                {
                    ...
                    string grade = "0";
                    string gradeBG = "0";
                    string obt_and_full_mark = "0/0/0";//get_obt_and_full_marks(session_id, branch_id, admission_no, term_id, class_id); // Obtain Mark/Full Mark/OverAll Percentage
                    ...
                    string Overall_grade = "0";//get_grade(session_id, branch_id, Grade_System_Id, My.toDouble(overall_persentage), class_id);   // Grade of a subject
                    ...
                    string exam_settings = kinder_exam_setting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show
                    return grade + "/" + hide_last_grade_clumn + "/" + obt_and_full_mark + "/" + exam_setting + "/" + attandance + "/" + Grade_head_text + "/" + remarkss + "/" + exam_settings + "/" + full_number + "/" + gradeBG;
                }
            }
            else
            {
                ...
                string hide_last_grade_clumn = "hidden";
                string exam_settings = kinder_exam_setting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show 
                return grade + "/" + hide_last_grade_clumn + "/" + obt_and_full_mark + "/" + exam_setting + "/" + attandance + "/" + Grade_head_text + "/" + remarkss + "/" + exam_settings + "/" + gradeBG;
            }
        }
        ...
        internal static string get_overall_total_grade_to_grade(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            string result = "-/-";
            DataTable dt = My.dataTable("select Marks from Exam_overall_grade_termwise where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Term='" + Term_id + "' and Admission_no='" + Admission_no + "' and Branch_id='" + Branch_id + "'");
            if (dt.Rows.Count > 0)
            {
                string grade = dt.Rows[0]["Marks"].ToString();
                string grade_color = My.get_single_column_data("select Background_color as Column_Name from Exam_Grade_System_Range_Grade where Session_Id='" + Session_id + "' and Branch_Id='" + Branch_id + "' and Grade='" + grade + "'");
                result = grade + "/" + grade_color;
            }
            return result;
        }

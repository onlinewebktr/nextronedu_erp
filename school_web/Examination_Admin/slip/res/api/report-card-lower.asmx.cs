...
namespace school_web.Examination_Admin.slip.res.api
{
    /// <summary>
    /// Summary description for report_card_lower
    /// </summary>
    ...
    public class report_card_lower : System.Web.Services.WebService
    {
        My mycode = new My();
        public class MyStudentsDetails
        {
            ...
            public string Promoted_to_class { get; set; }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Adm_no)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string signatures = generalFM.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = generalFM.get_term_name(Branch_id, Term_id);
                    ...
                    string promote_class = get_promot_to_class(Session_id, dr["Admission_no"].ToString(), Class_id, dr["Section"].ToString());
                    string[] stringSeparatorssz = new string[] { "月" };
                    string[] arrsz = promote_class.Split(stringSeparatorssz, StringSplitOptions.None);
                    string promoted_to_class = arrsz[0];
                    string result_date = arrsz[1];
        ...
        private string get_promot_to_class(string session_id, string admission_no, string class_id, string section)
        {
            string rtrN = "hidden月hidden";
            DataTable dt = My.dataTable("select Promoted_to_class,School_reopen_on from Exam_promoted_to_class where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Admission_no='" + admission_no + "' and Section='" + section + "'");
            if (dt.Rows.Count > 0)
            {
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = rpcard_for_term_general.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = generalFM.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = generalFM.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_Signature> findSigDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        stgnature = generalFM.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = generalFM.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = generalFM.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = generalFM.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string firm_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = generalFM.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string percent_remark = generalFM.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
               // string subject_name = generalFM.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string passMarks = get_pass_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    string marks = generalFM.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    MySubjectMarkItemIII.Add(new MySubjectMarkDetailsIII   ///PASS MARKS
                    {
                        Marks = passMarks,
                        Subject_Name = "",
                        Subject = Subject_id,
        ...
        private string get_pass_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string branch_id, string subj_type)
        {
            string rtrN = "-";
            string qry = "select Cut_Off_Percentage from Exam_Subject_Sub_Level where Session_Id='" + session_id + "' and Class_id='" + class_id + "' and Exam_Term_Id='" + term_id + "' and Assessment_Id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Branch_Id='" + branch_id + "' and Istatus=1";
        ...
        private void save_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string Branch_id, string marks, string full_marks, string marks_type, string total_no)
        {
            ...
            string first_term = generalExamSetting.get_first_term(session_id, Branch_id, class_id);
            if (first_term == term_id)
            {

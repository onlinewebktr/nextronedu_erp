    ...
    public class report_card : System.Web.Services.WebService
    {
        ...
        public void fetch_report_card(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                {
                    string extra_marks = exam_ips_I.get_max_marks_in_head(Session_id, Class_id, Branch_id, Term_id, dt.Rows[i]["Assessment_Id"].ToString());
                    ...
                    Show_of_report_head.Add(new Fetch_Details_of_report_head
                    {
                        ...
                        Maximum_Marks = extra_marks, //dt.Rows[i]["Maximum_Marks"].ToString(),
        ...
        {
            string sys_grade_id = exam_ips_I.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        public void fetch_rp_card_signature(string Session_id, string Class_id, string Admission_no, string Branch_id)
        {
            ...
            {
                string Section = exam_ips_I.get_student_section(Session_id, Admission_no, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string stgnature = "";
                    if (dr["Signature_type"].ToString() == "1")
                    {
                        stgnature = exam_ips_I.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = exam_ips_I.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = exam_ips_I.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        public void fetch_report_card_discipline_activity(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = exam_ips_I.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
                    ...
                    save_coscholastic_areas(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Exam_Term_Id"].ToString(), dr["Activity_Id"].ToString(), "0", dr["Branch_id"].ToString(), "DISCIPLINE", dr["Term_grade"].ToString(), "0", "0");
        ...
        {
            string section = exam_ips_I.get_section(Session_id, Class_id, Admission_no, Branch_id);
            ...
            {
                string system_grade_id = exam_ips_I.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = exam_ips_I.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        {
            string Total_marks = exam_ips_I.get_overall_total_marks(Session_id, Class_id, Admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);

...
namespace school_web.Examination_Admin.slip.ips.api
{
    public class exam_ips_ix
    {
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = Exam_setting.get_grade_of_a_subject(session_id, branch_id, Grade_System_Id, final_marks, full_number, class_id);   // Grade of a subject
        ...
        internal static string get_systm_grd_id(string Subject_id, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            try
            {
                string query = "select top 1 Grade_System_Id from dbo.[Exam_Subject_Sub_Level] where Subject_id=" + Subject_id + " and Session_Id=" + Session_id + " and Branch_Id='" + Branch_id + "' and Exam_Term_Id='" + Term_id + "' order by Grade_System_Id asc";
                DataTable dt = FillDatastatic(query);
                if (dt.Rows.Count == 0)
                {
                    gradeid = "0";

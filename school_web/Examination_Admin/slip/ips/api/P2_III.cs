...
namespace school_web.Examination_Admin.slip.ips.api
{
    public class P2_III
    {
        ...
        //=================================
        internal static string get_marks(string Session_id, string Class_id, string Admission_no, string Term_id, string Assessment_id, string Subject_id, string Branch_id, string Subject_type, string types)
        {
            ...
            double total_number = 0; double total_numbers = 0; string IsChar = ""; bool isNum = true; bool isNumTrue = false; double max_number = 0; double max_numbers = 0;
            ...
            else
            {
                ...
                else
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            ///============================
                            total_numbers = total_numbers + My.toDouble(dt.Rows[i]["Marks"].ToString());
                            max_numbers = max_numbers + My.toDouble(dt.Rows[i]["Maximum_Marks"].ToString());
                            isNumTrue = true;
                        }
                    }
                    ...
                    if (types == "1" || types == "2")
                    {
                        if (dt.Rows.Count == 1)
                        {
                            ...
                            if (types == "2")
                            {
                                isNum = valid_amount(dt.Rows[0]["Marks"].ToString());
                                if (isNum == true)
                                {
                                    ///============================
                                    total_number = My.toDouble(dt.Rows[0]["Marks"].ToString());
                                    max_number = My.toDouble(dt.Rows[0]["Maximum_Marks"].ToString());
                                    isNumTrue = true;
                                }
                                else
                                {
                        ...
                        else
                        {
                            ...
                            if (types == "2")
                            {
                                if (dt.Rows.Count == 2)
                                {
                                    isNum = valid_amount(dt.Rows[1]["Marks"].ToString());
                                    if (isNum == true)
                                    {
                                        ///============================
                                        total_number = My.toDouble(dt.Rows[1]["Marks"].ToString());
                                        max_number = My.toDouble(dt.Rows[1]["Maximum_Marks"].ToString());
                                        isNumTrue = true;
                                    }
                                    else
                                    {
            ...
            double final_marks = total_number / My.toDouble(max_number) * My.toDouble(max_number);
            double final_markss = (total_numbers / My.toDouble(max_numbers)) * My.toDouble(max_numbers);
            ...
            string round_off_value_ttl_mark = Math.Round(final_markss).ToString();

            //get_round_off_value(With_Decimal, Without_Decimal, Round_up, Round_down, Half_Round_Up, Half_Round_Down, With_Decimal_Per, Without_Decimal_Per, Round_up_Per, Round_down_Per, Half_Round_Up_Per, Half_Round_Down_Per, Round_Percentage_Checked, Maximum_numbe_decimal, final_markss);
            ...
            if (Subject_type == "Scholastic")
            {
                if (Output == "Marks")  //MARKS
                {
                    if (isNumTrue == true)
                    {
                        ...
                        return round_off_value + "/" + assesment_weightage + "/" + marks_type + "/" + round_off_value + "/" + Maximum_Marks + "/" + round_off_value_ttl_mark;
                    }
                    else
                    {
                        string marks_type = "Marks";
                        return IsChar + "/" + assesment_weightage + "/" + marks_type + "/" + round_off_value + "/" + Maximum_Marks + "/" + round_off_value_ttl_mark;
                    }
                }
                else   // GRADE
                {
        ...
        private static string get_grade_subj(string session_id, string branch_id, string Grade_System_Id, double final_marks, string Maximum_Marks, string assesment_weightage)
        {
            ...
            final_marks = final_marks = Math.Round(final_markss);
            string returN = "FAIL";
            string query = "select * from Exam_Grade_System_Range_Grade where Grade_System_Id=" + Grade_System_Id + " and  Session_Id=" + session_id + " and Branch_Id='" + branch_id + "'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
        ...
        internal static string get_subject_total_marksTII(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            Dictionary<string, object> dc1 = get_term_marks_calculation(session_id, branch_id, class_id, term_id);
            string Grade_System_Id = (String)dc1["Grade_System_Id"];
            string Maximum_Marks = (String)dc1["Maximum_Marks"];
            ...
            double final_marks = My.toDouble(Math.Round(total_number, 0, MidpointRounding.AwayFromZero).ToString());
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    return final_marks.ToString() + "/" + grade + "/" + Grade_head_text + "/" + Grade_System_Id;
                }
                else
                {
                    string Grade_head_text = "OVERALL GRADE";
                    string grade = get_grade(session_id, branch_id, Grade_System_Id, final_marks);   // Grade of a subject 
                    string hide_last_grade_clumn = "hidden";
                    return grade + "/" + hide_last_grade_clumn + "/" + Grade_head_text + "/" + Grade_System_Id;
                }
            }
            else
            {

...
{
    public class bda_examination_P2
    {
        My mycode = new My();
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string exam_settings = bda_exam_setting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show
        ...
        {
            string returN = "0";
            string query = "select Grade_System_Id from Exam_Subject_Sub_Level where Session_Id='" + session_id + "' and Branch_Id='" + branch_id + "' and Exam_Term_Id='" + term_id + "' and Subject_id='" + subject_id + "' and Class_id='" + class_id + "' and Istatus=1";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count > 0)
            {

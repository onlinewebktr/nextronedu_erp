...
{
    /// <summary>
    /// Summary description for term_report_card_Ld_fm
    /// </summary>
    ...
    public class term_report_card_Ld_fm : System.Web.Services.WebService
    {
        My mycode = new My();
            ...
            else
            {
                ...
                if (dtsig.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtsig.Rows)
                    {
                        ...
                        {
                            stgnatureClassTeacher = LdFM.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                            stgnatureClassTeacherName = dr["Name"].ToString();
                        }
                        if (dr["Signature_type"].ToString() == "2") // Principal
                        {
                            stgnaturePrincipal = LdFM.get_principal_signature(Session_id, Branch_id);
                            stgnaturePrincipalName = dr["Name"].ToString();
                        }
                        if (dr["Signature_type"].ToString() == "3") // Examincharge
                        {
                            stgnatureExaminee = LdFM.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string signatures = LdFM.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = LdFM.get_term_name(Branch_id, Term_id);
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = LdFM.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = LdFM.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = LdFM.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string firm_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = LdFM.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string percent_remark = LdFM.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
                string subject_name = LdFM.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = LdFM.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");

...
namespace school_web.Examination_Admin.slip.lddon.api
{
    public class LdFMLower
    {
        ...
        {
            string query = "select Maximum_Marks,Grade_System_Id from Exam_Subject_Sub_Level where Session_Id='" + Session_id + "' and Branch_Id='" + Branch_id + "' and Istatus=1 and Exam_Term_Id=" + Term_id + " and Assessment_Id=" + Assessment_id + " and Class_id=" + Class_id + " and Subject_id='" + Subject_id + "'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return "0/0";
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = lDexamSetting.get_grade_of_a_subject_with_bg(session_id, branch_id, Grade_System_Id, final_marks, full_number, class_id);   // Grade of a subject
                    ...
                    string exam_settings = lDexamSetting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show

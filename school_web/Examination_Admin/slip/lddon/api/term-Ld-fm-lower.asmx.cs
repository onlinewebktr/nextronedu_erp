...
namespace school_web.Examination_Admin.slip.lddon.api
{
    /// <summary>
    /// Summary description for term_Ld_fm_lower
    /// </summary>
    ...
    public class term_Ld_fm_lower : System.Web.Services.WebService
    {
        My mycode = new My();
        public class MyStudentsDetails
        {
            ...
            public string Class_teacher_sigName { get; set; }
            public string Principal_sigName { get; set; }
            public string Examinee_sigName { get; set; }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Adm_no)
        {
            ...
            else
            {
                ...
                string stgnatureClassTeacher = "", stgnatureClassTeacherName = "", stgnaturePrincipal = "", stgnaturePrincipalName = "", stgnatureExaminee = "", stgnatureExamineeName = "";
                ...
                if (dtsig.Rows.Count > 0)
                {
                    ...
                    {
                        if (dr["Signature_type"].ToString() == "1")  //Class Teacher
                        {
                            stgnatureClassTeacher = LdFMLower.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                            stgnatureClassTeacherName = dr["Name"].ToString();
                        }
                        if (dr["Signature_type"].ToString() == "2") // Principal
                        {
                            stgnaturePrincipal = LdFMLower.get_principal_signature(Session_id, Branch_id);
                            stgnaturePrincipalName = dr["Name"].ToString();
                        }
                        if (dr["Signature_type"].ToString() == "3") // Examincharge
                        {
                            stgnatureExaminee = LdFMLower.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                            stgnatureExamineeName = dr["Name"].ToString();
                        }
                    }
                }
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    List<Fetch_Details_of_report_head_assesment> MBdetailsHeadingAssement = findSubjectHeadingsAssesment(Session_id, Class_id, Branch_id, Term_id);
                    ...
                    // List<Fetch_Details_of_report_marks_range_coscholestic> MBdetailsMarkRange_coscholestic = findMarkRangeData_coscholestic(Session_id, Branch_id, Term_id, Class_id);
                    ...
                    //================ OverAllNo
                    List<Fetch_Details_of_overall_no> MBOverALL = findOverallNoDetails(Session_id, dr["Admission_no"].ToString(), Class_id, Branch_id, dr["Section"].ToString(), Term_id);

                    string signatures = LdFMLower.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = LdFMLower.get_term_name(Branch_id, Term_id);
                    ...
                    ShowMyStudents.Add(new MyStudentsDetails
                    {
                        ...
                        Class_teacher_sig = stgnatureClassTeacher,
                        Principal_sig = stgnaturePrincipal,
                        Examinee_sig = stgnatureExaminee,

                        Class_teacher_sigName = stgnatureClassTeacherName,
                        Principal_sigName = stgnaturePrincipalName,
                        Examinee_sigName = stgnatureExamineeName,
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = LdFMLower.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_graph> findGraphDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string first_term = lDexamSetting.get_first_term(Session_id, Branch_id, Class_id);
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = LdFMLower.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = LdFMLower.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = LdFMLower.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
            ...
            if (dtg.Rows.Count > 0)
            {
                ...
                foreach (DataRow drg in dtg.Rows)
                {
                    ...
                    else
                    {
                        ...
                        {
                            subjcount++;
                            List<MySubjectMarkDetailsIII> MBdetailsIII = findmyMarksIII(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id, drg["Group_id"].ToString(), "1");
                            string Total_marks = LdFMLower.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                            ...
                            string percent_remark = LdFMLower.get_percent_remark(overall_percent);
                            //QQQQQRRRR
                            string url = "";
                            string markType = "";
                            ...
                            if (groupcount != subjgroupttlrows)
                            {
                                if (subjcount == My.toint(drg["No_of_subject"].ToString()))
                                {
                                    double persentage = 0; string grades = ""; string gradeBGS = "";
                                    string first_term = lDexamSetting.get_first_term(Session_id, Branch_id, Class_id);
                                    ...
                                    DataTable dtgrd = My.dataTable("select (sum(convert(float, Marks)) / sum(convert(float, Full_marks))*100) as OverallPercentage from "+ table_name + " where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Admission_no='" + Admission_no + "' and Term_id='" + Term_id + "' and Subject_g_id='" + drg["Group_id"].ToString() + "'");
                                    if (dtgrd.Rows.Count > 0)
                                    {
                                        persentage = My.toDouble(dtgrd.Rows[0]["OverallPercentage"].ToString());
                                        grades = get_persentage_grades(persentage, Term_id, Class_id, Session_id);

                                        string[] stringSeparator = new string[] { "~" };
                                        string[] arr = grades.Split(stringSeparator, StringSplitOptions.None);
                                        grades = arr[0];
                                        gradeBGS = arr[1];
                                    }

                                    ///=============================================================
                                    List<MySubjectMarkDetailsIII> MBdetailsIIIG = findmyMarksIII(drg["Group_id"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id, drg["Group_id"].ToString(), "Total");
                                    MySubjectMarkShowItemII.Add(new MySubjectMarkShowII
                                    {
                                        Subject = drg["Group_name"].ToString(),
                                        Subject_name = drg["Group_name"].ToString(),
                                        Total_marks = "",
                                        ...
                                        Is_text_center = "",
                                        Is_website_show = "",
                                        IsfontBold = "fontboldtrue",
                                        MySubjectMarkItemIII = MBdetailsIIIG
                                    });
                                }
                            }
                        }
        ...
        private string get_persentage_grades(double final_marks, string Term_id, string Class_id, string Session_id)
        {
            string gradeid = My.get_single_column_data("select Grade_System_Id as Column_Name from Exam_Term_Details where Session_Id='" + Session_id + "' and Class_id='" + Class_id + "' and Exam_Term_Id='" + Term_id + "'");
            final_marks = Math.Round(final_marks);
            string returN = "FAIL~0";
            string query = "select * from Exam_Grade_System_Range_Grade where Grade_System_Id=" + gradeid + " and  Session_Id=" + Session_id + " and Branch_Id='1'";
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string sub_group_id, string type)
        {
            ...
            if (type == "1")
            {
                ...
                {
                    string subject_name = LdFMLower.get_subject_name(Subject_id, Class_id, Branch_id);
                    foreach (DataRow dr in dt.Rows)
                    {
                        string marks = LdFMLower.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                        ...
                        save_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), obt_marks, full_marks, marks_type, total_no, sub_group_id);
            ...
            {
                string tablName = "Exam_temp_assesment_total_no_term_II";
                string first_term = lDexamSetting.get_first_term(Session_id, Branch_id, Class_id);
                if (first_term == Term_id)
                {
                    tablName = "Exam_temp_assesment_total_no";
                }
                string query = "select sum(convert(float, Full_marks)) as Total_full_mark,sum(convert(float, Marks)) as Total_obt_mark from "+ tablName + " where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Admission_no='" + Admission_no + "' and Term_id='" + Term_id + "' and Subject_g_id='" + sub_group_id + "'";
                SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
                DataSet ds = new DataSet();
                ad.Fill(ds, "Exam_temp_assesment_total_no");
                DataTable dt = ds.Tables[0];
                int rowcount1 = dt.Rows.Count;
                if (rowcount1 == 0)
                {
                ...
                else
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        MySubjectMarkItemIII.Add(new MySubjectMarkDetailsIII
                        {
                            Marks = dr["Total_full_mark"].ToString(),
                            IsfontBold = "fontboldtrue",
                        });

                        MySubjectMarkItemIII.Add(new MySubjectMarkDetailsIII
                        {
                            Marks = dr["Total_obt_mark"].ToString(),
                            IsfontBold = "fontboldtrue",
                        });
                    }
                }
        ...
        private void save_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string Branch_id, string marks, string full_marks, string marks_type, string total_no, string sub_group_id)
        {
            try
            {
            ...
            if (first_term == term_id)
            {
                ...
                {
                    SqlCommand cmd;
                    string query = "INSERT INTO Exam_temp_assesment_total_no (Session_id,Branch_id,Class_id,Term_id,Assessment_id,Subject_id,Admission_no,Marks,Full_marks,Subject_g_id) values (@Session_id,@Branch_id,@Class_id,@Term_id,@Assessment_id,@Subject_id,@Admission_no,@Marks,@Full_marks,@Subject_g_id)";
                    ...
                    cmd.Parameters.AddWithValue("@Subject_g_id", sub_group_id);
            ...
            else
            {
                ...
                {
                    SqlCommand cmd;
                    string query = "INSERT INTO Exam_temp_assesment_total_no_term_II (Session_id,Branch_id,Class_id,Term_id,Assessment_id,Subject_id,Admission_no,Marks,Full_marks,Subject_g_id) values (@Session_id,@Branch_id,@Class_id,@Term_id,@Assessment_id,@Subject_id,@Admission_no,@Marks,@Full_marks,@Subject_g_id)";
                    cmd = new SqlCommand(query);
                    cmd.Parameters.AddWithValue("@Session_id", session_id);
                    cmd.Parameters.AddWithValue("@Branch_id", Branch_id);
                    cmd.Parameters.AddWithValue("@Class_id", class_id);

...
namespace school_web.Examination_Admin.slip.dps.api
{
    /// <summary>
    /// Summary description for termI_IX_X
    /// </summary>
    ...
    public class termI_IX_X : System.Web.Services.WebService
    {
        My mycode = new My();
        ...
        public void fetch_report_card(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (i == 1)
                    {
                        extra_marks = dps_termI_IX_X.get_extra_marks(Class_id, Branch_id);
                        if (extra_marks != "0")
                        {
        ...
        public void fetch_rp_card_subjects(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            {
                int isGroupScience = 0; int isGroupSScience = 0;
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["Subject_group"].ToString() == "")
                    {
                        List<MySubjectMarkDetails> MBdetails = findmyBookingProduct(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id, "0");
                        string Total_marks = dps_termI_IX_X.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                        ...
                        string Is_aff_no_hide = arrs[44];   // AffiliationNo
                        //QQQQQRRRR 
                        string url = "";
                    ...
                    else   // === GROUP SUBJECT
                    {
                        ...
                        else if (dr["Subject_group"].ToString() == "2001")
                        {
                            if (isGroupSScience == 0)
                            {
                                isGroupSScience++;
                                List<MySubjectMarkDetails> MBdetails = findmyBookingProduct(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id, dr["Subject_group"].ToString());
        ...
        private List<MySubjectMarkDetails> findmyBookingProduct(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string group_id)
        {
            ...
            {
                string subject_name = dps_termI_IX_X.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = "";
                    if (group_id == "0")
                    {
                        marks = dps_termI_IX_X.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    }
                    else
                    {
                        marks = dps_termI_IX_X.get_marks_of_group_subject(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic", group_id);
                    }
        ...
        {
            string sys_grade_id = dps_termI_IX_X.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        public void fetch_rp_card_signature(string Session_id, string Class_id, string Admission_no, string Branch_id)
        {
            ...
            {
                string Section = dps_termI_IX_X.get_student_section(Session_id, Admission_no, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string stgnature = "";
                    if (dr["Signature_type"].ToString() == "1")
                    {
                        stgnature = dps_termI_IX_X.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = dps_termI_IX_X.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = dps_termI_IX_X.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        public void fetch_report_card_coscholastic(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = dps_termI_IX_X.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        public void fetch_report_card_discipline_activity(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = dps_termI_IX_X.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void fetch_rp_card_abbreviation(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            string query = "select Short_Name,Condition from Exam_Marks_Entry_Label where Short_Name!='NA' and Branch_Id='" + Branch_id + "'";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Exam_Marks_Entry_Label");
            DataTable dt = ds.Tables[0];
            int rowcount1 = dt.Rows.Count;
            if (rowcount1 == 0)
            {
        ...
        {
            string Total_marks = dps_termI_IX_X.get_overall_total_marks(Session_id, Class_id, Admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);
            ...
            string percent_remark = dps_termI_IX_X.get_percent_remark(overall_percent);

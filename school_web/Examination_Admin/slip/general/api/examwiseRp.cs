...
{
    public class examwiseRp
    {
        public static string auto_serialS(string column, string branchid)
        {
            string result = "";
            try
            {
                SqlConnection conn = new SqlConnection(My.conn);
                SqlDataAdapter ad = new SqlDataAdapter("select * from globle_data  where Branch_Unique_id='" + branchid + "'", conn);
            ...
            catch (Exception e)
            {
                ...
                {
                    My.exeSql("Alter table globle_data add " + column + " varchar(500)");
                    result = auto_serialS(column, branchid);
                }
                else
                {
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type, string Assesment_id)
        {
            ...
            if (Calculation_Type == "Sum")
            {
                ...
                else
                {
                    ///=========================IS  NOT ADVANCE
                    ///
                    string query = "select eta.Marks,eta.Full_marks from " + table_name + " eta join Exam_Term_Details etd on eta.Session_id=etd.Session_id and eta.Branch_id=etd.Branch_id and eta.Class_id=etd.Class_id and eta.Term_id=etd.Exam_Term_Id join Exam_Assessment_Details ad on eta.Session_id=ad.Session_id and eta.Branch_id=ad.Branch_id and eta.Class_id=ad.Class_id and eta.Term_id=ad.Exam_Term_Id and eta.Assessment_id=ad.Assessment_Id  where  eta.Session_Id='" + session_id + "' and eta.Branch_Id='" + branch_id + "' and eta.Term_id=" + term_id + " and eta.Class_id=" + class_id + "  and eta.Subject_id=" + subject_id + " and eta.Admission_no='" + admission_no + "'  and eta.Assessment_id='" + Assesment_id + "' ";
                    DataTable dt = FillDatastatic(query);
                    if (dt.Rows.Count == 0)
                    {
                        total_number = 0;
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = examwiseSetting.get_grade_of_a_subject_with_bg(session_id, branch_id, Grade_System_Id, final_marks, full_number, class_id);   // Grade of a subject
                    ...
                    string attandance = get_attandance_details(session_id, branch_id, class_id, term_id, admission_no, Assesment_id);    //Total Class/Present Class/Present attendance percent
                    string remarkss = get_remarks(session_id, branch_id, class_id, term_id, admission_no, Assesment_id);  // REMARKS 
                    string exam_settings = examwiseSetting.get_exam_settings(session_id, branch_id, class_id);  // Settings
                ...
                else
                {
                    ...
                    string exam_settings = examwiseSetting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show
        ...
        private static string get_attandance_details(string session_id, string branch_id, string class_id, string term_id, string admission_no, string Assesment_id)
        {
            string returN = "0月0月0";
            string querys = "select * from Exam_Wise_Attendance where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Branch_id='" + branch_id + "' and Admission_no='" + admission_no + "' and Exam_Term_Id='" + term_id + "' and Exam_Id='" + Assesment_id + "'";
        ...
        private static string get_remarks(string session_id, string branch_id, string class_id, string term_id, string admission_no, string Assesment_id)
        {
            string returN = "hidden";
            string query = "select * from Examwise_Commentary_Remark where Session_id=" + session_id + " and Branch_id='" + branch_id + "' and Class_id=" + class_id + " and Exam_Term_Id=" + term_id + " and Exam_Id='" + Assesment_id + "' and Admission_no='" + admission_no + "'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;

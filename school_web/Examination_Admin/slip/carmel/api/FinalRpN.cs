    ...
    public class FinalRpN
    {
        ...
        internal static string get_ttl_mark_of_a_subject_final_year(string subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_idI, string Term_idII)
        {
            ...
            else
            {
                ...
                double final_marks_term1 = My.toDouble(dt.Rows[0]["Ttl_mark_for_term1"].ToString()); //My.toDouble(Math.Round(My.toDouble(dt.Rows[0]["Ttl_mark_for_term1"].ToString()), 0, MidpointRounding.AwayFromZero).ToString());
                double final_marks_term2 = My.toDouble(dt.Rows[0]["Ttl_mark_for_termII"].ToString()); //My.toDouble(Math.Round(My.toDouble(dt.Rows[0]["Ttl_mark_for_termII"].ToString()), 0, MidpointRounding.AwayFromZero).ToString());
                return final_marks_term1.ToString() + "/" + final_marks_term2.ToString() + "/" + termI_grade + "/" + termII_grade + "/" + termI_termIi_average_percent + "/" + termI_termII_grade + "/" + TermIattandance + "/" + TermIIattandance + "/" + Overall_mark + "/" + round_off_value_full_persentage + "/" + termI_termIi_average_percent_for_grade;
            }
        }

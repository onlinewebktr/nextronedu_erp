...
namespace school_web.Examination_Admin.slip.epic.api
{
    /// <summary>
    /// Summary description for report_card
    /// </summary>
    ...
    public class report_card : System.Web.Services.WebService
    {
        My mycode = new My();
        public class MyStudentsDetails
        {
            ...
            public List<MySubjectMarkShowIII> MySubjectMarkShowItemIII { get; set; }
            public List<Fetch_Details_of_report_head> MySubjectHeading { get; set; }
            public List<Fetch_Details_of_report_headOP> MySubjectHeadingOP { get; set; }
            public List<Fetch_Details_of_report_headVI> MySubjectHeadingVI { get; set; }
        ...
        public class Fetch_Details_of_report_headOP
        {
            public string Assessment_Name { get; set; }
            public string Short_Name { get; set; }
            public string Maximum_Marks { get; set; }
            public string Term_Maximum_Marks { get; set; }
            public string IshIdeassesment { get; set; }
        }
        public class Fetch_Details_of_report_headVI
        {
            public string Assessment_Name { get; set; }
            public string Short_Name { get; set; }
            public string Maximum_Marks { get; set; }
        ...
        public class MySubjectMarkShowIII
        {
            ...
            public List<MySubjectMarkDetailsIV> MySubjectMarkItemIV { get; set; }
        }
        ...
        public class MySubjectMarkDetailsIV
        {
            public string Marks { get; set; }
            public string Subject_Name { get; set; }
            public string Subject { get; set; }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Adm_no)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    List<MySubjectMarkShowIII> MBdetailsIII = findSubjectMarkShowIII(dr["Admission_no"].ToString(), Session_id, Class_id, Branch_id, Term_id, dtF.Rows[0]["firm_id"].ToString());

                    List<Fetch_Details_of_report_head> MBdetailsHeading = findSubjectHeadings(Session_id, Class_id, Branch_id, Term_id);
                    List<Fetch_Details_of_report_headOP> MBdetailsHeadingOP = findSubjectHeadingsOP(Session_id, Class_id, Branch_id, Term_id);
                    List<Fetch_Details_of_report_headVI> MBdetailsHeadingVI = findSubjectHeadingsVI(Session_id, Class_id, Branch_id, Term_id);
                    ...
                    string signatures = rpcardEpic.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = rpcardEpic.get_term_name(Branch_id, Term_id);
                    string rank = "0";
                    ...
                    ShowMyStudents.Add(new MyStudentsDetails
                    {
                        ...
                        Weignt = dr["Weight"].ToString(),

                        TermShortname = dr["Term_name"].ToString(),
                        Term_name = term_name,
                        ...
                        MySubjectMarkShowItemIII = MBdetailsIII,
                        MySubjectHeading = MBdetailsHeading,
                        MySubjectHeadingOP = MBdetailsHeadingOP,
                        MySubjectHeadingVI = MBdetailsHeadingVI,
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = rpcardEpic.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = rpcardEpic.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = rpcardEpic.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_Signature> findSigDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        stgnature = rpcardEpic.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = rpcardEpic.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = rpcardEpic.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        private List<Fetch_Details_of_report_head> findSubjectHeadings(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (i == 1)
                    {
                        extra_marks = rpcardEpic.get_extra_marks(Class_id, Branch_id);
                        if (extra_marks != "0")
                        {
        ...
        //=================================HEADING
        private List<Fetch_Details_of_report_headOP> findSubjectHeadingsOP(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            List<Fetch_Details_of_report_headOP> MySubjectHeadingOP = new List<Fetch_Details_of_report_headOP>();
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string ishIdeassesment = "showdd";
                    string Short_Name = dt.Rows[i]["Short_Name"].ToString();
                    if (Class_id == "6" || Class_id == "7")
                    {
                        if (i == 4)
                        {
                            string first_term = Exam_setting.get_first_term(Session_id, Branch_id, Class_id);
                    ...
                    MySubjectHeadingOP.Add(new Fetch_Details_of_report_headOP
                    {
                        Assessment_Name = dt.Rows[i]["Assessment_Name"].ToString(),
                        Short_Name = Short_Name,
                        Maximum_Marks = maxmarkS, //dt.Rows[i]["Maximum_Marks"].ToString(),
                        Term_Maximum_Marks = dt.Rows[i]["Term_Maximum_Marks"].ToString(),
                        IshIdeassesment = ishIdeassesment,
                    });
                }
            }
            return MySubjectHeadingOP;
        }
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = rpcardEpic.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        private List<Fetch_Details_of_report_descpline> findDescplineData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = rpcardEpic.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        private List<Fetch_Details_of_report_grade_remark> findGradeRemarkData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            string sys_grade_id = rpcardEpic.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string firm_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = rpcardEpic.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string percent_remark = rpcardEpic.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
        ...
        ///===============================
        ///
        private List<MySubjectMarkShowIII> findSubjectMarkShowIII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string firm_id)
        {
            string slNo = My.get_single_column_data("select count(Subject) as Column_Name from (select DISTINCT t1.Session_id,t1.Class_id,t1.Term,t1.Subject,t1.Admission_no,t1.Branch_id,t2.Subject_name,t2.Subject_Code,t2.Subject_position from Exam_marks t1 join Subject_Master t2 on t1.Subject=t2.Subject_id where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Admission_no='" + Admission_no + "' and t1.Term=" + Term_id + " and t2.Subject_Type_Scholastic_Co_Scholastic='Scholastic' and t2.Is_mandatory=1 and t1.Subject in (select Sub_id from Subject_Mapping_New where Class_id='" + Class_id + "' and Admission_no='" + Admission_no + "' and Session_id='" + Session_id + "')  and t1.Subject not in (select Subject_id from Exam_optional_subject_master where Class_id='" + Class_id + "' and Session_id='" + Session_id + "' and Term_id='" + Term_id + "')) t");
            int slnos = My.toIntS(slNo);

            List<MySubjectMarkShowIII> MySubjectMarkShowItemIII = new List<MySubjectMarkShowIII>();
            string query = "select DISTINCT t1.Session_id,t1.Class_id,t1.Term,t1.Subject,t1.Admission_no,t1.Branch_id,t2.Subject_name,t2.Subject_Code,t2.Subject_position,(select top 1 class from admission_registor where Session_id=t1.Session_id and Class_id=t1.Class_id and Branch_id=t1.Branch_id and admissionserialnumber=t1.Admission_no) as Class_name,(select top 1 studentname from admission_registor where Session_id=t1.Session_id and Class_id=t1.Class_id and Branch_id=t1.Branch_id and admissionserialnumber=t1.Admission_no) as Student_name,(select top 1 rollnumber from admission_registor where Session_id=t1.Session_id and Class_id=t1.Class_id and Branch_id=t1.Branch_id and admissionserialnumber=t1.Admission_no) as Roll_number,(select top 1 Section from admission_registor where Session_id=t1.Session_id and Class_id=t1.Class_id and Branch_id=t1.Branch_id and admissionserialnumber=t1.Admission_no) as Section,isnull((select top 1 Rank from Exam_rank_master where Session_id=t1.Session_id and Class_id=t1.Class_id and Branch_id=t1.Branch_id and Admission_no=t1.Admission_no and Term_id=" + Term_id + "),'0') as Rank from Exam_marks t1 join Subject_Master t2 on t1.Subject=t2.Subject_id where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Admission_no='" + Admission_no + "' and t1.Term=" + Term_id + " and t2.Subject_Type_Scholastic_Co_Scholastic='Scholastic' and t2.Is_mandatory=1 and t1.Subject in (select Sub_id from Subject_Mapping_New where Class_id='" + Class_id + "' and Admission_no='" + Admission_no + "' and Session_id='" + Session_id + "')  and t1.Subject in (select Subject_id from Exam_optional_subject_master where Class_id='" + Class_id + "' and Session_id='" + Session_id + "' and Term_id='" + Term_id + "') order by t2.Subject_position asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Exam_marks");
            DataTable dt = ds.Tables[0];
            ...
            else
            {
                ...
                {
                    slnos++;
                    List<MySubjectMarkDetailsIV> MBdetailsIV = findmyMarksIV(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id);
                    ...
                    MySubjectMarkShowItemIII.Add(new MySubjectMarkShowIII
                    {
                        Slnos = slnos.ToString(),
                        Subject = dr["Subject_Code"].ToString(),
                        Subject_name = dr["Subject_name"].ToString(),
                        Total_marks = obt_subject_mark,
                        Grade = subject_grade,
                        ...
                        Rowcounts = rowcounts.ToString(),
                        Is_overall_grade_show = Is_overall_grade_hide,
                        MySubjectMarkItemIV = MBdetailsIV
                    });
                }
            }
            return MySubjectMarkShowItemIII;
        }
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
                string subject_name = rpcardEpic.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = rpcardEpic.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
        ...
        private List<Fetch_Details_of_report_headVI> findSubjectHeadingsVI(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            List<Fetch_Details_of_report_headVI> MySubjectHeadingVI = new List<Fetch_Details_of_report_headVI>();
            string query = "select Assessment_Name,Short_Name,Assessment_Id,Maximum_Marks,(select top 1 Maximum_Marks from Exam_Term_Details where Session_Id=Exam_Assessment_Details.Session_Id and Branch_Id=Exam_Assessment_Details.Branch_Id and Exam_Term_Id=Exam_Assessment_Details.Exam_Term_Id) as Term_Maximum_Marks from Exam_Assessment_Details where Session_Id='" + Session_id + "' and  Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Exam_Term_Id=" + Term_id + "  and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Exam_marks");
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    MySubjectHeadingVI.Add(new Fetch_Details_of_report_headVI
                    {
                        Short_Name = "F.M.",
                    });
                    MySubjectHeadingVI.Add(new Fetch_Details_of_report_headVI
            ...
            return MySubjectHeadingVI;
        }


        private List<MySubjectMarkDetailsIV> findmyMarksIV(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            List<MySubjectMarkDetailsIV> MySubjectMarkItemIV = new List<MySubjectMarkDetailsIV>();
            ...
            {
                string subject_name = rpCardEpicFM.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = rpCardEpicFM.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };
                    string[] arrs = marks.Split(stringSeparatorss, StringSplitOptions.None);
                    string obt_marks = arrs[0];
                    string full_marks = arrs[1];
                    ...
                    MySubjectMarkItemIV.Add(new MySubjectMarkDetailsIV
                    {
                        Marks = full_marks,
                        //Subject_Name = subject_name,
                        Subject = Subject_id,

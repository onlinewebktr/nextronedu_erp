...
namespace school_web.Examination_Admin.slip.kes.api
{
    public class kesFM
    {
            ...
            if (Subject_type == "Scholastic")
            {
                if (Output == "Marks")  //MARKS
                {
                    if (isNumTrue == true)
                    {
                        ...
                        return round_off_value + "/" + assesment_weightage + "/" + marks_type + "/" + round_off_value + "/" + Maximum_Marks + "/" + PassMarks;
                    }
                    else
                    {
                        string marks_type = "Marks";
                        return IsChar + "/" + assesment_weightage + "/" + marks_type + "/" + round_off_value + "/" + Maximum_Marks + "/" + PassMarks;
                    }
                }
                else   // GRADE
                {
        ...
        {
            string query = "select Maximum_Marks,Grade_System_Id,Cut_Off_Percentage from Exam_Subject_Sub_Level where Session_Id='" + Session_id + "' and Branch_Id='" + Branch_id + "' and Istatus=1 and Exam_Term_Id=" + Term_id + " and Assessment_Id=" + Assessment_id + " and Class_id=" + Class_id + " and Subject_id='" + Subject_id + "'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return "0/0/0";
            ...
            {
                return dt.Rows[0]["Maximum_Marks"].ToString() + "/" + dt.Rows[0]["Grade_System_Id"].ToString() + "/" + dt.Rows[0]["Cut_Off_Percentage"].ToString();
            }
        }
        ...
        {
            Dictionary<string, object> dc = new Dictionary<string, object>();
            string query = " select * from Exam_Subject_Sub_Level where Session_Id='" + Session_id + "' and Branch_Id='" + Branch_id + "' and Istatus=1 and Exam_Term_Id=" + Term_id + " and Assessment_Id=" + Assessment_id + " and Subject_id=" + Subject_id + " and Class_id=" + Class_id + "";
            SqlCommand cmd;
            cmd = new SqlCommand(query);
            DataTable dt = getdata(cmd);
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            Grade_System_Id = get_system_grade_id_by_subj(session_id, branch_id, class_id, term_id, subject_id);
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = kesExamSetting.get_grade_of_a_subject_with_bg(session_id, branch_id, Grade_System_Id, final_marks, full_number, class_id);   // Grade of a subject
                    ...
                    string exam_settings = kesExamSetting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show
                ...
                else
                {
                    ...
                    string exam_settings = kesExamSetting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show
                    return grade + "月" + hide_last_grade_clumn + "月" + obt_and_full_mark + "月" + exam_setting + "月" + attandance + "月" + Grade_head_text + "月" + remarkss + "月" + exam_settings;
                }
            }
            else
            {
        ...
        private static string get_system_grade_id_by_subj(string session_id, string branch_id, string class_id, string term_id, string subject_id)
        {
            string returN = "0";
            string query = "select Grade_System_Id from Exam_Assessment_Subject_Mapping_Details where Session_Id='" + session_id + "' and Branch_Id='" + branch_id + "' and Exam_Term_Id='" + term_id + "' and Subject_id='" + subject_id + "' and Class_id='" + class_id + "' and Istatus=1";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count > 0)
            {
        ...
        private static string get_grade_if_50(string session_id, string branch_id, string Grade_System_Id, double final_marks, double full_number)
        {
        ...
        private static string get_obt_and_full_marks(string session_id, string branch_id, string admission_no, string term_id, string class_id)
        {
            ...
            string returN = "0/0/0";
            string query = "select sum(isnull(convert(float, Marks),0)) as total_obt_mark,sum(isnull(convert(float, Full_marks),0)) as total_full_mark from " + table_name + " where  Session_Id=" + session_id + " and Branch_Id='" + branch_id + "' and Admission_no='" + admission_no + "' and Term_id=" + term_id + " and Subject_id in (select Subject_id from Subject_Master where course_id='"+ class_id + "' and (Is_optional = 0 OR Is_optional IS NULL))";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;
        ...
        internal static string get_overall_total_marks_bg(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            Dictionary<string, object> dc1 = get_term_marks_calculation(Session_id, Branch_id, Class_id, Term_id);
            string Grade_System_Id = (String)dc1["Grade_System_Id"];
            ...
            if (Output == "Marks")
            {
                ...
                string overall_full_marks = arrs[1];
                string overall_percent = arrs[2];
                string grade = get_grade_bg(Session_id, Branch_id, Grade_System_Id, My.toDouble(overall_percent), Class_id);   // Grade of a subject
                string[] stringSeparator = new string[] { "~" };
                string[] arr = grade.Split(stringSeparator, StringSplitOptions.None);
                grade = arr[0];
                string gradeBG = arr[1];
        ...
        private static string get_grade_bg(string session_id, string branch_id, string Grade_System_Id, double final_marks, string Class_id)
        {
        ...
        private static string get_attandance_details(string session_id, string branch_id, string class_id, string term_id, string admission_no)
        {
            ...
            string query = "select Total_no_of_class,No_of_class_Attendance from Exam_Term_Wise_Attendance where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Exam_Term_Id='" + term_id + "' and Admission_no='" + admission_no + "' and Branch_id='" + branch_id + "'";
            DataTable dt = FillDatastatic(query);
            if (dt.Rows.Count == 0)
            {
                return returN;

...
namespace school_web.Examination_Admin.slip.bhc.api
{
    /// <summary>
    /// Summary description for bulk_report_card_P2
    /// </summary>
    ...
    public class bulk_report_card_P2 : System.Web.Services.WebService
    {
        My mycode = new My();
        ...
        public class MySubjectMarkShowII
        {
            ...
            public string Sub_highest_mark { get; set; }
            public string Is_co_scholastic_hide { get; set; }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string signatures = Examination_P2.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = Examination_P2.get_term_name(Branch_id, Term_id);
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = Examination_P2.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = Examination_P2.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    string sub_highest_mark = get_highest_mark(Session_id, Class_id, Branch_id, dr["Subject"].ToString(), Admission_no, dr["Section"].ToString(), Term_id);
                    string[] stringSeparatorss = new string[] { "/" };
                    string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);
                    ...
                    string Is_co_scholastic_hide = arrs[48];   // Is_co_scholastic_hide
                    string percent_remark = Examination_P2.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
                    ...
                    MySubjectMarkShowItemII.Add(new MySubjectMarkShowII
                    {
                        ...
                        Sub_highest_mark = sub_highest_mark,
                        Is_co_scholastic_hide = Is_co_scholastic_hide,
        ...
        private string get_highest_mark(string Session_id, string Class_id, string Branch_id, string Subject_id, string Admission_no, string Section, string Term_id)
        {
            string ReturN = "0";
            DataTable dts = mycode.FillData("select Marks from Exam_highest_mark_of_subject_termwise where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Branch_id='" + Branch_id + "' and Section='" + Section + "' and Subject_id='" + Subject_id + "' and Term_id='" + Term_id + "'");
            if (dts.Rows.Count > 0)
            {
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
                string subject_name = Examination_P2.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = Examination_P2.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");

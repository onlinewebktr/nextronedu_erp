...
namespace school_web.Examination_Admin.slip.naman.api
{
    /// <summary>
    /// Summary description for final_three_term
    /// </summary>
    ...
    public class final_three_term : System.Web.Services.WebService
    {
        My mycode = new My();
        public class MyStudentsDetails
        {
            ...
            public string IsgraphShow { get; set; }
        ...
        public class MySubjectMarkShow
        {
            ...
            public string Fub_full_mark { get; set; }
            ...
            public string RowscountS { get; set; }
            public string totaloverALLobtMarkSubj { get; set; }
        ...
        public class MyRpTotalCalculation
        {
            ...
            public string TermGradeT3 { get; set; }
            public string Session_start_from { get; set; }
            public string Issue_date { get; set; }

            public string RankT1 { get; set; }
            public string RankT2 { get; set; }
            public string RankT3 { get; set; }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_idI, string Term_idII, string Term_idIII, string UserType, string Adm_no)
        {
            string query = "";
            if (Adm_no == "0")
            {
            ...
            else
            {
                ...
                //Signature
                string signName1 = ""; string signName2 = ""; string signName3 = "";
                string sign1 = "hidden"; string sign2 = "hidden"; string sign3 = "hidden";
                string querySig = "select * from Exam_signature_setting where Branch_id='" + Branch_id + "' order by Position asc";
                DataTable dtSig = My.dataTable(querySig);
                if (dtSig.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtSig.Rows)
                    {
                        ...
                        {
                            sign1 = Examination.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                            signName1 = dr["Name"].ToString();
                        }
                        if (dr["Position"].ToString() == "2")
                        {
                            sign2 = Examination.get_principal_signature(Session_id, Branch_id);
                            signName2 = dr["Name"].ToString();
                        }
                        if (dr["Position"].ToString() == "3")
                        {
                            sign3 = Examination.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                            signName3 = dr["Name"].ToString();
                        }
                    }
                }
                ...
                {
                    My.exeSql("delete from Exam_temp_assesment_total_no where Session_id='" + Session_id + "' and Admission_no='" + dr["Admission_no"].ToString() + "'; delete from Exam_temp_assesment_total_no_term_II where Session_id='" + Session_id + "' and Admission_no='" + dr["Admission_no"].ToString() + "'; delete from  Exam_marks  where Session_id='" + Session_id + "' and Admission_no='" + dr["Admission_no"].ToString() + "' and Created_by='AI';INSERT INTO Exam_rp_bulk_print_adm (Session_id,Admission_no) values ('" + Session_id + "','" + dr["Admission_no"].ToString() + "')");
                    //================ GET FIRM DETAILS 
                    ...
                    List<MyRpTotalCalculation> MBTotCaldetails = findTotCalculation(dr["Admission_no"].ToString(), Session_id, Class_id, Branch_id, Term_idI, Term_idII, Term_idIII, dr["Section"].ToString());
                    ...
                    //================ Graph
                    List<Fetch_Details_of_graph> MBGraph = findGraphDetails(Session_id, dr["Admission_no"].ToString(), Class_id, Branch_id, dr["Section"].ToString(), Term_idI, Term_idII, Term_idIII);
                    ...
                    string isgraphShow = "";
                    if (Class_id == "32" || Class_id == "9" || Class_id == "10")
                    {
                        isgraphShow = "hidden";
                    }
        ...
        private List<Fetch_Details_of_graph> findGraphDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_idI, string Term_idII, string Term_idIII)
        {
            List<Fetch_Details_of_graph> MyGraphDetailData = new List<Fetch_Details_of_graph>();
            string query = "select Subject_Short_Name,Subject_position,(sum(convert(float, Obtt_mark))/sum(convert(float, Full_marks))*100) as total_persent,sum(convert(float, Full_marks)) as Full_marks from ( select t2.Subject_Short_Name,t2.Subject_position,sum(convert(float, Marks)) as Obtt_mark,sum(convert(float, Full_marks)) as Full_marks from Exam_temp_assesment_total_no t1 join Subject_Master t2 on t1.Class_id=t2.course_id and t1.Subject_id=t2.Subject_id where t1.Session_id='" + Session_id + "' and t1.Class_id='" + Class_id + "' and t1.Term_id='" + Term_idI + "' and  t1.Admission_no='" + admission_no + "' group by t2.Subject_Short_Name,t2.Subject_id,t2.Subject_position  union all select t2.Subject_Short_Name,t2.Subject_position,sum(convert(float, Marks)) as Obtt_mark,sum(convert(float, Full_marks)) as Full_marks from Exam_temp_assesment_total_no_term_II t1 join Subject_Master t2 on t1.Class_id=t2.course_id and t1.Subject_id=t2.Subject_id where t1.Session_id='" + Session_id + "' and t1.Class_id='" + Class_id + "' and t1.Term_id='" + Term_idI + "' and  t1.Admission_no='" + admission_no + "' group by t2.Subject_Short_Name,t2.Subject_id,t2.Subject_position union all select t2.Subject_Short_Name,t2.Subject_position,sum(convert(float, Marks)) as Obtt_mark,sum(convert(float, Full_marks)) as Full_marks from Exam_temp_assesment_total_no_term_II t1 join Subject_Master t2 on t1.Class_id=t2.course_id and t1.Subject_id=t2.Subject_id where t1.Session_id='" + Session_id + "' and t1.Class_id='" + Class_id + "' and t1.Term_id='" + Term_idI + "' and  t1.Admission_no='" + admission_no + "' group by t2.Subject_Short_Name,t2.Subject_id,t2.Subject_position) t group by Subject_Short_Name,Subject_position order by  Subject_position asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
        ...
        private List<MySubjectMarkShow> findSubjectMarkShow(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_idI, string Term_idII, string Term_idIII, string UserType, string Section)
        {
            ...
            else
            {
                ...
                double totalMaxMarks = 0; double totaloverALLobtMarkSubj = 0; double overallAverage = 0;
                foreach (DataRow dr in dt.Rows)
                {
                    string ttl_mark_of_a_subject = namanFInal.get_ttl_mark_of_a_subject_final_year(dr["Subject_id"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_idI, Term_idII, Term_idIII);

                    string sub_full_mark = get_full_mark_of_subject(dr["Subject_id"].ToString(), Session_id, Class_id, dr["Section"].ToString(), Term_idI);
                    totalMaxMarks = totalMaxMarks + My.toDouble(sub_full_mark);
                    ...
                    totaloverALLobtMarkSubj = totaloverALLobtMarkSubj + My.toDouble(Total_mark_of_a_subject);
                    overallAverage = overallAverage + My.toDouble(termI_termIi_average_percent);
                    ...
                    string Total_marks = namanFInal.get_overall_total_marks_final(Session_id, Class_id, Admission_no, Branch_id, Term_idI, Term_idII, Term_idIII);
                    ...
                    MySubjectMarkShowItem.Add(new MySubjectMarkShow
                    {
                        ...
                        Hdr_frst = hdr_frst,
                        Hdr_scnd = hdr_scnd,
                        Fub_full_mark = sub_full_mark,
                        IsPassI = IsPassI,
                        IsPassII = IsPassII,
                        IsPassIII = IsPassIII,
                        TotalMaxMarks = totalMaxMarks.ToString(),

                        duesclassdesabled = isDues,
                        duesclassdesabled2 = isDues2,
                        Desabletext = desabletext,
                        PrintShow = printShow,
                        RowscountS = (rowcount1 - 1).ToString(),
                        Total_marks_of_a_subject = Total_mark_of_a_subject,
                        totaloverALLobtMarkSubj = totaloverALLobtMarkSubj.ToString(),
                        OverallAverage = overallAverage.ToString("0.00"),
                    });
                }
            }
            return MySubjectMarkShowItem;
        }
        private string get_full_mark_of_subject(string Subject_id, string session_id, string class_id, string Section, string term_id)
        {
            string rtrnValue = "0";
            DataTable dt = My.dataTable("select Maximum_Marks from Exam_Subject_Sub_Level where Session_Id='" + session_id + "' and Class_id='" + class_id + "' and Branch_Id='1' and Istatus=1 and Exam_Term_Id='" + term_id + "' and Subject_id='" + Subject_id + "'");
            if (dt.Rows.Count > 0)
            {
                rtrnValue = dt.Rows[0]["Maximum_Marks"].ToString();
            }
            return rtrnValue;
        }
        ...
        private List<MyRpTotalCalculation> findTotCalculation(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_idI, string Term_idII, string Term_idIII, string Section)
        {
            ...
            string comentory_remarks = ""; // finalThree.get_comentoryRemarks(Session_id, Class_id, Admission_no, Branch_id, Term_idIII);
            ////======================================
            ...
            string termGradeT1 = get_grade(Session_id, Branch_id, Class_id, Term_idI, My.toDouble(finl_percT1), Grade_System_Id);
            string termGradeT2 = get_grade(Session_id, Branch_id, Class_id, Term_idI, My.toDouble(finl_percT2), Grade_System_Id);
            string termGradeT3 = get_grade(Session_id, Branch_id, Class_id, Term_idI, My.toDouble(finl_percT3), Grade_System_Id);


            //string percent_remark = exam_ips_I.get_percent_remark(overall_percent);
            string class_teacher_remark = get_class_teacher_remark(Session_id, Branch_id, Class_id, Admission_no, Term_idIII, Section);
            string session_start_from = "月";//session_starts_from(Session_id, Class_id);
            string[] stringSeparator = new string[] { "月" };
            string[] arr = session_start_from.Split(stringSeparator, StringSplitOptions.None);
            session_start_from = arr[0];
            string issue_date = arr[1];



            string ranks = get_ranks(Admission_no, Session_id, Class_id, Branch_id, Term_idI, Term_idII, Term_idIII, Section);
            string[] stringSeparatorR = new string[] { "月" };
            string[] arrR = ranks.Split(stringSeparatorR, StringSplitOptions.None);
            string RankT1 = arrR[0];
            string RankT2 = arrR[1];
            string RankT3 = arrR[2];
            string RankFinal = arrR[3];
            ...
            MyRpTotalCalculationItem.Add(new MyRpTotalCalculation
            {
                ...
                TermGradeT3 = termGradeT3,
                Session_start_from = session_start_from,
        ...
        private string get_ranks(string admission_no, string session_id, string class_id, string branch_id, string term_idI, string term_idII, string term_idIII, string section)
        {
            string rtrN = "0月0月0月0";
            string query = "SELECT  'Term1' AS Term,  ISNULL(er.Rank, 0) AS Ranks FROM  (SELECT 1 AS Dummy) AS d LEFT JOIN  Exam_rank_master er ON er.Session_id = '" + session_id + "' AND er.Admission_no = '" + admission_no + "' AND er.Class_id = '" + class_id + "'  AND er.Term_id = '" + term_idI + "' AND er.Section = '" + section + "' union all SELECT 'Term2' AS Term,  ISNULL(er.Rank, 0) AS Ranks FROM  (SELECT 1 AS Dummy) AS d LEFT JOIN  Exam_rank_master er ON er.Session_id = '" + session_id + "' AND er.Admission_no = '" + admission_no + "' AND er.Class_id = '" + class_id + "'  AND er.Term_id = '" + term_idII + "' AND er.Section = '" + section + "' union all SELECT 'Term3' AS Term,  ISNULL(er.Rank, 0) AS Ranks FROM  (SELECT 1 AS Dummy) AS d LEFT JOIN  Exam_rank_master er ON er.Session_id = '" + session_id + "' AND er.Admission_no = '" + admission_no + "' AND er.Class_id = '" + class_id + "'  AND er.Term_id = '" + term_idIII + "' AND er.Section = '" + section + "' union all SELECT 'Final' AS Term,  ISNULL(er.Rank, 0) AS Ranks FROM  (SELECT 1 AS Dummy) AS d LEFT JOIN  Exam_rank_master_final_year er ON er.Session_id = '" + session_id + "' AND er.Admission_no = '" + admission_no + "' AND er.Class_id = '" + class_id + "' AND er.Section = '" + section + "'";
        ...
        private string session_starts_from(string session_id, string class_id)
        {
            string rtrN = " 月 ";
            string query = "select top 1 Remark,Issue_date from Exam_session_start_from_date where Session_id='" + session_id + "' and Class_id='" + class_id + "'";
            DataTable dt = My.dataTable(query);
            if (dt.Rows.Count > 0)
            {
                rtrN = dt.Rows[0]["Remark"].ToString() + "月" + dt.Rows[0]["Issue_date"].ToString();
            }
            return rtrN;
        }

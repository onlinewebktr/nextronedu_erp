...
namespace school_web.Examination_Admin.slip.sdhr.api
{
    /// <summary>
    /// Summary description for bulk_report_card_vi_to_viii
    /// </summary>
    ...
    public class bulk_report_card_vi_to_viii : System.Web.Services.WebService
    {
        My mycode = new My();
        ...
        public class MySubjectMarkShowII
        {
            ...
            public string Is_aff_no_hide { get; set; }
            public string Full_marks { get; set; }
            public string CstmSUbj { get; set; }
        ...
        public class MySubjectMarkDetailsIII
        {
            ...
            public string Subject { get; set; }
            public string Total_marks { get; set; }
            public string AssesmtName { get; set; }
            public string BRhideN { get; set; }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Admission_no)
        {
            ...
            else
            {
                ...
                {
                    List<MySubjectMarkShowII> MBdetails = findSubjectMarkShowII(dr["Admission_no"].ToString(), Session_id, Class_id, Branch_id, Term_id);
                    ...
                    string signatures = Examination.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = Examination.get_term_name(Branch_id, Term_id);
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = Examination.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string grade = Examination.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_report_head> findSubjectHeadings(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (i == 1)
                    {
                        extra_marks = Examination.get_extra_marks(Class_id, Branch_id);
                        if (extra_marks != "0")
                        {
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = up_to_vi_to_viii.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic", "0", "0", "0");
                    string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_report_descpline> findDescplineData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = up_to_vi_to_viii.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        private List<Fetch_Details_of_report_grade_remark> findGradeRemarkData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            string sys_grade_id = up_to_vi_to_viii.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    List<MySubjectMarkDetailsIII> MBdetailsIII = findmyMarksIII(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id, dr["Subject_group"].ToString());
                    string Total_marks = up_to_vi_to_viii.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };
                    string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);
                    ...
                    string full_marks = arrs[48];   // FullMarks
                    //QQQQQRRRR 
                    string url = "";
                    ...
                    string cstmSUbj = "hidden";
                    if (dr["Subject_group"].ToString() == "1")
                    {
                        cstmSUbj = "showd";
                    }
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string Subject_group)
        {
            ...
            {
                string subject_name = Examination.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    if (Subject_group == "2")
                    {
                        if (rowcount1 == My.toIntS(dr["Sequence_No"].ToString()))
                        {
                            string qrys = "select * from Exam_Subject_Sub_Level where Session_Id='" + Session_id + "' and Branch_Id='" + Branch_id + "' and Exam_Term_Id='" + Term_id + "' and Assessment_Id='" + dr["Assessment"].ToString() + "' and Class_id='" + Class_id + "' and Subject_id='" + Subject_id + "' order by Subject_Activity_Name desc";
                            DataTable dtS = My.dataTable(qrys);
                            if (dtS.Rows.Count > 0)
                            {
                                foreach (DataRow drS in dtS.Rows)
                                {
                                    string marks = up_to_vi_to_viii.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic", Subject_group, drS["Maximum_Marks"].ToString(), drS["Subject_Sub_Level_Id"].ToString());
                                    string[] stringSeparatorss = new string[] { "/" };
                                    string[] arrs = marks.Split(stringSeparatorss, StringSplitOptions.None);
                                    string obt_marks = arrs[0];
                                    string full_marks = arrs[1];
                                    ...
                                    {
                                        save_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), obt_marks, full_marks, marks_type, total_no, drS["Subject_Sub_Level_Id"].ToString());
                                    }
                                    MySubjectMarkItemIII.Add(new MySubjectMarkDetailsIII
                                    {
                                        ...
                                        Subject = Subject_id,
                                        Total_marks = "0",
                                        AssesmtName = drS["Subject_Activity_Name"].ToString() + " (" + drS["Maximum_Marks"].ToString() + ")",
                                        colSpan = "2",
                                    }); ;
                                }
                            }
                    ...
                    {
                        string assesmtName = ""; string BRhideN = "hidden"; string maxMark = "0";
                        if (rowcount1 == My.toIntS(dr["Sequence_No"].ToString()))
                        {
                            if (Subject_group == "1")
                            {
                                string querys = "Select essl.*,(Select top 1 Assessment_Name from Exam_Assessment_Details where Assessment_Id=essl.Assessment_Id and Branch_Id=essl.Branch_Id) as Assessment_Name,(Select top 1 Short_Name from Exam_Assessment_Details where Assessment_Id=essl.Assessment_Id and Branch_Id=essl.Branch_Id) as Assessment_Short_Name from Exam_Subject_Sub_Level essl where essl.Class_id='" + Class_id + "' and essl.Session_Id='" + Session_id + "' and essl.Branch_Id=" + Branch_id + " and essl.Exam_Term_Id=" + Term_id + " and essl.Assessment_Id=" + dr["Assessment"].ToString() + " and essl.Subject_id=" + Subject_id + "";
                                DataTable dtassement = My.dataTable(querys);
                                if (dtassement.Rows.Count > 0)
                                {
                                    BRhideN = "showD";
                                    assesmtName = dtassement.Rows[0]["Assessment_Short_Name"].ToString() + " (" + dtassement.Rows[0]["Maximum_Marks"].ToString() + ")";
                                    maxMark = dtassement.Rows[0]["Maximum_Marks"].ToString();
                                }
                            }
                        }

                        string marks = up_to_vi_to_viii.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic", Subject_group, maxMark, "0");
                        string[] stringSeparatorss = new string[] { "/" };
                        string[] arrs = marks.Split(stringSeparatorss, StringSplitOptions.None);
                        string obt_marks = arrs[0];
                        string full_marks = arrs[1];
                        ...
                        {
                            save_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), obt_marks, full_marks, marks_type, total_no, "0");
                        }
                        MySubjectMarkItemIII.Add(new MySubjectMarkDetailsIII
                        {
                            //Marks = dr["Marks"].ToString(),
        ...
        private void save_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string Branch_id, string marks, string full_marks, string marks_type, string total_no, string Subject_Sub_Level_Id)
        {
            ...
            {
                if (mycode.IsUserExist("select Id from Exam_temp_assesment_total_no where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Subject_sub_level_id='" + Subject_Sub_Level_Id + "'"))
                {
                    SqlCommand cmd;
                    string query = "INSERT INTO Exam_temp_assesment_total_no (Session_id,Branch_id,Class_id,Term_id,Assessment_id,Subject_id,Admission_no,Marks,Full_marks,Subject_sub_level_id) values (@Session_id,@Branch_id,@Class_id,@Term_id,@Assessment_id,@Subject_id,@Admission_no,@Marks,@Full_marks,@Subject_sub_level_id)";
                ...
                {
                    SqlCommand cmd;
                    string query = "Update Exam_temp_assesment_total_no set Marks=@Marks,Full_marks=@Full_marks where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Subject_sub_level_id='" + Subject_Sub_Level_Id + "'";
                    cmd = new SqlCommand(query);
                    if (marks_type == "Marks")
                    {
                        cmd.Parameters.AddWithValue("@Marks", marks);
            ...
            else
            {
                ...
                {
                    SqlCommand cmd;
                    string query = "INSERT INTO Exam_temp_assesment_total_no_term_II (Session_id,Branch_id,Class_id,Term_id,Assessment_id,Subject_id,Admission_no,Marks,Full_marks,Subject_sub_level_id) values (@Session_id,@Branch_id,@Class_id,@Term_id,@Assessment_id,@Subject_id,@Admission_no,@Marks,@Full_marks,@Subject_sub_level_id)";
                    cmd = new SqlCommand(query);
                    cmd.Parameters.AddWithValue("@Session_id", session_id);
                    cmd.Parameters.AddWithValue("@Branch_id", Branch_id);
                    cmd.Parameters.AddWithValue("@Class_id", class_id);
                ...
                {
                    SqlCommand cmd;
                    string query = "Update Exam_temp_assesment_total_no_term_II set Marks=@Marks,Full_marks=@Full_marks where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Subject_sub_level_id='" + Subject_Sub_Level_Id + "'";
                    cmd = new SqlCommand(query);
                    if (marks_type == "Marks")
                    {
                        cmd.Parameters.AddWithValue("@Marks", marks);

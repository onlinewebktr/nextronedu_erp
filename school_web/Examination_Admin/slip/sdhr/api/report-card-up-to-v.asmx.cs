...
{
    /// <summary>
    /// Summary description for report_card_up_to_v
    /// </summary>
    ...
    public class report_card_up_to_v : System.Web.Services.WebService
    {
        My mycode = new My();
        ...
        private List<MySubjectMarkDetails> findmyBookingProduct(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
                string subject_name = up_to_v.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
        ...
        {
            string sys_grade_id = up_to_v.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        public void fetch_rp_card_signature(string Session_id, string Class_id, string Admission_no, string Branch_id)
        {
            ...
            {
                string Section = up_to_v.get_student_section(Session_id, Admission_no, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string stgnature = "";
                    if (dr["Signature_type"].ToString() == "1")
                    {
                        stgnature = up_to_v.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = up_to_v.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = up_to_v.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        public void fetch_report_card_coscholastic(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = up_to_v.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        public void fetch_report_card_discipline_activity(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = up_to_v.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        {
            string section = up_to_v.get_section(Session_id, Class_id, Admission_no, Branch_id);
            ...
            {
                string system_grade_id = up_to_v.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = up_to_v.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        {
            string Total_marks = up_to_v.get_overall_total_marks(Session_id, Class_id, Admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);
            ...
            string percent_remark = up_to_v.get_percent_remark(overall_percent);

...
namespace school_web.Examination_Admin.slip.sdhr.api
{
    public class up_to_vi_to_viii
    {
        ...
        //=================================
        internal static string get_marks(string Session_id, string Class_id, string Admission_no, string Term_id, string Assessment_id, string Subject_id, string Branch_id, string Subject_type, string Subject_group, string maxMark, string sub_subj_id)
        {
            ...
            if (Calculation_Type == "Sum")
            {
                ...
                else
                {
                    ...
                    if (Subject_group == "2" && sub_subj_id != "0")
                    {
                        query = "select em.Marks from Exam_marks em join Exam_Subject_Sub_Level essl on em.Session_id=essl.Session_id and em.Branch_id=essl.Branch_id and em.Class_id=essl.Class_id and em.Term=essl.Exam_Term_Id and em.Assessment=essl.Assessment_Id and em.Subject=essl.Subject_id and em.Subject_activity=essl.Subject_Sub_Level_Id where em.Session_Id='" + Session_id + "' and em.Branch_Id='" + Branch_id + "' and em.Term=" + Term_id + " and em.Assessment=" + Assessment_id + " and em.Class_id=" + Class_id + " and em.Subject=" + Subject_id + " and em.Admission_no='" + Admission_no + "' and essl.Subject_Sub_Level_Id='" + sub_subj_id + "'";
                    }
                    SqlCommand cmd = new SqlCommand(query);
                    DataTable dt = FillDatastatic(query);
                    if (dt.Rows.Count == 0)
            ...
            if (Subject_group == "1" || Subject_group == "2")
            {
                if (My.toDouble(maxMark) > 0)
                {
                    assesment_weightage = maxMark;
                }
            }
        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string exam_settings = SdhrExmSettings.get_exam_settings(session_id, branch_id, class_id);  // Settings

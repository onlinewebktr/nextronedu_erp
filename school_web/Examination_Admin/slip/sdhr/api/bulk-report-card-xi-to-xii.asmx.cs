...
{
    /// <summary>
    /// Summary description for bulk_report_card_xi_to_xii
    /// </summary>
    ...
    public class bulk_report_card_xi_to_xii : System.Web.Services.WebService
    {
        My mycode = new My();
        public class MyStudentsDetails
        {
            ...
            public List<Fetch_Details_of_report_head_obt_full> MySubjectHeading_obt_full { get; set; }
        ...
        public class Fetch_Details_of_report_head_obt_full
        {
            public string Short_Name { get; set; }
        }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Admission_no)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    List<Fetch_Details_of_report_head_obt_full> MBdetailsHeading_obt_full = findSubjectHeadings_obt_full(Session_id, Class_id, Branch_id, Term_id);
                    ...
                    ShowMyStudents.Add(new MyStudentsDetails
                    {
                        ...
                        MySubjectHeading_obt_full = MBdetailsHeading_obt_full,
        ...
        //=================================HEADING
        private List<Fetch_Details_of_report_head_obt_full> findSubjectHeadings_obt_full(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            List<Fetch_Details_of_report_head_obt_full> MySubjectHeading_obt_full = new List<Fetch_Details_of_report_head_obt_full>();
            string query = "select Assessment_Name,Short_Name,Assessment_Id,Maximum_Marks,(select top 1 Maximum_Marks from Exam_Term_Details where Session_Id=Exam_Assessment_Details.Session_Id and Branch_Id=Exam_Assessment_Details.Branch_Id and Exam_Term_Id=Exam_Assessment_Details.Exam_Term_Id) as Term_Maximum_Marks from Exam_Assessment_Details where Session_Id='" + Session_id + "' and  Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Exam_Term_Id=" + Term_id + "  and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Exam_marks");
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    MySubjectHeading_obt_full.Add(new Fetch_Details_of_report_head_obt_full
                    {
                        Short_Name = "F. M.",
                    });
                    MySubjectHeading_obt_full.Add(new Fetch_Details_of_report_head_obt_full
                    {
                        Short_Name = "MARKS OBTN.",
                    });
                }
            }
            return MySubjectHeading_obt_full;
        }
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = xi_to_xii.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = xi_to_xii.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };
                    string[] arrs = marks.Split(stringSeparatorss, StringSplitOptions.None);
                    string obt_marks = arrs[0];
                    string full_marks = arrs[1];

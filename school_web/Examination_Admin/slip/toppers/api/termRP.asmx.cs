...
namespace school_web.Examination_Admin.slip.toppers.api
{
    /// <summary>
    /// Summary description for termRP
    /// </summary>
    ...
    public class termRP : System.Web.Services.WebService
    {
        My mycode = new My();
        public class MyStudentsDetails
        {
            ...
            public List<Fetch_Details_of_performance_subject> MyPerformanceDatasubjItem { get; set; }
            public List<Fetch_Details_of_performance_content> MyPerformanceContentItem { get; set; }
            public List<Fetch_Details_of_analysis_content> MyanalysisContentItem { get; set; }
        }
        ...
        public class MySubjectMarkShowII
        {
            ...
            public string SubjHighestMark { get; set; }
        ...
        public class Fetch_Details_of_performance_subject
        {
            public string PerfSubj { get; set; }
        }

        public class Fetch_Details_of_performance_content
        {
            public string SubjName { get; set; }
            public string PerfSubjContent { get; set; }
            public string HighestMark { get; set; }
            public string Average { get; set; }
            public string Average_in_hundred { get; set; }

            public string Average_in_hundred_Self { get; set; }
            public string Average_in_hundred_for_grapg { get; set; }
            public string HighestMark_in_hundred { get; set; }
        }
        public class Fetch_Details_of_analysis_content
        {
            public string examName { get; set; }
            public List<Fetch_Details_of_analysis_sub_content> MyanalysisSubContentItem { get; set; }
        }

        public class Fetch_Details_of_analysis_sub_content
        {
            public string marks { get; set; }
        }
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Adm_no)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    //================ PerformanceSubject
                    List<Fetch_Details_of_performance_subject> MBOverALLPsubj = findPerfSubjDetails(Session_id, dr["Admission_no"].ToString(), Class_id, Branch_id, dr["Section"].ToString(), Term_id, dr["studentname"].ToString(), dr["class"].ToString(), dr["rollnumber"].ToString());


                    //================ PerformanceContent
                    List<Fetch_Details_of_performance_content> MBOverALLPsubjCnt = findPerfSubjContentDetails(Session_id, dr["Admission_no"].ToString(), Class_id, Branch_id, dr["Section"].ToString(), Term_id, dr["studentname"].ToString(), dr["class"].ToString(), dr["rollnumber"].ToString());

                    //================ Comprative Analysis
                    List<Fetch_Details_of_analysis_content> MBOverALLPsubjanalysisCnt = findanalysisSubjContentDetails(Session_id, dr["Admission_no"].ToString(), Class_id, Branch_id, dr["Section"].ToString(), Term_id, dr["studentname"].ToString(), dr["class"].ToString(), dr["rollnumber"].ToString());


                    string signatures = toppersClass.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = toppersClass.get_term_name(Branch_id, Term_id);
                    string rank = "0";
                    ...
                    ShowMyStudents.Add(new MyStudentsDetails
                    {
                        ...
                        MyPerformanceDatasubjItem = MBOverALLPsubj,
                        MyPerformanceContentItem = MBOverALLPsubjCnt,
                        MyanalysisContentItem = MBOverALLPsubjanalysisCnt,
                    });
                }
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id, string studentname, string class_name, string rollnumber)
        {
            ...
            string Total_marks = toppersClass.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    MyGraphDetailData.Add(new Fetch_Details_of_graph
                    {
                        ...
                        final_perc = My.toDouble(final_perc.ToString()).ToString("0.0"),
                        bg_color = bg_color,
                    });
                }
            }
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = toppersClass.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = toppersClass.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_Signature> findSigDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        stgnature = toppersClass.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = toppersClass.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = toppersClass.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        private List<Fetch_Details_of_report_head> findSubjectHeadings(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (i == 1)
                    {
                        extra_marks = toppersClass.get_extra_marks(Class_id, Branch_id);
                        if (extra_marks != "0")
                        {
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = toppersClass.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        private List<Fetch_Details_of_report_descpline> findDescplineData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = toppersClass.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        private List<Fetch_Details_of_report_grade_remark> findGradeRemarkData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            string sys_grade_id = toppersClass.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string firm_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = toppersClass.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string percent_remark = toppersClass.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
                    ...
                    string subjHighestMark = get_highestMark(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), dr["Section"].ToString());
                    MySubjectMarkShowItemII.Add(new MySubjectMarkShowII
                    {
                        ...
                        SubjHighestMark = subjHighestMark,
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            if (rowcount1 > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = toppersClass.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
        ...
        private List<Fetch_Details_of_performance_subject> findPerfSubjDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id, string studentname, string class_name, string rollnumber)
        {
            List<Fetch_Details_of_performance_subject> MyPerformanceDatasubjItem = new List<Fetch_Details_of_performance_subject>();

            string query = "select DISTINCT t1.Session_id,t1.Class_id,t1.Term,t1.Subject,t1.Admission_no,t1.Branch_id,t2.Subject_name,t2.Subject_Code,t2.Subject_position from Exam_marks t1 join Subject_Master t2 on t1.Subject=t2.Subject_id where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Admission_no='" + admission_no + "' and t1.Term=" + Term_id + " and t2.Subject_Type_Scholastic_Co_Scholastic='Scholastic' and t2.Is_mandatory=1 and t1.Subject in (select Sub_id from Subject_Mapping_New where Class_id='" + Class_id + "' and Admission_no='" + admission_no + "' and Session_id='" + Session_id + "') order by t2.Subject_position asc";
            DataTable dt = My.dataTable(query);
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    MyPerformanceDatasubjItem.Add(new Fetch_Details_of_performance_subject
                    {
                        PerfSubj = dr["Subject_name"].ToString(),
                    });
                }
            }
            return MyPerformanceDatasubjItem;
        }

        private List<Fetch_Details_of_performance_content> findPerfSubjContentDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id, string studentname, string class_name, string rollnumber)
        {
            List<Fetch_Details_of_performance_content> MyPerformanceContentItem = new List<Fetch_Details_of_performance_content>();
            string query = "select *,(cast(TotalMarkOfClass as float)/cast(TotalStd as float)) as Average,((cast(SelfMark as float)/cast(FullMarkSelf as float))*100) as Average_in_hundred,((cast(HighestMark as float)/cast(FullMarkSelf as float))*100) as HighestMark_in_hundred,(((cast(TotalMarkOfClass as float)/cast(TotalStd as float))/cast(FullMarkSelf as float))*100) as Average_in_hundred_for_grapg from (select DISTINCT t1.Session_id,t1.Class_id,t1.Term,t1.Subject,t1.Admission_no,t1.Branch_id,t2.Subject_name,t2.Subject_Code,t2.Subject_position,(select sum(cast(Marks as float)) from Exam_assesment_total_no_studentwise where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Term_id='" + Term_id + "' and Admission_no='" + admission_no + "' and Subject_id=t1.Subject) as SelfMark,(select sum(cast(Full_marks as float)) from Exam_assesment_total_no_studentwise where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Term_id='" + Term_id + "' and Admission_no='" + admission_no + "' and Subject_id=t1.Subject) as FullMarkSelf,(select top 1 Marks from Exam_highest_mark_of_subject_termwise where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Term_id='" + Term_id + "' and Section='" + Section + "' and Subject_id=t1.Subject) as HighestMark,(select sum(cast(Marks as float)) from Exam_assesment_total_no_studentwise where Session_id='" + Session_id + "' and Class_id='" + Class_id + "' and Term_id='" + Term_id + "' and Section='" + Section + "' and Subject_id=t1.Subject) as TotalMarkOfClass,(select count(distinct Admission_no)  from Exam_assesment_total_no_studentwise  where Session_id = '" + Session_id + "' and Class_id = '" + Class_id + "' and Term_id = '" + Term_id + "'  and Section = '" + Section + "' and Subject_id = t1.Subject) as TotalStd from Exam_marks t1 join Subject_Master t2 on t1.Subject = t2.Subject_id where t1.Session_id = '" + Session_id + "' and t1.Class_id ='" + Class_id + "' and t1.Branch_id = '" + Branch_id + "' and t1.Admission_no = '" + admission_no + "' and t1.Term ='" + Term_id + "' and t2.Subject_Type_Scholastic_Co_Scholastic = 'Scholastic' and t2.Is_mandatory = 1 and t1.Subject in (select Sub_id from Subject_Mapping_New where Class_id ='" + Class_id + "' and Admission_no ='" + admission_no + "' and Session_id = '" + Session_id + "')) t order by Subject_position asc";
            DataTable dt = My.dataTable(query);
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    MyPerformanceContentItem.Add(new Fetch_Details_of_performance_content
                    {
                        SubjName = dr["Subject_Code"].ToString(),
                        PerfSubjContent = dr["SelfMark"].ToString(),
                        HighestMark = dr["HighestMark"].ToString(),
                        Average = My.toDouble(dr["Average"].ToString()).ToString("0.0"),
                        Average_in_hundred = My.toDouble(dr["Average_in_hundred"].ToString()).ToString("0.0"),

                        Average_in_hundred_Self = My.toDouble(dr["Average_in_hundred"].ToString()).ToString("0.0"),
                        Average_in_hundred_for_grapg = My.toDouble(dr["Average_in_hundred_for_grapg"].ToString()).ToString("0.0"),
                        HighestMark_in_hundred = My.toDouble(dr["HighestMark_in_hundred"].ToString()).ToString("0.0"),
                    });
                }
            }
            return MyPerformanceContentItem;
        }


        private List<Fetch_Details_of_analysis_content> findanalysisSubjContentDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id, string studentname, string class_name, string rollnumber)
        {
            List<Fetch_Details_of_analysis_content> MyanalysisContentItem = new List<Fetch_Details_of_analysis_content>();
            string subquery = "select   * from Exam_Assessment_Details where Session_Id='" + Session_id + "' and Class_id='" + Class_id + "' and Exam_Term_Id='" + Term_id + "' and Istatus=1 and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc ";

            DataTable dt1 = My.dataTable(subquery);
            if (dt1.Rows.Count > 0)
            {
                foreach (DataRow dr in dt1.Rows)
                {
                    List<Fetch_Details_of_analysis_sub_content> MBOversubjanalysis = findanalysisSubjmarks(Session_id, admission_no, Class_id, Branch_id, Section, Term_id, studentname, class_name, rollnumber, dr["Assessment_Id"].ToString(), dr["Maximum_Marks"].ToString());

                    MyanalysisContentItem.Add(new Fetch_Details_of_analysis_content
                    {
                        examName = dr["Assessment_Name"].ToString(),
                        MyanalysisSubContentItem = MBOversubjanalysis,
                    });
                }
            }
            return MyanalysisContentItem;
        }



        private List<Fetch_Details_of_analysis_sub_content> findanalysisSubjmarks(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id, string studentname, string class_name, string rollnumber, string Assessment_Id, string fullmarks)
        {
            List<Fetch_Details_of_analysis_sub_content> MyanalysisSubContentItem = new List<Fetch_Details_of_analysis_sub_content>();
            string query = "select DISTINCT t2.Subject_name,t2.Subject_Code,t2.Subject_id,t2.Subject_position from Exam_marks t1 join Subject_Master t2 on t1.Subject=t2.Subject_id where t1.Session_id=" + Session_id + " and t1.Class_id=" + Class_id + " and t1.Branch_id='" + Branch_id + "' and t1.Admission_no='" + admission_no + "' and t1.Term=" + Term_id + " and t2.Subject_Type_Scholastic_Co_Scholastic='Scholastic' and t2.Is_mandatory=1 and t1.Subject in (select Sub_id from Subject_Mapping_New where Class_id='" + Class_id + "' and Admission_no='" + admission_no + "' and Session_id='" + Session_id + "') and t1.Assessment='" + Assessment_Id + "' order by t2.Subject_position asc";
            DataTable dt1 = My.dataTable(query);
            if (dt1.Rows.Count > 0)
            {
                foreach (DataRow dr in dt1.Rows)
                {
                    string obt_marks = get_marks_subject_wise(Session_id, Class_id, admission_no, Term_id, Assessment_Id, Branch_id, dr["Subject_id"].ToString(), fullmarks);

                    MyanalysisSubContentItem.Add(new Fetch_Details_of_analysis_sub_content
                    {
                        marks = obt_marks,
                    });
                }
            ...
            return MyanalysisSubContentItem;
        }

        public string get_marks_subject_wise(string session_id, string class_id, string admission_no, string term_id, string assessment_Id, string branch_id, string Subject_id, string assessmentweighted)
        {
            string Maximum_Marks = toppersClass.get_assesment_max_marks(session_id, branch_id, class_id, term_id, assessment_Id, Subject_id);
            DataTable dt1 = My.dataTable("select  Marks from Exam_marks where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Subject='" + Subject_id + "' and Assessment='" + assessment_Id + "' and Term='" + term_id + "' and Admission_no='" + admission_no + "' and Branch_id='" + branch_id + "'   ");
            if (dt1.Rows.Count > 0)
            {
                string Marks = dt1.Rows[0]["Marks"].ToString();
                bool tot = mycode.cheknumer_Double(Marks);
                if (tot == false)
                {
                    return "-";
                ...
                {
                    double final_marks = My.toDouble(Marks) / My.toDouble(Maximum_Marks) * My.toDouble(assessmentweighted);

                    // now claculate 100 weighted

                    double final_marks_100 = My.toDouble(final_marks) / My.toDouble(assessmentweighted) * 100;
                    if (decimal.TryParse(final_marks_100.ToString(), out decimal val))
                    {
                        return val % 1 == 0
                            ? ((int)val).ToString()
                            : val.ToString().TrimEnd('0').TrimEnd('.');
                    ...
                    {
                        return final_marks_100.ToString();
                    }
                }

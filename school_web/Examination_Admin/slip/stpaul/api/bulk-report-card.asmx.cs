...
using school_web.Examination_Admin.slip.stmarry.api;
using System;
...
namespace school_web.Examination_Admin.slip.stpaul.api
{
    ...
    public class bulk_report_card : System.Web.Services.WebService
    {
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string signatures = termwise.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = termwise.get_term_name(Branch_id, Term_id);
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string Total_marks = termwise.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            ...
            double roundPrc = Math.Round(My.toDouble(overall_percent));
            string finl_perc = overall_percent;
            if (roundPrc == My.toDouble(overall_percent))
            {
                finl_perc = My.toDouble(overall_percent).ToString();
            }
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            {
                string system_grade_id = termwise.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = termwise.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_Signature> findSigDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        stgnature = termwise.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = termwise.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = termwise.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        private List<Fetch_Details_of_report_head> findSubjectHeadings(string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (i == 1)
                    {
                        extra_marks = termwise.get_extra_marks(Class_id, Branch_id);
                        if (extra_marks != "0")
                        {
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = termwise.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        private List<Fetch_Details_of_report_descpline> findDescplineData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = termwise.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        private List<Fetch_Details_of_report_grade_remark> findGradeRemarkData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            string sys_grade_id = termwise.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = termwise.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string Full_mark_of_a_subject = arrs[49];   // Full_mark_of_a_subject
                    string percent_remark = termwise.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
                    ...
                    if (My.toint(Full_mark_of_a_subject) == 100)
                    {
                    ...
                    //Is PASS or FAILS
                    string PassoRFail = "classpass";
                    if (My.toint(Full_mark_of_a_subject) == 100)
                    {
                        if (My.toint(obt_subject_mark) < 40)
                        {
                            PassoRFail = "classfail";
                        }
                    }
                    else
                    {
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            {
                string subject_name = termwise.get_subject_name(Subject_id, Class_id, Branch_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = termwise.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");

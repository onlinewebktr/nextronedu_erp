...
{
    /// <summary>
    /// Summary description for admit_card_third_shift
    /// </summary>
    ...
    public class admit_card_third_shift : System.Web.Services.WebService
    {
        #region THREESHIFT
        ...
        public class MyRoutineDetails
        {
            ...
            public string Subject_name3 { get; set; }
        ...
        public void fetch_admit_cards(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id, string Section, string Session_name, string Exam_id, string CheckedStd, string UserType, string PageNo)
        {
            ...
            string shift_time = get_shift_time(Session_id, Class_id, Branch_id, Term_id, Section, Exam_id);
            ...
            string shift3_start = arrs[4];
            string shift3_end = arrs[5];
            ...
            if (rowcount1 > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isduesShowOnlyStd == "No")
                    {
                        if (UserType != "Admin")
                        {
                            check_is_dues = Exam_setting.check_dues_for_admit_card(Session_id, Class_id, Admission_no, Term_id, Exam_id);
                        }
                    }
        ...
        private string get_sign(string type)
        {
            ...
            DataTable dt = My.dataTable("select Signature from user_details where User_Type='OFFICE IN CHARGE'");
            if (dt.Rows.Count > 0)
            {
        ...
        private string get_shift_time(string session_id, string class_id, string branch_id, string term_id, string section, string Exam_id)
        {
            string shift_time = "";
            DataTable dtt = My.dataTable("Select top 1 format(es.Exam_Date_time, 'hh:mm tt') as Exam_time1,format(es.Exam_E_Date_time, 'hh:mm tt') as Exam_E_time1 from Subject_Master sm join Exam_Time_Table es on es.Subject_id=sm.Subject_id and es.Class_id=sm.course_id where es.Class_id=" + class_id + " and es.Branch_id='" + branch_id + "' and es.Session_Id=" + session_id + " and es.Section='" + section + "' and es.Exam_Term_Id=" + term_id + " and es.Exam_id=" + Exam_id + " and Shift=1 UNION all Select top 1 format(es.Exam_Date_time, 'hh:mm tt') as Exam_time1, format(es.Exam_E_Date_time, 'hh:mm tt') as Exam_E_time1 from Subject_Master sm join Exam_Time_Table es on es.Subject_id = sm.Subject_id and es.Class_id = sm.course_id where es.Class_id= " + class_id + " and es.Branch_id = '" + branch_id + "' and es.Session_Id = " + session_id + " and es.Section = '" + section + "' and es.Exam_Term_Id = " + term_id + " and es.Exam_id=" + Exam_id + " and Shift = 2  UNION all Select top 1 format(es.Exam_Date_time, 'hh:mm tt') as Exam_time1, format(es.Exam_E_Date_time, 'hh:mm tt') as Exam_E_time1 from Subject_Master sm join Exam_Time_Table es on es.Subject_id = sm.Subject_id and es.Class_id = sm.course_id where es.Class_id= " + class_id + " and es.Branch_id = '" + branch_id + "' and es.Session_Id = " + session_id + " and es.Section = '" + section + "' and es.Exam_Term_Id = " + term_id + " and es.Exam_id=" + Exam_id + " and Shift =3");
            if (dtt.Rows.Count == 3)
            {
                shift_time = dtt.Rows[0]["Exam_time1"].ToString() + "~" + dtt.Rows[0]["Exam_E_time1"].ToString() + "~" + dtt.Rows[1]["Exam_time1"].ToString() + "~" + dtt.Rows[1]["Exam_E_time1"].ToString() + "~" + dtt.Rows[2]["Exam_time1"].ToString() + "~" + dtt.Rows[2]["Exam_E_time1"].ToString();
            }
            else if (dtt.Rows.Count == 2)
            {
                shift_time = dtt.Rows[0]["Exam_time1"].ToString() + "~" + dtt.Rows[0]["Exam_E_time1"].ToString() + "~" + dtt.Rows[1]["Exam_time1"].ToString() + "~" + dtt.Rows[1]["Exam_E_time1"].ToString() + "~NA" + "~NA";
            }
            else if (dtt.Rows.Count == 1)
            {
                shift_time = dtt.Rows[0]["Exam_time1"].ToString() + "~" + dtt.Rows[0]["Exam_E_time1"].ToString() + "~NA" + "~NA" + "~NA" + "~NA";
            }
            else
            {
                shift_time = "NA~NA~NA~NA~NA~NA";
            }
            return shift_time;
        }
        ...
        private List<MyRoutineDetails> findmyRoutine(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id, string Section, string Session_name, string Exam_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    //Second Setting
                    string secondShiftSubject = "";
                    DataTable dtSF = My.dataTable("Select es.Day,sm.Subject_name,format(es.Exam_Date_time, 'dd/MM/yyyy') as Exam_datetime1,format(es.Exam_Date_time, 'hh:mm tt') as Exam_time1,format(es.Exam_E_Date_time, 'hh:mm tt') as Exam_E_time1,Shift from Subject_Master sm join Exam_Time_Table es on es.Subject_id=sm.Subject_id and es.Class_id=sm.course_id join   Subject_Mapping_New smn on smn.Class_id=es.Class_id and smn.Sub_id=es.Subject_id and smn.Session_id=es.Session_Id  where es.Class_id=" + Class_id + " and es.Branch_id='" + Branch_id + "' and es.Session_Id=" + Session_id + " and es.Section='" + Section + "' and smn.Admission_no='" + Admission_no + "' and es.Exam_Term_Id=" + Term_id + " and format(es.Exam_Date_time, 'yyyyMMdd')='" + dr["Exam_idate"].ToString() + "' and es.Exam_id='" + Exam_id + "' and Shift='2' order by Shift asc");
                    if (dtSF.Rows.Count > 0)
                    {
                        foreach (DataRow drRF in dtSF.Rows)
                        {
                            secondShiftSubject = secondShiftSubject + drRF["Subject_name"].ToString() + " / ";
                        }
                        secondShiftSubject = secondShiftSubject.Remove(secondShiftSubject.Length - 2);
                        date = dtSF.Rows[0]["Exam_datetime1"].ToString();
                        day = dtSF.Rows[0]["Day"].ToString();
                        subjSecondShift = secondShiftSubject;
                    }
                    else
                    {
                        subjSecondShift = "-";
                    ...
                    //Third Setting
                    string thirdShiftSubject = "";
                    DataTable dtTHF = My.dataTable("Select es.Day,sm.Subject_name,format(es.Exam_Date_time, 'dd/MM/yyyy') as Exam_datetime1,format(es.Exam_Date_time, 'hh:mm tt') as Exam_time1,format(es.Exam_E_Date_time, 'hh:mm tt') as Exam_E_time1,Shift from Subject_Master sm join Exam_Time_Table es on es.Subject_id=sm.Subject_id and es.Class_id=sm.course_id join   Subject_Mapping_New smn on smn.Class_id=es.Class_id and smn.Sub_id=es.Subject_id and smn.Session_id=es.Session_Id  where es.Class_id=" + Class_id + " and es.Branch_id='" + Branch_id + "' and es.Session_Id=" + Session_id + " and es.Section='" + Section + "' and smn.Admission_no='" + Admission_no + "' and es.Exam_Term_Id=" + Term_id + " and format(es.Exam_Date_time, 'yyyyMMdd')='" + dr["Exam_idate"].ToString() + "' and es.Exam_id='" + Exam_id + "' and Shift='3' order by Shift asc");
                    if (dtTHF.Rows.Count > 0)
                    {
                        foreach (DataRow drRF in dtTHF.Rows)
                        {
                            thirdShiftSubject = thirdShiftSubject + drRF["Subject_name"].ToString() + " / ";
                        }
                        thirdShiftSubject = thirdShiftSubject.Remove(thirdShiftSubject.Length - 2);
                        date = dtTHF.Rows[0]["Exam_datetime1"].ToString();
                        day = dtTHF.Rows[0]["Day"].ToString();
                        subjThirdShift = thirdShiftSubject;
                    }
                    else
                    {
                        subjThirdShift = "-";

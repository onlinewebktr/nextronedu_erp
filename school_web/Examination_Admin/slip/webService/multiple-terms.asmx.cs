...
{
    /// <summary>
    /// Summary description for multiple_terms
    /// </summary>
    ...
    public class multiple_terms : System.Web.Services.WebService
    {
        My mycode = new My();
        ...
        public void fetch_report_card(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            {
                double Multi_term_max_marks = 0;
                for (int ix = 0; ix < dt.Rows.Count; ix++)
                {
                    Multi_term_max_marks = Multi_term_max_marks + My.toDouble(dt.Rows[ix]["Term_Maximum_Marks"].ToString());
                }
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    Show_of_report_head.Add(new Fetch_Details_of_report_head
                    {
                        ...
                        Term_Maximum_Marks = Multi_term_max_marks.ToString(),
                    });
                }
        ...
        public class Fetch_Details_of_report_head_ass
        {
            public string Subject_Activity_Name { get; set; }
        }

        List<Fetch_Details_of_report_head_ass> Show_of_report_head_ass = new List<Fetch_Details_of_report_head_ass>();
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        ...
        {
            string querys = "select Sequence_No,Session_Id,Branch_Id,Exam_Term_Id,Grade_System_Id,Assessment_Name,Class_id,Short_Name,Assessment_Id,Maximum_Marks,(select top 1 Maximum_Marks from Exam_Term_Details where Session_Id=Exam_Assessment_Details.Session_Id and Branch_Id=Exam_Assessment_Details.Branch_Id and Exam_Term_Id=Exam_Assessment_Details.Exam_Term_Id) as Term_Maximum_Marks from Exam_Assessment_Details where Session_Id=" + Session_id + " and  Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Istatus=1  and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc";
            DataTable dts = My.dataTable(querys);
            foreach (DataRow drs in dts.Rows)
            {
                string query = "select DISTINCT Subject_Activity_Name from Exam_Subject_Sub_Level where Session_Id='" + Session_id + "' and  Class_id='" + Class_id + "' and Istatus='1' and  Branch_Id='" + Branch_id + "' and Exam_Term_Id=" + drs["Exam_Term_Id"].ToString() + " and Assessment_Id='" + drs["Assessment_Id"].ToString() + "' and Grade_System_Id='" + drs["Grade_System_Id"].ToString() + "'";
                SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
                DataSet ds = new DataSet();
                ad.Fill(ds, "Complain_chat");
                DataTable dt = ds.Tables[0];
                ...
                else
                {
                    ...
                    {
                        Show_of_report_head_ass.Add(new Fetch_Details_of_report_head_ass
                        {
            ...
            JavaScriptSerializer js = new JavaScriptSerializer();
            Context.Response.Write(js.Serialize(Show_of_report_head_ass));
        }
        ...
        public void fetch_rp_card_subjects(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = MultipleTerm.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
        ...
        private List<MySubjectMarkDetails> findmyBookingProduct(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (i == 0)
                    {
                        string marks = MultipleTerm.get_marks(dt.Rows[i]["Session_id"].ToString(), dt.Rows[i]["Class_id"].ToString(), Admission_no, dt.Rows[i]["Term"].ToString(), dt.Rows[i]["Assessment"].ToString(), Subject_id, dt.Rows[i]["Branch_id"].ToString(), "Scholastic");
                        ...
                        string theory = arrs[5];


                        save_marks(dt.Rows[i]["Session_id"].ToString(), dt.Rows[i]["Class_id"].ToString(), Admission_no, dt.Rows[i]["Term"].ToString(), dt.Rows[i]["Assessment"].ToString(), Subject_id, dt.Rows[i]["Branch_id"].ToString(), obt_marks, full_marks, marks_type, total_no, "Practical");
                        ...
                        string markss = MultipleTerm.get_marks(dt.Rows[i]["Session_id"].ToString(), dt.Rows[i]["Class_id"].ToString(), Admission_no, dt.Rows[i]["Term"].ToString(), dt.Rows[i]["Assessment"].ToString(), Subject_id, dt.Rows[i]["Branch_id"].ToString(), "Scholastic");
                        ...
                        string theorys = arrss[5];

                        save_marks(dt.Rows[i]["Session_id"].ToString(), dt.Rows[i]["Class_id"].ToString(), Admission_no, dt.Rows[i]["Term"].ToString(), dt.Rows[i]["Assessment"].ToString(), Subject_id, dt.Rows[i]["Branch_id"].ToString(), theorys, full_markss, marks_types, total_nos, "Theory");
                        MySubjectMarkItem.Add(new MySubjectMarkDetails
                        {
                            //Marks = dr["Marks"].ToString(),
                            Marks = theorys,
        ...
        private void save_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string Branch_id, string marks, string full_marks, string marks_type, string total_no, string marksType)
        {
            try
            {
            ...
            if (mycode.IsUserExist("select Id from Exam_temp_assesment_total_no where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Marks_type='" + marksType + "'"))
            {


                SqlCommand cmd;
                string query = "INSERT INTO Exam_temp_assesment_total_no (Session_id,Branch_id,Class_id,Term_id,Assessment_id,Subject_id,Admission_no,Marks,Full_marks,Marks_type) values (@Session_id,@Branch_id,@Class_id,@Term_id,@Assessment_id,@Subject_id,@Admission_no,@Marks,@Full_marks,@Marks_type)";
                ...
                cmd.Parameters.AddWithValue("@Marks_type", marksType);
            ...
            {
                SqlCommand cmd;
                string query = "Update Exam_temp_assesment_total_no set Marks=@Marks,Full_marks=@Full_marks where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Marks_type='" + marksType + "'";
                cmd = new SqlCommand(query);
                if (marks_type == "Marks")
                {
                    cmd.Parameters.AddWithValue("@Marks", marks);
        ...
        public void fetch_rp_card_signature(string Session_id, string Class_id, string Admission_no, string Branch_id)
        {
            ...
            {
                string Section = Examination.get_student_section(Session_id, Admission_no, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string stgnature = "";
        ...
        {
            string query = "select ptt.*,pt.Activity_name from Exam_Personality_Traits_Term_Wise ptt join Exam_Personality_Traits pt on ptt.Activity_Id=pt.Activity_Id where ptt.Session_id=" + Session_id + " and ptt.Branch_id='" + Branch_id + "' and ptt.Class_id=" + Class_id + "   and ptt.Admission_no='" + Admission_no + "'  order by pt.Position asc";
        ...
        {
            string Total_marks = MultipleTerm.get_overall_total_marks(Session_id, Class_id, Admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);

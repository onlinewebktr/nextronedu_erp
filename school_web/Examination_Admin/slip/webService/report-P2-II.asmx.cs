...
{
    /// <summary>
    /// Summary description for report_P2_II
    /// </summary>
    ...
    public class report_P2_II : System.Web.Services.WebService
    {
        My mycode = new My();

        //======================================
        //=======================================================
        public class MySubjectHeadAhoW
        {
        ...
        List<MySubjectHeadAhoW> ShowMySubjectHeadAhoW = new List<MySubjectHeadAhoW>();
        [WebMethod]
        ...
        public void fetch_report_card_assesment(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    ShowMySubjectHeadAhoW.Add(new MySubjectHeadAhoW
                    {
                ...
                Context.Response.Write(js.Serialize(ShowMySubjectHeadAhoW));
            }
        }
        ...
        //==============================================================
        public class Fetch_Details_of_sub_sub_level
        {
            public string Subject_Activity_Name { get; set; }
        }

        List<Fetch_Details_of_sub_sub_level> Show_of_report_sub_sub_level = new List<Fetch_Details_of_sub_sub_level>();
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void fetch_report_card_sub_level(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            {
                string query = "select DISTINCT Subject_Activity_Name from Exam_Subject_Sub_Level where Session_Id=" + Session_id + " and Branch_Id='" + Branch_id + "' and Istatus=1 and Exam_Term_Id=" + Term_id + " and Grade_System_Id=" + drs["Grade_System_Id"].ToString() + " and Assessment_Id=" + drs["Assessment_Id"].ToString() + " and Class_id=" + Class_id + "";
                SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
                DataSet ds = new DataSet();
                ad.Fill(ds, "Complain_chat");
                DataTable dt = ds.Tables[0];
                ...
                else
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        Show_of_report_sub_sub_level.Add(new Fetch_Details_of_sub_sub_level
                        {
            ...
            JavaScriptSerializer js = new JavaScriptSerializer();
            Context.Response.Write(js.Serialize(Show_of_report_sub_sub_level));
        }


        //==============================================================MAXMARKS Of Subject Sub Level
        public class Fetch_max_mark_subj_sub_level
        {
            public string MaxMark { get; set; }
        }

        List<Fetch_max_mark_subj_sub_level> Show_max_mark_subj_sub_level = new List<Fetch_max_mark_subj_sub_level>();
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void fetch_report_card_max_mark_sub_level(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            string querys = "select Session_Id,Branch_Id,Exam_Term_Id,Assessment_Name,Grade_System_Id,Class_id,Short_Name,Assessment_Id,Maximum_Marks,(select top 1 Maximum_Marks from Exam_Term_Details where Session_Id=Exam_Assessment_Details.Session_Id and Branch_Id=Exam_Assessment_Details.Branch_Id and Exam_Term_Id=Exam_Assessment_Details.Exam_Term_Id) as Term_Maximum_Marks from Exam_Assessment_Details where Session_Id=" + Session_id + " and  Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Exam_Term_Id=" + Term_id + "  and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc";
            DataTable dts = My.dataTable(querys);
            foreach (DataRow drs in dts.Rows)
            {
                string rows_of_sub_level = get_rows_of_sub_level(drs["Session_Id"].ToString(), drs["Branch_Id"].ToString(), drs["Exam_Term_Id"].ToString(), drs["Grade_System_Id"].ToString(), drs["Assessment_Id"].ToString(), drs["Class_id"].ToString());


                string maxMark = "";
                string query = "select DISTINCT Maximum_Marks from Exam_Subject_Sub_Level where Session_Id=" + Session_id + " and Branch_Id='" + Branch_id + "' and Istatus=1 and Exam_Term_Id=" + Term_id + " and Grade_System_Id=" + drs["Grade_System_Id"].ToString() + " and Assessment_Id=" + drs["Assessment_Id"].ToString() + " and Class_id=" + Class_id + "";
                SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
                DataSet ds = new DataSet();
                ad.Fill(ds, "Complain_chat");
                DataTable dt = ds.Tables[0];
                ...
                else
                {
                    if (rowcount1 == 1)
                    {
                        ...
                        {
                            maxMark = dt.Rows[i]["Maximum_Marks"].ToString();
                        }
                    }
                    else
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            maxMark = maxMark + dt.Rows[i]["Maximum_Marks"].ToString() + "/";
                        }
                        maxMark = maxMark.Remove(maxMark.Length - 1);
                    }


                    for (int i = 0; i < My.toIntS(rows_of_sub_level); i++)
                    {
                        Show_max_mark_subj_sub_level.Add(new Fetch_max_mark_subj_sub_level
                        {
                            MaxMark = maxMark,
                        });
                    }
            ...
            JavaScriptSerializer js = new JavaScriptSerializer();
            Context.Response.Write(js.Serialize(Show_max_mark_subj_sub_level));
        }



        //==============================================================MAXMARKS Of ABCD...
        public class Fetch_abcd
        {
            public string Alphabate { get; set; }
        }

        List<Fetch_abcd> Show_abcd = new List<Fetch_abcd>();
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public void fetch_report_card_abcd(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            double ttl_no_of_rows = 0;
            string querys = "select Session_Id,Branch_Id,Exam_Term_Id,Assessment_Name,Grade_System_Id,Class_id,Short_Name,Assessment_Id,Maximum_Marks,(select top 1 Maximum_Marks from Exam_Term_Details where Session_Id=Exam_Assessment_Details.Session_Id and Branch_Id=Exam_Assessment_Details.Branch_Id and Exam_Term_Id=Exam_Assessment_Details.Exam_Term_Id) as Term_Maximum_Marks from Exam_Assessment_Details where Session_Id=" + Session_id + " and  Class_id='" + Class_id + "' and Branch_Id='" + Branch_id + "' and Exam_Term_Id=" + Term_id + "  and Scholastic_Co_scholastic='Scholastic' order by Sequence_No asc";
            DataTable dts = My.dataTable(querys);
            foreach (DataRow drs in dts.Rows)
            {
                string rows_of_sub_level = get_rows_of_sub_level(drs["Session_Id"].ToString(), drs["Branch_Id"].ToString(), drs["Exam_Term_Id"].ToString(), drs["Grade_System_Id"].ToString(), drs["Assessment_Id"].ToString(), drs["Class_id"].ToString());
                ttl_no_of_rows = ttl_no_of_rows + My.toDouble(rows_of_sub_level);
            }

            for (int i = 0; i < My.toIntS(ttl_no_of_rows); i++)
            {
                string charater = "";
                if (i == 0)
                {
            ...
            string lastCharater = "";
            int il = My.toIntS(ttl_no_of_rows);
            if (il == 1)
            {
                lastCharater = "A";
            }
            ...
            Show_abcd.Add(new Fetch_abcd
            {
                Alphabate = "(A to " + lastCharater + ")",
            });

            JavaScriptSerializer js = new JavaScriptSerializer();
            Context.Response.Write(js.Serialize(Show_abcd));
        }
        ...
        public void fetch_rp_card_subjects(string Session_id, string Class_id, string Admission_no, string Branch_id, string Term_id)
        {
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string Total_marks = Examination_P3.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
        ...
        private List<MySubjectMarkDetails> findmyBookingProduct(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id)
        {
            ...
            foreach (DataRow drs in dts.Rows)
            {
                ...
                {
                    string subject_name = Examination_P3.get_subject_name(Subject_id, Class_id, Branch_id);
                    foreach (DataRow dr in dt.Rows)
                    {
                        string marks = Examination_P3.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic", dr["Subject_Sub_Level_Id"].ToString());
                        ...
                        save_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), obt_marks, full_marks, marks_type, total_no, dr["Subject_Sub_Level_Id"].ToString());
        ...
        private void save_marks(string session_id, string class_id, string admission_no, string term_id, string assesment_id, string subject_id, string Branch_id, string marks, string full_marks, string marks_type, string total_no, string subject_sub_level)
        {
            try
            {
            ...
            {
                if (mycode.IsUserExist("select Id from Exam_temp_assesment_total_no where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Subject_sub_level_id=" + subject_sub_level + ""))
                {
                    SqlCommand cmd;
                    ...
                    cmd.Parameters.AddWithValue("@Subject_sub_level_id", subject_sub_level);
                ...
                {
                    SqlCommand cmd;
                    string query = "Update Exam_temp_assesment_total_no set Marks=@Marks,Full_marks=@Full_marks where Session_id='" + session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + class_id + "' and Term_id='" + term_id + "' and Assessment_id='" + assesment_id + "' and Subject_id='" + subject_id + "' and Admission_no='" + admission_no + "' and Subject_sub_level_id=" + subject_sub_level + "";
                    cmd = new SqlCommand(query);
                    if (marks_type == "Marks")
                    {
                        cmd.Parameters.AddWithValue("@Marks", marks);
        ...
        public void fetch_rp_card_signature(string Session_id, string Class_id, string Admission_no, string Branch_id)
        {
            ...
            {
                string Section = Examination_P3.get_student_section(Session_id, Admission_no, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string stgnature = "";
                    if (dr["Signature_type"].ToString() == "1")
                    {
                        stgnature = Examination_P3.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = Examination_P3.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = Examination_P3.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        { 
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = Examination_P3.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic", "0");
        ...
        {
            string section = Examination_P3.get_section(Session_id, Class_id, Admission_no, Branch_id);
            ...
            {
                string system_grade_id = Examination_P3.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    if (dr["Rank"].ToString() == "1")
                    ...
                    string grade = Examination_P3.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        {
            string Total_marks = Examination_P3.get_overall_total_marks(Session_id, Class_id, Admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
            string[] arrs = Total_marks.Split(stringSeparatorss, StringSplitOptions.None);
            ...
            string percent_remark = Examination_P3.get_percent_remark(overall_percent);

...
namespace school_web.Examination_Admin.slip.saws.api
{
    /// <summary>
    /// Summary description for reportCard
    /// </summary>
    ...
    public class reportCard : System.Web.Services.WebService
    {
        My mycode = new My();
        ...
        public void fetch_rp_card_bulks(string Session_id, string Class_id, string Section, string Branch_id, string Term_id, string Adm_no)
        {
            ...
            if (rowcount1 > 0)
            {
                string termSequence = My.get_single_column_data("select Sequence_No as Column_Name from Exam_Term_Details where Session_Id='" + Session_id + "' and Istatus=1 and Class_id='" + Class_id + "' and Exam_Term_Id='" + Term_id + "'");
                string termOneId = "0";
                if (termSequence == "2")
                {
                    termOneId = My.get_single_column_data("select top 1 Exam_Term_Id as Column_Name from Exam_Term_Details where Session_Id='" + Session_id + "' and Istatus=1 and Class_id='" + Class_id + "' order by Sequence_No asc");
                }
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    List<MySubjectMarkShowII> MBdetails = findSubjectMarkShowII(dr["Admission_no"].ToString(), Session_id, Class_id, Branch_id, Term_id, dtF.Rows[0]["firm_id"].ToString(), termSequence, termOneId);

                    List<Fetch_Details_of_report_head> MBdetailsHeading = findSubjectHeadings(Session_id, Class_id, Branch_id, Term_id, termSequence);
                    ...
                    string signatures = rpCardSawS.get_signature(Session_id, Class_id, Section, Branch_id);
                    ...
                    string term_name = rpCardSawS.get_term_name(Branch_id, Term_id);
                    string rank = "0";
                    string ranksup = "";
                    if (dr["Rank"].ToString() == "1" || dr["Rank"].ToString() == "21" || dr["Rank"].ToString() == "31" || dr["Rank"].ToString() == "41" || dr["Rank"].ToString() == "51" || dr["Rank"].ToString() == "61" || dr["Rank"].ToString() == "71")
                    {
                        rank = dr["Rank"].ToString();
                        ranksup = "st";
                    }
                    else if (dr["Rank"].ToString() == "2" || dr["Rank"].ToString() == "22" || dr["Rank"].ToString() == "32" || dr["Rank"].ToString() == "42" || dr["Rank"].ToString() == "52" || dr["Rank"].ToString() == "62" || dr["Rank"].ToString() == "72")
                    {
                        rank = dr["Rank"].ToString();
                        ranksup = "nd";
                    }
                    else if (dr["Rank"].ToString() == "3" || dr["Rank"].ToString() == "23" || dr["Rank"].ToString() == "33" || dr["Rank"].ToString() == "43" || dr["Rank"].ToString() == "53" || dr["Rank"].ToString() == "63" || dr["Rank"].ToString() == "73")
                    {
                        rank = dr["Rank"].ToString();
                        ranksup = "rd";
                    }
        ...
        private List<Fetch_Details_of_overall_no> findOverallNoDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id, string studentname, string class_name, string rollnumber)
        {
            ...
            string Total_marks = rpCardSawS.get_overall_total_marks(Session_id, Class_id, admission_no, Branch_id, Term_id);
            string[] stringSeparatorss = new string[] { "/" };
        ...
        private List<Fetch_Details_of_graph> findGraphDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            string tableName = "Exam_temp_assesment_total_no";
        ...
        private List<Fetch_Details_of_Rank> findRankDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section, string Term_id)
        {
            ...
            if (rowcount1 > 0)
            { 
                string system_grade_id = rpCardSawS.get_system_grade_id_final(Session_id, Branch_id, Class_id);
                foreach (DataRow dr in dt.Rows)
                {
                    string rank = "0";
                    ...
                    string grade = rpCardSawS.get_grade_final(Session_id, Branch_id, system_grade_id, My.toDouble(dr["Mark_percentage"].ToString()));
        ...
        private List<Fetch_Details_of_Signature> findSigDetails(string Session_id, string admission_no, string Class_id, string Branch_id, string Section)
        {
            ...
            { 
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        stgnature = rpCardSawS.get_class_teacher_signature(Session_id, Class_id, Section, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "2")
                    {
                        stgnature = rpCardSawS.get_principal_signature(Session_id, Branch_id);
                    }
                    if (dr["Signature_type"].ToString() == "3")
                    {
                        stgnature = rpCardSawS.get_examinee_or_parent_signature(Session_id, Branch_id, dr["Signature_type_e_p"].ToString());
                    }
        ...
        //=================================HEADING
        private List<Fetch_Details_of_report_head> findSubjectHeadings(string Session_id, string Class_id, string Branch_id, string Term_id, string termSequence)
        {
            ...
            if (rowcount1 > 0)
            {
                if (termSequence == "2")
                {
                    ...
                    {
                        Assessment_Name = "PT-1",
                        Short_Name = "PT-1",
                        Maximum_Marks = "10",
                        Term_Maximum_Marks = "10",
                    });
                }
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    MySubjectHeading.Add(new Fetch_Details_of_report_head
                    {
                        ...
                        Maximum_Marks = dt.Rows[i]["Maximum_Marks"].ToString(), //dt.Rows[i]["Maximum_Marks"].ToString(),
        ...
        private List<Fetch_Details_of_report_CoScholastic> findCoScholasticData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            { 
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = rpCardSawS.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Assessment"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Co-Scholastic");
        ...
        private List<Fetch_Details_of_report_descpline> findDescplineData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            { 
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    if (isNum == true)
                    {
                        grade = rpCardSawS.get_personality_grade(Session_id, Class_id, Branch_id, dr["Term_grade"].ToString());
                    }
                    else
                    {
        ...
        private List<Fetch_Details_of_report_grade_remark> findGradeRemarkData(string Session_id, string Admission_no, string Class_id, string Branch_id, string Term_id)
        {
            ...
            string sys_grade_id = rpCardSawS.get_co_scholastic_system_grade_id(Session_id, Class_id, Admission_no, Branch_id, Term_id);
        ...
        private List<MySubjectMarkShowII> findSubjectMarkShowII(string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string firm_id, string termSequence, string termOneId)
        {
            ...
            if (rowcount1 > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    List<MySubjectMarkDetailsIII> MBdetailsIII = findmyMarksIII(dr["Subject"].ToString(), Admission_no, Session_id, Class_id, Branch_id, Term_id, termSequence, termOneId);
                    string Total_marks = rpCardSawS.get_subject_total_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Admission_no"].ToString(), dr["Term"].ToString(), dr["Subject"].ToString(), dr["Branch_id"].ToString(), "Scholastic");
                    ...
                    string percent_remark = rpCardSawS.get_percent_remark(overall_percent);
                    //QQQQQRRRR
                    string url = "";
                    string markType = "";
        ...
        private List<MySubjectMarkDetailsIII> findmyMarksIII(string Subject_id, string Admission_no, string Session_id, string Class_id, string Branch_id, string Term_id, string termSequence, string termOneId)
        {
            ...
            if (rowcount1 > 0)
            {
                if (termSequence == "2")
                {
                    update_term_one_marks(Session_id, Class_id, Branch_id, termOneId, Admission_no, Subject_id, termSequence);
                    DataTable dtT1 = My.dataTable("select isnull(sum(convert(float, Marks)),0) as term1ObtMark from Exam_temp_assesment_total_no where Session_id='" + Session_id + "' and Branch_id='" + Branch_id + "' and Class_id='" + Class_id + "' and Term_id='" + termOneId + "' and Subject_id='" + Subject_id + "' and Admission_no='" + Admission_no + "'");
                    if (dtT1.Rows.Count > 0)
                    {
                        double obtMark = (My.toDouble(dtT1.Rows[0]["term1ObtMark"].ToString()) / 3);
                        save_marks(Session_id, Class_id, Admission_no, Term_id, "9001", Subject_id, Branch_id, obtMark.ToString("0.0"), "10", "Marks", "10");
                        string obt_marks = obtMark.ToString("0.0");
                        try
                        {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = rpCardSawS.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), Admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), Subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };
                    string[] arrs = marks.Split(stringSeparatorss, StringSplitOptions.None);
                    string obt_marks = arrs[0];
                    string full_marks = arrs[1];
        ...
        private void update_term_one_marks(string session_id, string class_id, string branch_id, string Term_id, string admission_no, string subject_id, string termSequence)
        {
            string query = "select DISTINCT t1.Session_id,t1.Class_id,t1.Exam_Term_Id as Term,t1.Assessment_Id as Assessment,t1.Branch_Id,t1.Assessment_Name,t1.Sequence_No from Exam_Assessment_Details t1 where t1.Session_Id=" + session_id + " and t1.Branch_id='" + branch_id + "' and t1.Class_id=" + class_id + " and t1.Exam_Term_Id=" + Term_id + " and t1.Scholastic_Co_scholastic='Scholastic' and t1.Istatus=1 order by t1.Sequence_No asc";
            SqlDataAdapter ad = new SqlDataAdapter(query, My.con);
            DataSet ds = new DataSet();
            ...
            if (rowcount1 > 0)
            {
                My.exeSql("delete from Exam_temp_assesment_total_no where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Branch_Id='" + branch_id + "' and Term_id='" + Term_id + "' and Admission_no='" + admission_no + "' and Subject_id='" + subject_id + "'");
                foreach (DataRow dr in dt.Rows)
                {
                    string marks = rpCardSawS.get_marks(dr["Session_id"].ToString(), dr["Class_id"].ToString(), admission_no, dr["Term"].ToString(), dr["Assessment"].ToString(), subject_id, dr["Branch_id"].ToString(), "Scholastic");
                    string[] stringSeparatorss = new string[] { "/" };

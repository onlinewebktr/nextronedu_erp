        ...
        internal static string get_subject_total_marks(string session_id, string class_id, string admission_no, string term_id, string subject_id, string branch_id, string Type)
        {
            ...
            if (Calculation_Type == "Sum")
            {
                ...
                else
                {
                    ///=========================IS  NOT ADVANCE
                    ///
                    string query = "select eta.Marks,eta.Full_marks from " + table_name + " eta where  eta.Session_Id='" + session_id + "' and eta.Branch_Id='" + branch_id + "' and eta.Term_id=" + term_id + " and eta.Class_id=" + class_id + "  and eta.Subject_id=" + subject_id + " and eta.Admission_no='" + admission_no + "' ";
                    DataTable dt = FillDatastatic(query);
                    if (dt.Rows.Count == 0)
                    {
                        total_number = 0;
            ...
            if (Type == "Scholastic")
            {
                if (Output == "Marks")
                {
                    ...
                    string grade = sawsExamSetting.get_grade_of_a_subject_with_bg(session_id, branch_id, Grade_System_Id, final_marks, full_number, class_id);   // Grade of a subject
                    ...
                    string exam_settings = sawsExamSetting.get_exam_settings(session_id, branch_id, class_id);  // Settings
                ...
                else
                {
                    ...
                    string exam_settings = sawsExamSetting.get_exam_settings(session_id, branch_id, class_id);  // Is SpecialNote & QR Show

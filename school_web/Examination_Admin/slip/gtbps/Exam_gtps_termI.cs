...
namespace school_web.Examination_Admin.slip.gtbps
{
    public class Exam_gtps_termI
    {
        ...
        internal static string get_marks(string Session_id, string Class_id, string Admission_no, string Term_id, string Assessment_id, string Subject_id, string Branch_id, string Subject_type)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            {
                                IsChar = dt.Rows[i]["Marks"].ToString();
                                if (dt.Rows[i]["Marks"].ToString() == "")
                                {
                                }
                                else
                                {
            ...
            assesment_weightage = max_number.ToString();
        ...
        internal static string get_extra_marks(string Session_id, string Class_id, string Branch_id, string Term_id, String assesment_id, string maxmarkS, string Admission_no)
        {
            string extra_marks = "";
            try
            {
                DataTable dtS = FillDatastatic("select DISTINCT t2.Maximum_Marks from Exam_without_weightage_subject t1 join Exam_Subject_Sub_Level t2 on t1.Class_id=t2.Class_id and t1.Branch_id=t2.Branch_id and t1.Subject_id=t2.Subject_id join Exam_marks t3 on t1.Class_id=t3.Class_id and t1.Branch_id=t3.Branch_id and t1.Subject_id=t3.Subject and t2.Exam_Term_Id=t3.Term and t2.Session_id=t3.Session_id and t2.Assessment_Id=t3.Assessment where t1.Class_id='" + Class_id + "' and t1.Branch_id='" + Branch_id + "' and t2.Session_Id='" + Session_id + "' and t2.Exam_Term_Id='" + Term_id + "' and t2.Assessment_Id='" + assesment_id + "' and t2.Class_id='" + Class_id + "' and t2.Branch_id='" + Branch_id + "' and t3.Admission_no='" + Admission_no + "'");
                if (dtS.Rows.Count > 0)
                {
                    for (int i = 0; i < dtS.Rows.Count; i++)
                    {
                        if (maxmarkS == dtS.Rows[i]["Maximum_Marks"].ToString())
                        {
                        }
                        else
                        {
                            extra_marks = extra_marks + dtS.Rows[i]["Maximum_Marks"].ToString() + "/";
                        }
                    }
                }

                extra_marks = extra_marks.Remove(extra_marks.Length - 1);
                return extra_marks;
            }
            catch (Exception ex)
            {

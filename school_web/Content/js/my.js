...
function runSQLapi(apicode,result) {
    ...
    gowithData('../../hr/data/api', 'Post', data, function (resp) { 
        result(resp);
    })
}
function runSQLapi(apicode,aditional_data,result) {
    ...
    gowithData('../../hr/data/api', 'Post', JSON.stringify({ data: mdata }), function (resp) {
        result(resp);
    })
}
...
function fileuploadWithCrop(element, area) {
    ...
    var cropper;
    image.src = "../../Content/Images/loading.gif";
    $("#mdl-crop").modal("show");
...
function loadmenu (menu_id,id,resp) {
    //console.log('../../../getmenu/' + id);
    var menu = $(menu_id);
    $.ajax({
        url: '../../../hr/getmenu/' + id,
...
function bind_ddl(ddl_source,ddl_id,fiterId) {
    ...
    var data = { data : data}
    gowithData("../../hr/data/onchange", 'POST', JSON.stringify(data), function (resp) {
        console.log(resp);
        var ddl = $('#'+ddl_id);
        $("#" + ddl_id + " option:not(:first)").remove();
        var dt = resp.data;

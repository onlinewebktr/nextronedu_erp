...
{
    public partial class PayProcess_blank : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["Regid"] != null)
                {
                    ...
                    try
                    {
                        ...
                        ViewState["razorpay_payment_id"] = Request.QueryString["razorpay_payment_id"].ToString();
                        ViewState["razorpay_order_id"] = Request.QueryString["razorpay_order_id"].ToString();
                        ViewState["razorpay_signature"] = Request.QueryString["razorpay_signature"].ToString();
                        ...
                        if (status == "captured")
                        {
                            ...
                            My.exeSql("update Payment_transaction_process set razorpay_payment_id='" + ViewState["razorpay_payment_id"].ToString() + "',razorpay_signature='" + ViewState["razorpay_signature"].ToString() + "',status='" + ViewState["statuS"].ToString() + "' where  razorpay_order_id='" + ViewState["razorpay_order_id"].ToString() + "'");
                        }
                    ...
                    {
                        Response.Redirect("Payment_Error_Message.aspx?orderid=" + ViewState["orderid"].ToString() + "&Regid=" + ViewState["Regid"].ToString(), false);
                    }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ViewState["statuS"].ToString() == "Paid")
                {
                    ...
                    bool status = mypayment.save_final_payment(ViewState["Regid"].ToString(), ViewState["orderid"].ToString());
                    if (status == true)
                    {

                        Response.Redirect("Payment_Success_Message.aspx?orderid=" + ViewState["orderid"].ToString() + "&Regid=" + ViewState["Regid"].ToString(), false);
                    }
                    else
                    {
                        Response.Redirect("Payment_Error_Message.aspx?orderid=" + ViewState["orderid"].ToString() + "&Regid=" + ViewState["Regid"].ToString(), false);

...
{
    public partial class View_Sent_Message_to_Teacher : System.Web.UI.Page
    {
        My mycode = new My();
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        if (Session["querymessage_Teacher"] == null)
                        {
                            find_data();
                        }
                        else
                        {
                            BindRepeater(Session["querymessage_Teacher"].ToString());
                        }
            ...
            {
                My.submitException(ex, "View_Sent_Message_to_Teacher");
            }
        }
        ...
        protected void lnkEdit_Click(object sender, EventArgs e)
        {
            if (ViewState["Is_Edit"].ToString() == "1")
            {
                ...
                Response.Redirect("Send_Message_to_Teacher.aspx?id=" + lbl_Id.Text, false);
            }
            else
            {
        ...
        protected void lnkDel_Click(object sender, EventArgs e)
        {
            try
            {
                if (ViewState["Is_Edit"].ToString() == "1")
                {
                    ...
                    GridViewRow row = (GridViewRow)lnk.Parent.Parent;
                    Label lbl_Id = (Label)row.FindControl("lbl_Id");
                    mycode.executequery("delete from Private_Messages_For_Teacher where Id=" + lbl_Id.Text + "");
        ...
        private void BindRepeater(string query)
        {
            ...
            Session["querymessage_Teacher"] = query;
                ...
                else
                {
                    if (ddl_teacher.SelectedItem.Text == "ALL")
                    {
                        ...
                        if (Convert.ToInt32(code.ConvertStringToiDate(txt_date.Text)) <= Convert.ToInt32(code.ConvertStringToiDate(txt_enddate.Text)))
                        {
                            ...
                            BindRepeater("select  * from Private_Messages_For_Teacher where  Idate>=" + Convert.ToInt32(code.ConvertStringToiDate(txt_date.Text)) + " and Idate<=" + Convert.ToInt32(code.ConvertStringToiDate(txt_enddate.Text)) + "  order by  Idate Desc");
                    ...
                    {
                        BindRepeater("select  * from Private_Messages_For_Teacher where  Idate>=" + Convert.ToInt32(code.ConvertStringToiDate(txt_date.Text)) + " and Idate<=" + Convert.ToInt32(code.ConvertStringToiDate(txt_enddate.Text)) + " and Teacher_Id='" + ddl_teacher.SelectedValue + "' order by  Idate Desc");
                    }
                }
            }
            catch

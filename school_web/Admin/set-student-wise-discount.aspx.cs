...
{
    public partial class set_student_wise_discount : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        private void for_discount(string session_name, string session_id, string class_id, string admission_no, string hostel_taken, string transport_taken, string hostel_id, string room_category_id, string TransportPath_id, string Boarding_Point_id)
        {
            ...
            else
            {
                if (transport_taken.ToUpper() == "YES")
                {
                    ...
                    {
                        query1 = "select fmc.content,fmc.content_id,fmc.amount,cm.group_id,'0' as discount from dbo.[Fee_master_content_wise] fmc join Content_master cm on fmc.content_id=cm.content_id  where fmc.class_id='" + class_id + "' and fmc.session_id='" + session_id + "' and fmc.parameter_id='44' and fmc.Month='" + startmonthname + "' UNION all select t1.Parameter as content,parameter_id as content_id,Amount as amount,'0' as group_id,'0' as discount from Transportation_Fee_Master t1 join Month_Index t2 on t1.Month_id=t2.Month_Id where t1.session_id='" + session_id + "' and t1.Transportation_path_id='" + TransportPath_id + "'and t1.Boarding_Point_id='" + Boarding_Point_id + "' and t1.Month='" + startmonthname + "'";
                    }
                    else
                    {
                ...
                else
                {
                    if (ViewState["parameteridS"].ToString() == "44")
                    {
                        query1 = "select fmc.content,fmc.content_id,fmc.amount,cm.group_id,'0' as discount from dbo.[Fee_master_content_wise] fmc join Content_master cm on fmc.content_id=cm.content_id  where fmc.class_id='" + class_id + "' and fmc.session_id='" + session_id + "' and fmc.parameter_id='44' and fmc.Month='" + startmonthname + "'";
                    }
                    else
                    {

...
{
    public partial class Create_Sports_Certificate : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        protected void btn_create_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    SqlDataAdapter ad = new SqlDataAdapter("select * from Certificate_Master where Session_id=" + ViewState["sessioN"].ToString() + " and Class_id='" + ViewState["class_id"].ToString() + "' and Admission_no='" + ViewState["adm_no"].ToString() + "' and Certificate_type='Sport' and Participated_in='"+ txt_Participated_in_the.Text.Trim() + "'", My.conn);
                    DataSet ds = new DataSet();
                    ad.Fill(ds, "Certificate_Master");
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count == 0)
                    {
                        string query = "INSERT INTO Certificate_Master (Participated_in,Division,Remarks,Session_id,Class_id,Admission_no,Certificate_type,Create_date,Create_idate,User_id,Firm_id,Issue_date,Certificate_no) values (@Participated_in,@Division,@Remarks,@Session_id,@Class_id,@Admission_no,@Certificate_type,@Create_date,@Create_idate,@User_id,@Firm_id,@Issue_date,@Certificate_no)";
                        ...
                        cmd.Parameters.AddWithValue("@Certificate_type", "Sport");
                        ...
                        cmd.Parameters.AddWithValue("@Firm_id", Session["firm"].ToString());
                        cmd.Parameters.AddWithValue("@Issue_date", txt_issue_date.Text);
                        cmd.Parameters.AddWithValue("@Certificate_no", certificate_no);
                        if (My.InsertUpdateData(cmd))
                        {
                            Alertme("Sports certificate has been created successfully.", "success");
        ...
        [WebMethod]
        public static List<string> GetParticipated(string PathRooT)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select distinct Participate_Name from Sports_Participate_Master where Participate_Name LIKE '%'+@SearchName+'%'  order by Participate_Name";
                    cmd.Connection = con;
                    con.Open();
                    cmd.Parameters.AddWithValue("@SearchName", PathRooT);
                    SqlDataReader dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
                        MobResult.Add(dr["Participate_Name"].ToString());
                    }
                    con.Close();
                    return MobResult;
                }
        ...
        [WebMethod]
        public static List<string> GetDivision(string PathRooT)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select distinct Division_name from Sports_Division_Master where Division_name LIKE '%'+@SearchName+'%'  order by Division_name ";
                    cmd.Connection = con;
                    con.Open();
                    cmd.Parameters.AddWithValue("@SearchName", PathRooT);
                    SqlDataReader dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
                        MobResult.Add(dr["Division_name"].ToString());
                    }
                    con.Close();
                    return MobResult;
                }
        ...
        [WebMethod]
        public static List<string> GetRemarks(string PathRooT)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select distinct Remark_name from Sports_Remark_Master where Remark_name LIKE '%'+@SearchName+'%' order by Remark_name asc  ";
                    cmd.Connection = con;
                    con.Open();
                    cmd.Parameters.AddWithValue("@SearchName", PathRooT);
                    SqlDataReader dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
                        MobResult.Add(dr["Remark_name"].ToString());
                    }
                    con.Close();
                    return MobResult;
                }

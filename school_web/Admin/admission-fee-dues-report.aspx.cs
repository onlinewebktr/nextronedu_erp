...
{
    public partial class admission_fee_dues_report : System.Web.UI.Page
    {
        My mycode = new My();
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        #region sms
                        Dictionary<string, object> autosms = mycode.get_auto_message_template("Admission Dues");
        ...
        private void find_data()
        {
            ...
            {
                bind_grd_view("select * from admission_registor t1 join Add_course_table t2 on t1.Class_id=t2.course_id where t1.Session_Id='" + ddlsession.SelectedValue + "'  and Transfer_Status='New' and t1.Status=1 order by t2.Position,t1.rollnumber asc");
            }
            else if (ddlclass.SelectedItem.Text != "ALL" && ddl_section.SelectedItem.Text == "ALL")
            {
                bind_grd_view("select * from admission_registor t1 join Add_course_table t2 on t1.Class_id=t2.course_id where t1.Session_Id='" + ddlsession.SelectedValue + "' and t1.Class_id='" + ddlclass.SelectedValue + "'  and Transfer_Status='New' and t1.Status=1 order by t2.Position,t1.rollnumber asc");
            }
            else
            {
                bind_grd_view("select * from admission_registor t1 join Add_course_table t2 on t1.Class_id=t2.course_id where t1.Session_Id='" + ddlsession.SelectedValue + "' and t1.Class_id='" + ddlclass.SelectedValue + "'  and Transfer_Status='New' and t1.Status=1 and t1.Section='" + ddl_section.Text + "' order by t2.Position,t1.rollnumber asc");
            }
        }
        ...
        private void bind_grd_view(string qry)
        {
            ...
            else
            {
                ...
                {
                    dr["ActualDues"] = find_admission_dues(dr);
                }
        ...
        private object find_admission_dues(DataRow dr)
        {
        ...
        private void find_std_details(DataRow drx)
        {
            SqlDataAdapter ad_contactus = new SqlDataAdapter("select * from admission_registor where admissionserialnumber='" + drx["admissionserialnumber"].ToString() + "' and Session='" + drx["session"].ToString() + "'  and Status='1' order by id asc", My.conn);
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    ViewState["parameter_id"] = dr["hosteltaken"].ToString().ToUpper() == "YES" ? "5" : "1";
                    Dictionary<string, object> dc1 = mycode.Bind_hostel_data_for_assined_student(ViewState["sessionIDs"].ToString(), ViewState["class_id"].ToString(), drx["admissionserialnumber"].ToString());
        ...
        public static List<string> GetRooPath(string PathRooT)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                ...
                {
                    cmd.CommandText = "select distinct studentname from admission_registor where studentname LIKE '%'+@SearchMobNo+'%'";
                    cmd.Connection = con;
                    con.Open();
        ...
        [WebMethod]
        public static List<object> GetChartData(string Session, string Class, string Stunt_name, string Flags)
        {
            string sections = get_sections(Session, Class, Stunt_name, Flags);
            string query = "";
            if (Flags == "0")
            {
                if (Class == "0")
                {
                    query = "Select DISTINCT ar.Session_id,ar.Class_id,ad.Course_Name as Class,ad.Position,sm.Status,'Section '+sm.Status as Section,0 as Total from Chart_status_master sm CROSS JOIN admission_registor ar join Add_course_table ad on ar.Class_id=ad.course_id join Student_Payment_History t1 on t1.Addmission_no=ar.admissionserialnumber and t1.Class_id=ar.Class_id and t1.Session=ar.Session where t1.Adjust_type is null and ar.Session_id='" + Session + "' and  sm.Status in (" + sections + ") order by ad.Position asc";
                }
                else
                {
                    query = "Select DISTINCT ar.Session_id,ar.Class_id,ad.Course_Name as Class,ad.Position,sm.Status,'Section '+sm.Status as Section,0 as Total from Chart_status_master sm CROSS JOIN admission_registor ar join Add_course_table ad on ar.Class_id=ad.course_id join Student_Payment_History t1 on t1.Addmission_no=ar.admissionserialnumber and t1.Class_id=ar.Class_id and t1.Session=ar.Session where t1.Adjust_type is null and ar.Class_id='" + Class + "' and ar.Session_id='" + Session + "' and sm.Status in (" + sections + ") order by ad.Position asc";
                }
            }
            else
            {
                query = "Select DISTINCT ar.Session_id,ar.Class_id,ad.Course_Name as Class,ad.Position,sm.Status,'Section '+sm.Status as Section,0 as Total from Chart_status_master sm CROSS JOIN admission_registor ar join Add_course_table ad on ar.Class_id=ad.course_id where ar.Session_id='" + Session + "' and ar.studentname='" + Stunt_name + "' and sm.Status in (" + sections + ") order by ad.Position asc";
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                int total_count = get_total(dr["Session_id"].ToString(), dr["Class_id"].ToString(), dr["Status"].ToString(), Stunt_name, Flags);
        ...
        private static int get_total(string session_id, string class_id, string section, string Stunt_name, string Flags)
        {
            string query = "";
            if (Flags == "0")
            {
                query = "select  sum(isnull(convert(float, dues),0)) as Total_dues_amt from (select (isnull((select sum(cast(dues as float)) from dbo.[Typewise_fee_collection] where (parameter='AdmissionFee' or parameter='HostelAdmissionFee') and  admission_no=admission_registor.admissionserialnumber and session=admission_registor.session),(select sum(cast(amount as float)) from dbo.[Fee_master_content_wise] where parameter='AdmissionFee' and session_id=admission_registor.Session_id and class_id=admission_registor.Class_id))) dues from dbo.[admission_registor] where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Section='" + section + "' and Transfer_Status='New' and Status='1') t";
            }
            else
            {
                query = "select  sum(isnull(convert(float, dues),0)) as Total_dues_amt from (select (isnull((select sum(cast(dues as float)) from dbo.[Typewise_fee_collection] where (parameter='AdmissionFee' or parameter='HostelAdmissionFee') and  admission_no=admission_registor.admissionserialnumber and session=admission_registor.session),(select sum(cast(amount as float)) from dbo.[Fee_master_content_wise] where parameter='AdmissionFee' and session_id=admission_registor.Session_id and class_id=admission_registor.Class_id))) dues from dbo.[admission_registor] where Session_id='" + session_id + "' and Class_id='" + class_id + "' and Section='" + section + "' and Transfer_Status='New' and Status='1'  and studentname='" + Stunt_name + "') t";
            }
            SqlDataAdapter ad = new SqlDataAdapter(query, My.conn);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Cart_Table");
        ...
        private static string get_sections(string Session, string Class, string Stunt_name, string Flags)
        {
            string query = "";
            if (Flags == "0")
            {
                ...
                {
                    query = "select  DISTINCT t2.Section from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Admission' and t1.Adjust_type is null and t2.Session_id='" + Session + "' and t2.Class_id='" + Class + "' order by t2.Section desc";
                }
            }
            else
            {
                query = "select  DISTINCT Section from admission_registor where Session_id='" + Session + "' and studentname='" + Stunt_name + "' order by Section asc";
            }
            SqlDataAdapter ad = new SqlDataAdapter(query, My.conn);
            DataSet ds = new DataSet();

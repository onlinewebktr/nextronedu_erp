...
{
    public partial class admission_discount_report : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        protected void btn_find_Click(object sender, EventArgs e)
        {
            ...
            find_student_discount_admission_Fee();

            scrpt = "<script> click_empty();</script>";
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "tmp", scrpt, false);
        }

        private void find_student_discount_admission_Fee()
        {
            ...
            if (srowcount > 0)
            {
                ...
                fdt.Columns.Add("Discount", Type.GetType("System.Double"));
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    dr["Discount"] = find_discount(dr);
                    total_amt += My.toDouble(dr["Discount"].ToString());
        ...
        private string find_discount(DataRow dr)
        {
            ...
            ViewState["parameter"] = dr["hosteltaken"].ToString() == "yes" ? "HostelAdmissionFee" : "AdmissionFee";
            ...
            string type = "";
            if (My.dataTable("select * from Typewise_fee_collection where admission_no='" + dr["Admission_no"].ToString() + "' and session='" + ddl_session.SelectedItem.Text + "' and (parameter='" + ViewState["parameter"].ToString() + "' or parameter='HostelAdmissionFee')").Rows.Count > 0)
            {
                feedt = My.dataTable("select isnull(sum(convert(float, disc_amount)),0)  Total_discount from (select  (select  disc_amt from Monthly_Fee_Collection_Slip where slipno=Typewise_fee_collection.transection and content_id=Typewise_fee_collection.content_id and Month=Typewise_fee_collection.month) as disc_amount from dbo.[Typewise_fee_collection]   where admission_no='" + dr["Admission_no"].ToString() + "' and  session='" + ddl_session.SelectedItem.Text + "' and (parameter='" + ViewState["parameter"].ToString() + "' or parameter='HostelAdmissionFee') and content_id!='6121') t");

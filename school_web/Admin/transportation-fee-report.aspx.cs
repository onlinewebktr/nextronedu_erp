...
{
    public partial class transportation_fee_report : System.Web.UI.Page
    {
        ...
        private void find_transportation_monthly()
        {
            if (ddl_month.SelectedItem.Text == "All")
            {
                hd_months.Value = "0";
                bind_grd_view("select t2.*,t1.studentname,t1.class,t1.Session_id,t1.Class_id,t1.rollnumber from admission_registor t1 join Typewise_fee_collection t2 on t1.session=t2.session and t1.admissionserialnumber=t2.admission_no and t1.class_id=t2.class_id join Add_course_table ad on t1.Class_id=ad.course_id where t2.parameter='MonthlyFee' and  t2.feetype='TransportationFee' and t2.session='" + ddlsession.SelectedItem.Text + "' order by ad.Position");
            }
            else
            {
                hd_months.Value = ddl_month.SelectedItem.Text;
                bind_grd_view("select t2.*,t1.studentname,t1.class,t1.Session_id,t1.Class_id,t1.rollnumber from admission_registor t1 join Typewise_fee_collection t2 on t1.session=t2.session and t1.admissionserialnumber=t2.admission_no and t1.class_id=t2.class_id join Add_course_table ad on t1.Class_id=ad.course_id where t2.parameter='MonthlyFee' and  t2.feetype='TransportationFee' and t2.session='" + ddlsession.SelectedItem.Text + "' and t2.month='" + ddl_month.SelectedItem.Text + "' order by ad.Position");
            }
        }
        ...
        private void bind_grd_view(string qry)
        {
            ...
            {
                lbl_class22.Text = " Session  :"+ddlsession.SelectedItem.Text+" Month :"+ddl_month.SelectedItem.Text;
        ...
        protected void GrdView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (lbl_paid.Text != "")
                {
                    finelAmt = finelAmt + My.toDouble(lbl_paid.Text);
                }
                decimal value;
                if (decimal.TryParse(lbl_paid.Text.Trim(), out value))
                {
                    lbl_paid.Text = value.ToString("0.00");
                }
            ...
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                lbl_total_amount.Text = finelAmt.ToString("0.00");
        ...
        private static int get_total(string session_id, string class_id, string section, string Months)
        {
            ...
            if (Months == "0")
            {
                query = "select sum(cast(payable as float)) as Fine_amt from Typewise_fee_collection t1 join admission_registor t2 on t1.admission_no=t2.admissionserialnumber and t1.class_id=t2.Class_id and t1.session=t2.Session where t1.feetype='TransportationFee' and t2.Session_id='" + session_id + "' and t2.Class_id='" + class_id + "' and t2.Section='" + section + "'";
            }
            else
            {
                query = "select sum(cast(payable as float)) as Fine_amt from Typewise_fee_collection t1 join admission_registor t2 on t1.admission_no=t2.admissionserialnumber and t1.class_id=t2.Class_id and t1.session=t2.Session where t1.feetype='TransportationFee' and t2.Session_id='" + session_id + "' and t2.Class_id='" + class_id + "' and t2.Section='" + section + "' and t1.month='" + Months + "'";
            }
            SqlDataAdapter ad = new SqlDataAdapter(query, My.conn);
            DataSet ds = new DataSet();
            ad.Fill(ds, "Cart_Table");
        ...
        private static string get_sections(string Session, string Months)
        {
            ...
            if (Months == "0")
            {
                query = "select  DISTINCT t2.Section from Typewise_fee_collection t1 join admission_registor t2 on t1.admission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.session=t2.Session where t1.feetype='TransportationFee' and t2.Session_id='" + Session + "' order by t2.Section asc";
            }
            else
            {
                query = "select  DISTINCT t2.Section from Typewise_fee_collection t1 join admission_registor t2 on t1.admission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.session=t2.Session where t1.feetype='TransportationFee' and t2.Session_id='" + Session + "' and t1.month='" + Months + "' order by t2.Section asc";
            }

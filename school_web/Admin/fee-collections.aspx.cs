...
{
    public partial class fee_collections : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        private void find_details(string query)
        {
            ...
            else
            {
                ...
                DataTable dtmnths = My.dataTable("select * from Month_Index where Position<='1' order by Position asc");
        ...
        private void check_is_cheque_pending(string session_id, string admission_no)
        {
            try
            {
                string queryChk = "select top 1 * from Fee_payment_by_cheque_status where Session_id='" + session_id + "' and  Admission_no='" + admission_no + "' and Status='PENDING' order by id desc";
                ...
                if (dtChk.Rows.Count > 0)
                {
                    ...
                    {
                        if (My.DateConvertToIdate(dr["Cheque_date"].ToString()) != My.DateConvertToIdate(txt_bank_date.Text))
                        {
                        ...
                        if (iS_any_payment_done == "1")
                        {
                            ...
                            if (paymentType == "Monthly")
                            {
                                ...
                                try
                                {
                                    ...
                                    // "Dear " + lbl_name.Text + " you have paid monthly fee :- " + monthlYPaid.ToString() + " date :- " + txt_payment_date.Text + " slip no.:-" + slipno;
                                ...
                                try
                                {
                                    ...
                                    string messge = message;// "Dear " + lbl_name.Text + " you have paid monthly fee :- " + monthlYPaid.ToString() + " date :- " + txt_payment_date.Text + " slip no.:-" + slipno;
                            ...
                            if (paymentType == "MonthlyAdmission")
                            {
                                ...
                                try
                                {
                                    ...
                                    if (admissionPaid > 0)
                                    {
                                        ...
                                        //  "Dear " + lbl_name.Text + " you have paid annual fee :- " + admissionPaid.ToString() + " date : " + txt_payment_date.Text + " slip no.:-" + ViewState["yearlYSLipNo"].ToString();
                                        try
                                        {
                                        ...
                                        if (ViewState["Transfer_Status"].ToString() == "New")
                                        {
                                            ...
                                            messge = message;// "Dear " + lbl_name.Text + " you have paid admission fee :- " + admissionPaid.ToString() + " date : " + txt_payment_date.Text + " slip no.:-" + ViewState["yearlYSLipNo"].ToString();
                                        }
                                        else
                                        {
                                            sub = "Annual Fee Deposit";
                                            messge = message;// "Dear " + lbl_name.Text + " you have paid annual fee :- " + admissionPaid.ToString() + " date : " + txt_payment_date.Text + " slip no.:-" + ViewState["yearlYSLipNo"].ToString();
                                        }
                                        Dictionary<String, String> ss = new Dictionary<string, string>();
                                        ss["notification_id"] = Guid.NewGuid().ToString();
                                        ss["message"] = messge;
                            ...
                            if (confirmValue == "Yes")
                            {
                                ...
                                else if (paymentType == "MonthlyAdmission")
                                {
                                ...
                                {
                                    if (feeTypes == "Admission")
                                    {
                                        Response.Redirect("slip/Admission_slip.aspx?admissionno=" + lbl_admission_no.Text + "&sessionid=" + ddlsessionad.SelectedValue + "&classid=" + ViewState["classid"].ToString() + "&Slip_no=" + ViewState["yearlYSLipNo"].ToString(), false);
                                    }
                                    if (feeTypes == "Annual")
                                    {
                                        Response.Redirect("slip/annual-slip.aspx?admissionno=" + lbl_admission_no.Text + "&sessionid=" + ddlsessionad.SelectedValue + "&classid=" + ViewState["classid"].ToString() + "&Slip_no=" + ViewState["yearlYSLipNo"].ToString(), false);
                                    }
                                }
                            }
                            else
                            {
                                ...
                                {
                                    Response.Redirect("fee-collections.aspx?adm=" + lbl_admission_no.Text, false);
        ...
        private void make_admission_fee(SqlConnection con, double PaidAmt)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                admission_payment(slip_no, entry_id, ad_no, con, PaidAmt);
        ...
        private void admission_payment(string slip_no, string entry_id, string ad_no, SqlConnection con, double PaidAmt)
        {
            string Uid = slip_no;
            ...
            send_data_to_school_ledger_admission(slip_no, entry_id, con, PaidAmt);
        ...
        private void make_annual_fee(SqlConnection con, double PaidAmt)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                slip_no = payments.invoice_readmission("slip_no", con);
                ViewState["yearlYSLipNo"] = slip_no;
                annual_payment(slip_no, entry_id, ad_no, con, PaidAmt);
                payments.send_data_to_user_log_history(ViewState["Userid"].ToString(), ViewState["Userid"].ToString() + " received " + type + " fee :-" + PaidAmt + " Adjust Amount" + hd_total_discount.Value + " rs,Slip No :- " + slip_no + " from " + lbl_studentname.Text + ", Admission No :-" + ad_no, con);
                //try
                //{
        ...
        private void annual_payment(string slip_no, string entry_id, string ad_no, SqlConnection con, double PaidAmt)
        {
            string Uid = slip_no;
            string Tag = PaidAmt.ToString();
            string parameter = "";
            parameter = ViewState["hostaltaken"].ToString().ToUpper() == "NO" ? "AnnualFee" : "HostelAnnualFee";
            send_data_in_student_payment_history_annual(type, slip_no, entry_id, ad_no, parameter, con, PaidAmt);
            send_data_to_school_ledger_annual(slip_no, entry_id, con, PaidAmt);
                ...
                if (ViewState["Is_add_Discount"].ToString() == "1")
                {
                    ...
                    if (chk_is_admission_disc.Checked == true)
                    {
                        ...
                        if (ViewState["issubmit"].ToString() == "1")
                        {
                            ...
                            Response.Redirect("fee-collections.aspx?adm=" + txt_admission_no.Text, false);
                        }
                    }
                    else
                    {
        ...
        protected void lnkDelExtFee_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                log_hostory.delete_log(lbl_session_id.Text, ViewState["classid"].ToString(), lbl_admission_no.Text, "DeleteExtraFeeHead", desc, "fee-collections.aspx", ViewState["Userid"].ToString());
        ...
        protected void btn_Submit_Click(object sender, EventArgs e)
        {
            if (ViewState["Is_Edit_Bus"].ToString() == "1")
            {
                ...
                else
                {
                    ...
                    Response.Redirect("fee-collections.aspx?adm=" + ViewState["Admission_no"].ToString(), false);
                }
            }
            else
            {
            ...
            try
            {
                if (ViewState["Is_delete_Bus"].ToString() == "1")
                {
                    ...
                    if (My.InsertUpdateData(cmd1))
                    {
                        ...
                        log_hostory.delete_log(ViewState["sessionIDs"].ToString(), ViewState["classid"].ToString(), lbl_admission_no.Text, "RemoveTransport", desc, "fee-collections.aspx", ViewState["Userid"].ToString());
        ...
        protected void btn_remove_hostel_Click(object sender, EventArgs e)
        {
            try
            {
                if (ViewState["Is_delete_Hostel"].ToString() == "1")
                {
                    ...
                    log_hostory.delete_log(ViewState["sessionIDs"].ToString(), ViewState["classid"].ToString(), lbl_admission_no.Text, "RemoveHostel", desc, "fee-collections.aspx", ViewState["Userid"].ToString());

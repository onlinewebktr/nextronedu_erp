...
{
    public partial class annual_fee_collection_report : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        private void final_find_report_by_date(int idate, int idate2)
        {
            ...
            {
                bind_grd_view("select  t2.studentname,t1.Addmission_no,t2.class,t2.session,t1.Date,t1.Slip_no,t1.mode,t2.Session_id,t1.Class_id, isnull(convert(float, t1.Amount), 0) as Total_paid_amt from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Annual'  and t1.Idate>='" + idate + "' and t1.Idate<='" + idate2 + "' and t1.Adjust_type is null order by t1.id desc");
            }
            else
            {
                bind_grd_view("select  t2.studentname,t1.Addmission_no,t2.class,t2.session,t1.Date,t1.Slip_no,t1.mode,t2.Session_id,t1.Class_id, isnull(convert(float, t1.Amount), 0) as Total_paid_amt from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Annual'  and t1.Idate>='" + idate + "' and t1.Idate<='" + idate2 + "' and  t1.mode='" + ddl_paymentmode.Text + "' and t1.Adjust_type is null order by t1.id desc");
            }
        }
            ...
            else
            {
                ...
                if (ViewState["flag"].ToString() == "0")
                {
                    if (ddl_paymentmode.Text == "All")
                    {
                        ...
                        qrySS = "select sum(isnull(convert(float, Amount),0)) as Paid_amt,mode from Student_Payment_History where Type='Annual' and Session='" + hd_sessions.Value + "'  group by mode";
                    }
                    else
                    {
                        qryS = "select  t2.studentname,t1.Addmission_no,t2.class,t2.session,t1.Date,t1.Slip_no,t1.mode,t2.Session_id,t1.Class_id,  isnull(convert(float, t1.Amount), 0) as Total_paid_amt,from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Annual' and t2.Session_id='" + hd_session.Value + "' and  t1.mode='" + ddl_paymentmode.Text + "' and t1.Adjust_type is null";

                        qrySS = "select sum(isnull(convert(float, Amount),0)) as Paid_amt,mode from Student_Payment_History where Type='Annual' and Session='" + hd_sessions.Value + "' and   mode='" + ddl_paymentmode.Text + "'  group by mode";
                ...
                if (ViewState["flag"].ToString() == "1")
                {
                    ...
                    if (ddl_paymentmode.Text == "All")
                    {
                        qryS = "select  t2.studentname,t1.Addmission_no,t2.class,t2.session,t1.Date,t1.Slip_no,t1.mode,t2.Session_id,t1.Class_id,  isnull(convert(float, t1.Amount), 0) as Total_paid_amt from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Annual' and  t1.Idate>='" + idate + "' and t1.Idate<='" + idate2 + "'  order by t1.id desc";

                        qrySS = "select sum(isnull(convert(float, Amount),0)) as Paid_amt,mode from Student_Payment_History where Type='Annual' and  Idate>='" + idate + "' and Idate<='" + idate2 + "' group by mode";
                    }
                    else
                    {
                        qryS = "select  t2.studentname,t1.Addmission_no,t2.class,t2.session,t1.Date,t1.Slip_no,t1.mode,t2.Session_id,t1.Class_id, isnull(convert(float, t1.Amount), 0) as Total_paid_amt from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Annual' and t1.Idate>='" + idate + "' and t1.Idate<='" + idate2 + "' and  t1.mode='" + ddl_paymentmode.Text + "'  order by t1.id desc";

                        qrySS = "select sum(isnull(convert(float, Amount),0)) as Paid_amt,mode from Student_Payment_History where Type='Annual'  and Idate>='" + idate + "' and Idate<='" + idate2 + "' and  mode='" + ddl_paymentmode.Text + "' group by mode";
                    }
        ...
        private static string get_sections(string Session, string From_date, string To_date)
        {
            ...
            if (From_date == "0")
            {
                query = "select  DISTINCT t2.Section from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t1.Type='Annual' and t1.Adjust_type is null and t2.Session_id='" + Session + "' order by t2.Section desc";
            }
            else
            {
                query = "select  DISTINCT t2.Section from Student_Payment_History t1 join admission_registor t2 on t1.Addmission_no=t2.admissionserialnumber and t1.Class_id=t2.Class_id and t1.Session=t2.Session where t2.Session_id='" + Session + "' and t1.Type='Annual' and t1.Adjust_type is null and t1.Idate>='" + From_date + "' and t1.Idate<='" + To_date + "' order by t2.Section desc";
            }
            SqlDataAdapter ad = new SqlDataAdapter(query, My.conn);
            DataSet ds = new DataSet();

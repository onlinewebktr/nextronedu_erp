<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="print-demand-bill-inst.aspx.cs" Inherits="school_web.Admin.slip.print_demand_bill_inst" %>
    ...
    <form id="form1" runat="server">
                                    ...
                                    <div class="administration-paragraph ">
                                        <table>
                                            ...
                                            <tr data-ng-repeat="item in x.MyFeeDetailsItem track by $index" class="{{item.rowsHidden}}">
                                                <td class="txtcntr {{item.InstHide}}" rowspan="{{item.RowSpan}}">{{item.SlNo}}<%--{{$index+1}}--%></td>
                                                <td rowspan="{{item.RowSpan}}" class="{{item.InstHide}}">{{item.InstallmentName}} ({{item.Month_name}})</td>
                                                <td>{{item.Content}}</td>
                                                <td class="txtcntr">{{item.Amount}}</td>
                                            </tr>

                                            <tr class="{{x.lateFine_zero}}">
                                                <td colspan="3" class="txt-right txt-bold">Late Fine</td>
                                                <td class="txt-bold txtcntr">{{x.FIne_amt}}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="3" class="txt-right txt-bold">Total</td>
                                        ...
                                        <div class="sig-dv">
                                            <div class="sig-left">
                                                 <div class="rght-sig-img-dv" style="text-align: right;">
                                                    <img src="{{x.Principal_sig}}" class="rght-sig-img {{x.Principal_sig}}" style="margin: 0px 16px 0px 0px;" />
                                                </div>
        ...
        <script type="text/javascript">
            ...
            app.controller('RpCardAppCtrl', function ($scope, $http) {
                ...
                $http.get("webServices/demandBillInst.asmx/fetch_demand_bill", { params: { "Session_id": session_id, "Class_id": class_id, "Admission_no": admission_no, "Branch_id": branch_id, "Section": section, "Checked": checked, "Month_name": month_name, "Paydate": paydate, "Session_name": session_name, "Bill_type": bill_type, "Std_type": std_type } }).then(function (response) {

...
{
    public partial class Create_Abacas_Certificate : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        protected void btn_create_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    SqlCommand cmd;
                    string certificate_no = My.Sport_format("Abacus_sl");
                    SqlDataAdapter ad = new SqlDataAdapter("select * from Certificate_Master where Session_id=" + ViewState["sessioN"].ToString() + " and Class_id='" + ViewState["class_id"].ToString() + "' and Admission_no='" + ViewState["adm_no"].ToString() + "' and Certificate_type='Abacus' and Participated_in='" + txt_Participated_in_the.Text.Trim() + "'", My.conn);
                    DataSet ds = new DataSet();
                    ad.Fill(ds, "Certificate_Master");
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count == 0)
                    {
                        string query = "INSERT INTO Certificate_Master (Participated_in,Session_id,Class_id,Admission_no,Certificate_type,Create_date,Create_idate,User_id,Firm_id,Issue_date,Certificate_no,Remarks) values (@Participated_in,@Session_id,@Class_id,@Admission_no,@Certificate_type,@Create_date,@Create_idate,@User_id,@Firm_id,@Issue_date,@Certificate_no,@Remarks)";
                        ...
                        cmd.Parameters.AddWithValue("@Certificate_type", "Abacus");
                        ...
                        if (My.InsertUpdateData(cmd))
                        {
                            ...
                            Alertme("Abacus certificate has been created successfully.", "success");

                            try
                            {
                                string sub = "Abacus Certificate";
                                string messge = "Dear student your Abacus certificate has been generated please come to the school and collect the certificate.";

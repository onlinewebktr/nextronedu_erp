...
{
    public partial class fees_collections : System.Web.UI.Page
    {
        My mycode = new My();
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    else
                    {
                        ...
                        ViewState["IsEarlyPaidDiscount"] = "0";
                        ViewState["Payment_up_to_day"] = "0";
        ...
        private void find_firm_details()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                try
                {
                    if (dt.Rows[0]["Is_early_payment_discount"].ToString() == "True")
                    {
                        ViewState["IsEarlyPaidDiscount"] = "1";
                        ViewState["Payment_up_to_day"] = dt.Rows[0]["Payment_up_to_day"].ToString();
                    }
                }
                catch (Exception ex)
                {
            ...
            try
            {
                ...
                if (ViewState["Is_month_selection_free"].ToString() == "1")
                {
                    ...
                    if (dtmnths.Rows.Count > 0)
                    {
                        ...
                        if (ViewState["IsEarlyPaidDiscount"].ToString() == "1")
                        {
                            calculate_eary_disc();
                        }
                        else
        ...
        private void bind_ttl_fee()
        {
            ...
            if (ViewState["IsEarlyPaidDiscount"].ToString() == "1")
            {
                DataTable dtds = My.dataTable("select isnull(sum(convert(float, disc_amount)),0) as disc_amount from Temp_Discount_Master where session_id='" + ViewState["sessionIDs"].ToString() + "' and admission_no='" + txt_admission_no.Text + "'");
                txt_discount.Text = (totaldisc + My.toDouble(dtds.Rows[0]["disc_amount"].ToString())).ToString();
                txtfineamount.Text = ViewState["fineAmt"].ToString();
                string monthlyFee = (totalpblE + My.toDouble(ViewState["fineAmt"].ToString()) + My.toDouble(txt_other_fee.Text)).ToString();
                txt_monthlyFee.Text = (My.toDouble(monthlyFee) - My.toDouble(dtds.Rows[0]["disc_amount"].ToString())).ToString();
            }
            else
            {
                        ...
                        if (iS_any_payment_done == "1")
                        {
                            ...
                            if (chk_collect_adm_ann_fee.Checked == true)
                            {
                                ...
                                try
                                {
                                    ...
                                    //{
                                    //    DataTable dtD = My.dataTable("select *,(convert(float, payable)-(convert(float, paid_amt)+convert(float, disc_amt)+convert(float, prev_paid_amt))) as Dues_amt from (select isnull(sum(convert(float, payable)),0) as payable,isnull(sum(convert(float, disc_amt)),0) as disc_amt,isnull(sum(convert(float, paid)),0) as paid_amt,isnull(sum(convert(float, previously_paid)),0) as prev_paid_amt,(select top 1 Month from Monthly_Fee_Collection_Slip where slipno='" + slip_no + "' order by month_position desc) as Month from Monthly_Fee_Collection_Slip where slipno='" + slip_no + "') t");
                                    //    ViewState["SMS_Tempate"] = "Received with thanks from you on the account of Master/Miss " + lbl_name.Text + ", Class " + lblclass.Text + ", Adm. no. " + lbl_admission_no.Text + ", Roll no. " + txtroll_no.Text + " for / upto the Month " + dtD.Rows[0]["Month"].ToString() + " Rs. " + txt_paid_amount.Text + "/-, Dues " + dtD.Rows[0]["Dues_amt"].ToString() + "/- on Date " + txt_payment_date.Text + ". \nRegards \nDeep English School, \nKatihar 😊";
                                    //}
                                    ...
                                    //if (ViewState["firm_idS"].ToString() == "DEEP-1" || ViewState["firm_idS"].ToString() == "DEEP-2")
                                    //{
                                    //    message = ViewState["SMS_Tempate"].ToString();
                                    //}
                                    //else
                                    //{
                                    //    message = String.Format(ViewState["SMS_Tempate"].ToString(), lst);
                                        ...
                                        if (ViewState["Is_Send_WhatsApp"].ToString().ToUpper() == "TRUE")
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                if (dt.Rows.Count == 1)
                                                {
                                                    ...
                                                    ViewState["Wid"] = dt.Rows[0]["Wid"].ToString();
                                                }
                                                else
                                                {
                                                ...
                                                var query = new Dictionary<string, string>()
    {
        ...
        { "9", lst[8] }
                            ...
                            else
                            {
                                ...
                                {
                                    Response.Redirect("fees-collections.aspx?adm=" + lbl_admission_no.Text, false);
        ...
        private void calculate_dues_for_new_month(string slipno, SqlConnection con)
        {
            if (ViewState["IsEarlyPaidDiscount"].ToString() == "1")
            {
                ...
                double paidAmtS = My.toDouble(txt_paid_amount.Text);
                try
                {
                    if (chk_collect_adm_ann_fee.Checked == true)
                    {
                        paidAmtS = My.toDouble(txt_paid_amount.Text) - My.toDouble(txt_adm_ann_fee.Text);
                    }
                }
                catch (Exception ex)
                {
                ...
                for (int i = 0; i < growcount; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        int growcountFee = rp_fee_details.Items.Count;
                        for (int iF = 0; iF < growcountFee; iF++)
                        {
                            Label lbl_mnth = (Label)rp_fee_details.Items[iF].FindControl("lbl_mnth");
                            Label lbl_tot_pble = (Label)rp_fee_details.Items[iF].FindControl("lbl_tot_pble");
                            Label lblcontent_id = (Label)rp_fee_details.Items[iF].FindControl("lblcontent_id");


                            Label lbl_amount = (Label)rp_fee_details.Items[iF].FindControl("lbl_amount");

                            if (lbl_Month.Text == lbl_mnth.Text)
                            {
                                paidAmtS = paidAmtS - My.toDouble(lbl_tot_pble.Text);
                                if (My.toDouble(lbl_amount.Text) == My.toDouble(lbl_tot_pble.Text))
                                {
                                    if (lblcontent_id.Text == "72" || lblcontent_id.Text == "73" || lblcontent_id.Text == "74")
                                    {
                                        paidAmtS = paidAmtS + 200;
                                    }

                                    if (ViewState["firm_idS"].ToString() == "My-001" || ViewState["firm_idS"].ToString() == "My-003" || ViewState["firm_idS"].ToString() == "Ndeep-001" || ViewState["firm_idS"].ToString() == "Ndeep-002")
                                    {
                                        if (lblcontent_id.Text == "69" || lblcontent_id.Text == "70" || lblcontent_id.Text == "71")
                                        {
                                            paidAmtS = paidAmtS + 200;
                                        }
                                    }
                        ...
                        string lastPayDate = ViewState["Payment_up_to_day"].ToString() + "/" + monthid + "/" + s_year;
                        if (My.toIntS(My.DateConvertToIdate(lastPayDate)) >= My.toIntS(My.DateConvertToIdate(txt_payment_date.Text)))
                        {
                            if (paidAmtS >= 0)
                            {
                                DataTable dtds = payments.dataTable("select * from Temp_Discount_Master where session_id='" + ViewState["sessionIDs"].ToString() + "' and admission_no='" + txt_admission_no.Text + "' and month='" + lbl_Month.Text + "'", con);
                                if (dtds.Rows.Count > 0)
                                {
                                    foreach (DataRow drDS in dtds.Rows)
                                    {
                                        ...
                                        {
                                            query = "select * from Discount_Master where Class_id=" + drDS["Class_id"].ToString() + " and session='" + drDS["session"].ToString() + "' and group_id='3' and month='" + drDS["month"].ToString() + "' and admission_no='" + txt_admission_no.Text + "' and fee_head_id='" + drDS["fee_head_id"].ToString() + "' and parameter_id='" + drDS["parameter_id"].ToString() + "' and Hostel_id=" + drDS["Hostel_id"].ToString() + " and Room_Category_id=" + drDS["Room_Category_id"].ToString() + "";
                                        }
                                        else
                                        {
                                            query = "select * from Discount_Master where Class_id=" + drDS["Class_id"].ToString() + " and session='" + drDS["session"].ToString() + "' and group_id='3' and month='" + drDS["month"].ToString() + "' and admission_no='" + txt_admission_no.Text + "' and fee_head_id='" + drDS["fee_head_id"].ToString() + "' and parameter_id='" + drDS["parameter_id"].ToString() + "'";
                                        }
                                        ...
                                        if (dt.Rows.Count == 0)
                                        {
                                            ...
                                            dr["Class_id"] = drDS["Class_id"].ToString();
                                            dr["Discount_on"] = "Monthly";
                                            dr["session"] = drDS["session"].ToString();
                                            dr["Discount_per"] = drDS["Discount_per"].ToString();
                                            dr["group_id"] = "3";
                                            dr["admission_no"] = drDS["admission_no"].ToString();
                                            dr["month"] = drDS["month"].ToString();
                                            dr["fee_head_id"] = drDS["fee_head_id"].ToString();
                                            dr["disc_amount"] = drDS["disc_amount"].ToString();
                                            dr["parameter_id"] = drDS["parameter_id"].ToString();
                                            dr["category_id"] = drDS["category_id"].ToString();
                                            dr["sub_category_id"] = drDS["sub_category_id"].ToString();
                                            dr["session_id"] = drDS["session_id"].ToString();
                                            dr["Branch_id"] = drDS["Branch_id"].ToString();
                                            dr["User_id"] = drDS["User_id"].ToString();
                                            dr["Date"] = mycode.date();
                                            dr["time"] = mycode.time();
                                            dr["Hostel_id"] = drDS["Hostel_id"].ToString();
                                            dr["Room_Category_id"] = drDS["Room_Category_id"].ToString();
                                            dr["Student_Discunt_Type_id"] = "0";
                                            dr["Student_Discunt_Remarks"] = "Payment on time";
                                            dt.Rows.Add(dr);
                                        }
        ...
        protected void btn_find_name_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string stdnamed = txt_student_name.Text.Trim();
                string query = "";
                if (ViewState["Isadmissionfeetekent"].ToString() == "1")// admission fee not mindtree
                {
                    query = "select * from admission_registor where studentname like '%" + stdnamed + "%' and session='" + ddl_session_student.SelectedItem.Text + "' and  Status='1' order by id asc";
                }
                else
                {
                    query = "select * from admission_registor where studentname like '%" + stdnamed + "%' and session='" + ddl_session_student.SelectedItem.Text + "' and payment_status!='Unpaid' and  Status='1' order by id asc";
                }
                ...
                else if (dt.Rows.Count == 1)
                {
                    if (ViewState["Isadmissionfeetekent"].ToString() == "1")// admission fee not mindtree
                    {
                        query = "select * from admission_registor where admissionserialnumber='" + dt.Rows[0]["admissionserialnumber"].ToString() + "' and Session='" + ddl_session_student.SelectedItem.Text + "' order by id asc";
                    }
                    else
                    {
                        query = "select * from admission_registor where admissionserialnumber='" + dt.Rows[0]["admissionserialnumber"].ToString() + "' and Session='" + ddl_session_student.SelectedItem.Text + "' and payment_status!='Unpaid' order by id asc";
                    }
                    find_details(query);
                }
                else
        ...
        private void calculate_eary_disc()
        {
            My.exeSql("delete from Temp_Discount_Master where admission_no='" + txt_admission_no.Text + "'");
            double totalDiscount = 0;
            string pay_date = txt_payment_date.Text;
            int payidate = My.DateConvertToIdate(pay_date);
            string Payment_up_to_day = ViewState["Payment_up_to_day"].ToString();
            ...
            for (int ixi = 0; ixi < mgrowcount; ixi++)
            {
                ...
                if (chkM.Checked == true)
                {
                    ...
                    s_year = My.check_start_months(pay_month, s_year);
                    string last_day_of_payments = Payment_up_to_day + "/" + pay_month_two_digit + "/" + s_year;
                    ...
                    if (totaldays <= 0)
                    {
                        ...
                        {
                            int feecount = rp_fee_details.Items.Count;
                            for (int i = 0; i < feecount; i++)
                            {
                                ...
                                Label lbl_disc_amt = (Label)rp_fee_details.Items[i].FindControl("lbl_disc_amt");
                                Label lbl_amount = (Label)rp_fee_details.Items[i].FindControl("lbl_amount");
                                if (My.toDouble(lbl_disc_amt.Text) <= 0)
                                {
                                    if (lbl_mnth.Text.ToUpper() == lbl_Month.Text.ToUpper())
                                    {
                                        DataTable dtDS = My.dataTable("select * from Early_payment_discount_setup where Status=1");
                                        if (dtDS.Rows.Count > 0)
                                        {
                                            foreach (DataRow drDS in dtDS.Rows)
                                            {
                                                if (drDS["Content_id"].ToString() == lblcontent_id.Text)
                                                {
                                                    totalDiscount = totalDiscount + My.toDouble(drDS["Discount_amt"].ToString());

                                                    //SaveTempDiscount
                                                    string query = "";
                                                    if (ViewState["parameterDisc"].ToString() == "3") // hostel
                                                    {
                                                        query = "select * from Temp_Discount_Master where Class_id=" + ViewState["class_id"].ToString() + " and session='" + ViewState["session"].ToString() + "' and group_id='3' and month='" + lbl_Month.Text + "' and admission_no='" + txt_admission_no.Text + "' and fee_head_id='" + drDS["Content_id"].ToString() + "' and parameter_id='" + ViewState["parameterDisc"].ToString() + "' and Hostel_id=" + ViewState["Hostel_id"].ToString() + " and Room_Category_id=" + ViewState["Room_Category_id"].ToString() + "";
                                                    }
                                                    else
                                                    {
                                                        query = "select * from Temp_Discount_Master where Class_id=" + ViewState["class_id"].ToString() + " and session='" + ViewState["session"].ToString() + "' and group_id='3' and month='" + lbl_Month.Text + "' and admission_no='" + txt_admission_no.Text + "' and fee_head_id='" + drDS["Content_id"].ToString() + "' and parameter_id='" + ViewState["parameterDisc"].ToString() + "'";
                                                    }
                                                    ...
                                                    if (dt.Rows.Count == 0)
                                                    {
                                                        ...
                                                        dr["Discount_per"] = My.Round((My.toDouble(drDS["Discount_amt"].ToString()) * 100) / My.toDouble(lbl_amount.Text), 2);
                                                        dr["group_id"] = "3";
                                                        dr["admission_no"] = txt_admission_no.Text;
                                                        dr["month"] = lbl_Month.Text;
                                                        dr["fee_head_id"] = drDS["Content_id"].ToString();
                                                        dr["disc_amount"] = My.toDouble(drDS["Discount_amt"].ToString()).ToString("0.00");
            ...
            bind_ttl_fee();
            //Alertme("Total discount : " + totalDiscount.ToString(), "warning");
        }
                ...
                if (ViewState["Is_add_Discount"].ToString() == "1")
                {
                    ...
                    if (chk_is_admission_disc.Checked == true)
                    {
                        ...
                        if (ViewState["issubmit"].ToString() == "1")
                        {
                            ...
                            Response.Redirect("fees-collections.aspx?adm=" + txt_admission_no.Text, false);
                        }
                    }
                    else
                    {
            ...
            if (isamountfill == true)
            {
                ...
                if (flag == true)
                {
                    if (ViewState["issubmit"].ToString() == "1")
                    {
                        Session["IsDiscountOpen"] = "0";
                        Session["msg"] = "Discount Applied successfully for " + txt_student_name.Text;
                        Response.Redirect("fees-collections.aspx?adm=" + txt_admission_no.Text + "&sessionid=" + ddlsessionad.SelectedValue, false);
                    }
                }
            }
            else
                ...
                if (ViewState["Is_delete_Discount"].ToString() == "1")
                {
                    ...
                    if (lbl_discount_on.Text == "Monthly" || lbl_discount_on.Text == "TransportFee")
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            if (flag == true)
                            {
                                ...
                                Response.Redirect("fees-collections.aspx?adm=" + lbl_admission_no.Text + "&sessionid=" + ddlsessionad.SelectedValue, false);
                            }
                        }
                        else
                        {
                    ...
                    else if (lbl_discount_on.Text == "Annual" || lbl_discount_on.Text == "Admission")
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.RequiresNew, new TimeSpan(1, 0, 0)))
                            {
                                payments.exeSql("delete from Discount_Master where admission_no='" + lbl_admission_no.Text + "' and Class_id='" + lbl_class_id.Text + "'  and session_id='" + lbl_session_id.Text + "' and Discount_on='" + lbl_discount_on.Text + "' and fee_head_id='" + lbl_content_id.Text + "'", con);
                                //CheckinTypewise
        ...
        private void for_extra_fee_head(string session_name, string session_id, string class_id, string admission_no, string hostel_taken, string transport_taken, string hostel_id, string room_category_id, string TransportPath_id, string Boarding_Point_id)
        {
            ...
            else
            {
                ...
                if (ViewState["Is_quarterwise_payment"].ToString() == "1")
                {
                    qryExtHeadMonth = "select Month, Position,Month_Id from (select Month, Position,(select isnull(sum(convert(float, amount)),0) from Fee_master_content_wise where (parameter = 'MonthlyFee'  or parameter = 'HostelMonthlyFee') and session_id='" + ViewState["sessionIDs"].ToString() + "' and class_id='" + ViewState["classid"].ToString() + "' and Month=Month_Index.Month) as Total_fee from Month_Index where Month in (select month from Typewise_fee_collection where admission_no = '" + admission_no + "' and(parameter = 'MonthlyFee'  or parameter = 'HostelMonthlyFee') and session = '" + session_name + "' and class_id = '" + class_id + "' and status = 'Dues') or Month not in (select month from Typewise_fee_collection where admission_no = '" + admission_no + "' and(parameter = 'MonthlyFee'  or parameter = 'HostelMonthlyFee') and session = '" + session_name + "' and class_id = '" + class_id + "' and status = 'Paid')) t where convert(float, Total_fee)>0 order by Position asc";
                }
        ...
        {
            string month_name = payments.get_single_column_data("select Month as Column_Name from Month_Index where Position=1", con);
            SqlCommand cmd;
            string query = "INSERT INTO Misc_Fee_Master_Studentwise (Admission_No,Month,Session,Session_id,Perticular,Amount,Ledger,Type_Mode,Date,Idate,Created_by) values (@Admission_No,@Month,@Session,@Session_id,@Perticular,@Amount,@Ledger,@Type_Mode,@Date,@Idate,@Created_by)";
            cmd = new SqlCommand(query);
            cmd.Parameters.AddWithValue("@Admission_No", hd_admission_no.Value);
        ...
        protected void lnkDelExtFee_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope(TransactionScopeOption.RequiresNew, new TimeSpan(1, 0, 0)))
                {
                    ...
                    payments.delete_log(lbl_session_id.Text, ViewState["classid"].ToString(), lbl_admission_no.Text, "DeleteExtraFeeHead", desc, "fees-collections.aspx", ViewState["Userid"].ToString(), con);
                    dues_update_headwise_transaction.update_student_dues(ViewState["sessionIDs"].ToString(), ViewState["class_id"].ToString(), lbl_admission_no.Text, "0", "0", con);
        ...
        protected void btn_Submit_Click(object sender, EventArgs e)
        {
            if (ViewState["Is_Edit_Bus"].ToString() == "1")
            {
                ...
                else
                {
                    ...
                    Response.Redirect("fees-collections.aspx?adm=" + ViewState["Admission_no"].ToString(), false);
                }
            }
            else
            {
...
values ('" + ViewState["Admission_no"].ToString() + "','" + class1 + "','" + ViewState["session"].ToString() + "','MonthlyFee','" + mycode.date() + "','" + mycode.idate() + "','" + TransportationFeetype + "','" + My.toDouble(Amount).ToString("0.00") + "','0','" + My.toDouble(afterdicount).ToString("0.00") + "','Dues','" + ddl_monthname.SelectedItem.Text + "','" + content_id1 + "','','School','false','false','false','" + My.toDouble(disc_amount).ToString("0.00") + "','" + section + "','" + ViewState["Userid"].ToString() + "','" + ViewState["Branchid"].ToString() + "','" + group_id + "','" + class_id + "','" + position + "','" + afterdicount.ToString("0.00") + "')", con);
            ...
            {
                Session["msg"] = "Transportation has been successfully updated.";
                Response.Redirect("fees-collections.aspx?adm=" + ViewState["Admission_no"].ToString() + "&sessionid=" + ddlsessionad.SelectedValue, false);
            }
        }
                ...
                if (ViewState["Is_delete_Bus"].ToString() == "1")
                {
                    ...
                    using (TransactionScope scope = new TransactionScope(TransactionScopeOption.RequiresNew, new TimeSpan(1, 0, 0)))
                    {
                        ...
                        if (payments.InsertUpdateData(cmd1, con))
                        {
                            ...
                            payments.delete_log(ViewState["sessionIDs"].ToString(), ViewState["classid"].ToString(), lbl_admission_no.Text, "RemoveTransport", desc, "fees-collections.aspx", ViewState["Userid"].ToString(), con);
            ...
            try
            {
                if (ViewState["Is_delete_Hostel"].ToString() == "1")
                {
                    ...
                    using (TransactionScope scope = new TransactionScope(TransactionScopeOption.RequiresNew, new TimeSpan(1, 0, 0)))
                    {
                        ...
                        payments.exeSql("update Hostel_assign_master set Status='0',Remove_cause='Remove From Fee Collection Page',Updated_by='" + ViewState["Userid"].ToString() + "',Updated_date='" + mycode.date() + "',Updated_idate='" + mycode.idate() + "' where Hostel_assign_id='" + ViewState["Hostel_assign_id"].ToString() + "'", con);
                        ...
                        payments.delete_log(ViewState["sessionIDs"].ToString(), ViewState["classid"].ToString(), lbl_admission_no.Text, "RemoveHostel", desc, "fees-collections.aspx", ViewState["Userid"].ToString(), con);
                        dues_update_headwise_transaction.update_student_dues(ViewState["sessionIDs"].ToString(), ViewState["class_id"].ToString(), lbl_admission_no.Text, "0", "0", con);

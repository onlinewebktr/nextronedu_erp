...
{
    public partial class pending_coupon_list : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        {
            string query = "select t2.session,t2.class,t2.Section,t2.rollnumber,t2.studentname,t2.fathername,t2.mothername,t2.Father_whatsApp_no,t1.* from Coupon_applied_list t1 join admission_registor t2 on t1.Session_id=t2.Session_id and t1.Class_id=t2.Class_id and t1.Admission_no=t2.admissionserialnumber where t1.Status='Pending' and t1.Session_id='" + ddl_session_serach.SelectedValue + "' order by t1.Id desc";
            Bind_data(query);
        }
        ...
        {
            string query = "select t2.session,t2.class,t2.Section,t2.rollnumber,t2.studentname,t2.fathername,t2.mothername,t2.Father_whatsApp_no,t1.* from Coupon_applied_list t1 join admission_registor t2 on t1.Session_id=t2.Session_id and t1.Class_id=t2.Class_id and t1.Admission_no=t2.admissionserialnumber where t1.Status='Pending' and t1.Session_id='" + ddl_session_serach.SelectedValue + "' and t1.Created_idate>='" + My.DateConvertToIdate(txt_s_date.Text) + "' and t1.Created_idate<='" + My.DateConvertToIdate(txt_s_date.Text) + "' order by t1.Id desc";
            Bind_data(query);
        }
        ...
        protected void btn_update_status_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            if (ViewState["Is_Send_WhatsApp"].ToString().ToUpper() == "TRUE")
                            {
                                try
                                {
                                    ...
                                    if (ViewState["mobile_no"].ToString().Length > 9)
                                    {
                    ...
                    Alertme("Coupon status has been updated successfully.", "success");
                    find_all_data();
                }
            }
            catch (Exception ex)
        ...
        protected void btn_update_disc_Click(object sender, EventArgs e)
        {
            try
            {
                update_cp_discount();
        ...
        private void update_cp_discount()
        {
            for (int i = 0; i < rd_coupon_head.Items.Count; i++)
            {
                Label lbl_disc_row_id = (Label)rd_coupon_head.Items[i].FindControl("lbl_disc_row_id");
                TextBox txt_head_fee = (TextBox)rd_coupon_head.Items[i].FindControl("txt_head_fee");
                TextBox txt_disc_fee = (TextBox)rd_coupon_head.Items[i].FindControl("txt_disc_fee");
                TextBox txt_disc_perc = (TextBox)rd_coupon_head.Items[i].FindControl("txt_disc_perc");
                My.exeSql("update Discount_Master_temp set disc_amount='" + txt_disc_fee.Text + "',Discount_per='" + My.toDouble(txt_disc_perc.Text).ToString("0.00") + "' where Id='" + lbl_disc_row_id.Text + "'");
                string desc = "Coupon discount update for coupon no. " + ViewState["couponCodE"].ToString();
                log_hostory.edit_log(ViewState["session_id"].ToString(), ViewState["class_id"].ToString(), ViewState["adm_no"].ToString(), "Coupon", desc, "pending-coupon-list.aspx", ViewState["Userid"].ToString());
            }

            DataTable dtaD = My.dataTable("select isnull(sum(convert(float, Discount_amt)),0) as Discount_amt,isnull(sum(convert(float, Total_amount)),0) as Total_amount from (select isnull(sum(convert(float, disc_amount)),0) as Discount_amt,isnull(sum(convert(float, Fee_amount)),0) as Total_amount from Discount_Master_temp where session='" + ViewState["sessionS"].ToString() + "' and admission_no='" + ViewState["adm_no"].ToString() + "' and Student_discount_type_entry_id='" + ViewState["couponCodE"].ToString() + "') t");
            if (dtaD.Rows.Count > 0)
            {
                if (My.toDouble(dtaD.Rows[0]["Discount_amt"].ToString()) > 0)
                { 
                    SqlCommand cmds;
                    string queryss = "Update Coupon_applied_list set Amount=@Amount,Total_amount=@Total_amount,Discount_persent=@Discount_persent where Coupon_id=@Coupon_id and Session_id=@Session_id";
                    ...
                    cmds.Parameters.AddWithValue("@Coupon_id", ViewState["couponCodE"].ToString()); 
                    cmds.Parameters.AddWithValue("@Session_id", ViewState["session_id"].ToString()); 
                    cmds.Parameters.AddWithValue("@Admission_no", ViewState["adm_no"].ToString()); 
            ...
            Alertme("Discount has been updated sucessfully.", "success");
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModalC();", true);
        }
    }
}
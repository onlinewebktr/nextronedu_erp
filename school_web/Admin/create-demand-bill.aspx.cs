...
{
    public partial class create_demand_bill : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    string isinstallmetWise = My.get_single_column_data("select top 1 Fee_collect_mode as Column_Name from Firm_Details");
                    if (isinstallmetWise == "Installment")
                    {
                        Response.Redirect("create-demand-bill-installment-wise.aspx", false);
                    }
        ...
        {
            DataTable dt = My.dataTable("select firm_id,Is_fine_repeat,Demand_bill_with_month,Is_demand_bill_show_back_dues from Firm_Details");
            ...
            try
            {
                if (dt.Rows[0]["Is_demand_bill_show_back_dues"].ToString() == "True")
                {
                    ViewState["Is_demand_bill_show_back_dues"] = "Yes";
                }
                else
                {
                    ViewState["Is_demand_bill_show_back_dues"] = "No";
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btn_find_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (ddl_student_type.SelectedValue == "1")  //HOSTEL
                    {
                    ...
                    else if (ddl_student_type.SelectedValue == "2") //Days Scholar
                    {
        ...
        protected void btn_final_submit_Click(object sender, EventArgs e)
        {
            try
            {
                string respPage = "print-demand-bill.aspx";
                if (ViewState["Is_demand_bill_show_back_dues"].ToString() == "Yes")
                {
                    respPage = "print-demand-bill-backdues.aspx";
                }

                if (ddl_month_name.SelectedItem.Text == "Select")
                {
                    Alertme("Please choose month.", "warning");
                    ddl_month_name.Focus();
                }
                ...
                else
                {
                    ...
                    if (flag == true)
                    {
                        ...
                        if (k == growcount)
                        {
                            ...
                            if (ddl_section.SelectedItem.Text.ToUpper() == "ALL")
                            {
                                reslink = "slip/" + respPage + "?session_Id=" + ddlsession.SelectedValue + "&branch_id=" + ViewState["branchid"].ToString() + "&classid=" + ddl_class.SelectedValue + "&section=0&admno=0&checked=0&month=" + ddl_month_name.SelectedItem.Text + "&paydate=" + txt_payment_date.Text + "&billType=" + ddl_with.SelectedValue + "&stdType=" + ddl_student_type.SelectedValue;
                            }
                            else
                            {
                                reslink = "slip/" + respPage + "?session_Id=" + ddlsession.SelectedValue + "&branch_id=" + ViewState["branchid"].ToString() + "&classid=" + ddl_class.SelectedValue + "&section=" + ddl_section.Text + "&admno=0&checked=0&month=" + ddl_month_name.SelectedItem.Text + "&paydate=" + txt_payment_date.Text + "&billType=" + ddl_with.SelectedValue + "&stdType=" + ddl_student_type.SelectedValue;
                            }
            ...
            {
                My.submitException(ex, "create-demand-bill.aspx");
            }
        }
    }
}
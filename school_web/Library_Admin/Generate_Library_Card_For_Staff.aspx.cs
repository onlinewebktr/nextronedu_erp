...
{
    public partial class Generate_Library_Card_For_Staff : System.Web.UI.Page
    {
        ...
        protected void Btn_Generate_Click(object sender, EventArgs e)
        {
            ...
            string Prefix = lb.get_prefix(ViewState["Branch_id"].ToString(), "staff");
            string Postfix = lb.get_postfix(ViewState["Branch_id"].ToString(), "staff");
            string serialNo = lb.get_serialNo(ViewState["Branch_id"].ToString(), "staff");
        ...
        public static List<string> Getusername(string username)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                ...
                {
                    cmd.CommandText = "select distinct name from user_details where name LIKE '%'+@name+'%' and Istatus=1 and User_Type != 'Teacher' ";
                    cmd.Connection = con;
                    con.Open();
        ...
        public static List<string> GetMobile(string Mobile)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select distinct mobile from user_details where mobile LIKE '%'+@mobile+'%' and Istatus=1 and User_Type != 'Teacher' ";
                    cmd.Connection = con;
                    con.Open();
        ...
        public static List<string> get_user_id(string user_id)
        {
            ...
            using (SqlConnection con = new SqlConnection(My.conn))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select distinct user_id from user_details where user_id LIKE '%'+@user_id+'%' and Istatus=1 and User_Type != 'Teacher'  ";
                    cmd.Connection = con;
                    con.Open();
        ...
        protected void Txt_Name_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                bind_grd_view("select * from user_details where  name ='" + Txt_Name.Text + "'  and User_Type != 'Teacher' and Branch_id = '" + ViewState["Branch_id"].ToString() + "' and  Istatus='1' User_Type != 'Teacher' order by id desc");
            }
        }

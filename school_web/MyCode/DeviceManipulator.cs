    ...
    public class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetLogData(ZkemClient objZkeeper, int machineNumber)
        {
            ...
            while (objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))  
            {
                ...
                objInfo.Staff_ID = dwEnrollNumber1;
                objInfo.DateTimeRecord = inputDate.ToString();
                objInfo.Attendance_time = inputDate;
        ...
        public List<String> GetAllEnrollID(ZkemClient objZkeeper, int machineNumber, System.Collections.ArrayList existing_user)
        {
            string sdwEnrollNumber = string.Empty, sName = string.Empty, sPassword = string.Empty, sTmpData = string.Empty;
            int iPrivilege = 0;
            bool bEnabled = false;
        ...
        //    objZkeeper.ReadAllUserID(machineNumber); 
            ...
            lstUserid.Add("Auto");
            lstUserid.Add("Manual");
            while (objZkeeper.SSR_GetAllUserInfo(machineNumber, out sdwEnrollNumber, out sName, out sPassword, out iPrivilege, out bEnabled))
            {
                if (!existing_user.Contains(sdwEnrollNumber.ToString()))
                {
                    lstUserid.Add(sdwEnrollNumber.ToString());
                }
            }
            return lstUserid;
        }
        ...
        public bool PushUserDataToDevice(ZkemClient objZkeeper, int machineNumber, string userName, string enrollNo)
        {

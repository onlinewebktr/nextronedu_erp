...
{
    public partial class Sale_Entry : System.Web.UI.Page
    {
        My mycode = new My();
        ...
        public static List<string> GetRooPath(string PathRooT)
        {
            ...
            //using (SqlCommand cmd = new SqlCommand())
            //{
            //    cmd.CommandText = "select distinct party_name,address,mobile,party_id from party_details where (studentname LIKE ''+@SearchMobNo+'%')  ";
            //    cmd.Connection = con;
            //    con.Open();
            ...
            //    while (dr.Read())
            //    {
            //        MobResult.Add(dr["studentname"].ToString()+","+ dr["address"].ToString()+","+ dr["mobile"].ToString()+","+ dr["party_id"].ToString());
            //    }
            //    con.Close();
            //    return MobResult;
            //}
            ...
            else
            {
                if (check_duplicate(txt_regid.Text.Trim()))
                {
                    ...
                    if (mycodeMy.IsUserExist("select Id from admission_registor where admissionserialnumber='" + txt_regid.Text.Trim() + "'"))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@class", ddl_classname.SelectedItem.Text);
                        cmd.Parameters.AddWithValue("@rollnumber", "0");
                        cmd.Parameters.AddWithValue("@Section", txt_section.Text);
                        cmd.Parameters.AddWithValue("@admissionserialnumber", txt_regid.Text.Trim());
                        ...
                        cmd.Parameters.AddWithValue("@Old_Admission_Date", mycode.date());
                        cmd.Parameters.AddWithValue("@OLd_Admission_Idate", mycode.idate());
                        cmd.Parameters.AddWithValue("@Prev_board_type", "STATE");
                        //Father Details
                        cmd.Parameters.AddWithValue("@fathername", txt_father_name.Text.Trim());
                        ...
                        cmd.Parameters.AddWithValue("@father_mob", txt_Mobile_no.Text);
                        cmd.Parameters.AddWithValue("@Father_whatsapp_country_code", "+91");
                        cmd.Parameters.AddWithValue("@Father_whatsApp_no", txt_Mobile_no.Text);
                        cmd.Parameters.AddWithValue("@email_id", "");
                        cmd.Parameters.AddWithValue("@guardianname", txt_father_name.Text.Trim());
                        //Mother Details
                        ...
                        cmd.Parameters.AddWithValue("@Class_id", ddl_classname.SelectedValue);
                        cmd.Parameters.AddWithValue("@Is_TC_Taken", false);
                        cmd.Parameters.AddWithValue("@Student_id", txt_regid.Text.Trim());
                        ...
                        cmd.Parameters.AddWithValue("@Mother_Name_First", txt_mothername.Text.Trim());
                        ...
                        if (My.InsertUpdateData(cmd))
                        {
                            My.create_student_ledger(txt_regid.Text.Trim(), txt_student_name.Text, ddl_gender.Text, txt_dob.Text, "N/A", "N/A", "N/A", txt_Mobile_no.Text, "West Bengal", txt_father_name.Text, mycode.date());
                            ...
                            Alertme("Student information has been saved successfully ", "success");
                        }
                    }
                    else
                    {
                ...
                {
                    lbl_msg2.Text = "Your entered registration number duplicate";
                    Alertme("Your entered registration number duplicate", "warning");
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "open_add_student();", true);
